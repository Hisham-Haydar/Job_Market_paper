...
**Mission:** JMP-M07I — Manuscript Identity Alignment, Stage A (bounded redraft)
**Output file (repository path):** `docs/JMP_core_packet_v2.md`
**Supersedes:** `JMP_core_packet_v1.md` — retained immutable as history
...
`FR_P2a_inference_appendix_note_v2.md`; `JMP_M07_goal_manager_acceptance_v1.md`; `JMP_M08_welfare_input_handoff_v1.md`; `JMP_project_state_latest.md`. Consequence: this document makes no statement about welfare-design internals, appendix content or project-state line numbering beyond what `FR_P2a_empirical_inference_v2.md` establishes. See `JMP_project_state_identity_addendum_v1.md` §5.
...
- Exact construction of the opportunity-equalization and preference-neutralization counterfactuals is fixed by the frozen M08 welfare-input handoff, which was not supplied to Stage A.
- Whether the headline inequality index differs from the Gini (the accepted inference section contemplates both).

**Next authorised action:** Stage B independent consistency review (ChatGPT GPT-5.6 Thinking, high reasoning, no repository writes).
...
- **Do not present couples or pooled years as delivered results**, as co-primary applications, or on a timetable. Equally, do not describe the singles application as a fallback or as obsolete; that characterisation is superseded.
...
This is the application for which accepted estimation and inference evidence exists, and it is therefore the writing anchor for every paper-facing empirical statement. Extension to couples and to pooled survey years is subsequent work, conditional on validation of the singles welfare pipeline.
...
The exact construction of the two counterfactuals is fixed by the frozen M08 welfare-input handoff and is not specified in this document.
