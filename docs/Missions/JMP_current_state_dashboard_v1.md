...
every stored welfare object named W1-W6 in the codebase uses an inclusive
attained value and is classed **DIFFERENT OBJECT** relative to the corresponding
observed-bundle theory measure (never `u(z_obs)`); literal Mapping-F W1 and the
staying-home equivalent W4 agree to machine precision on the three authorized
household checks (worker, nonworker, couple), and NO stored artifact holds the
literal full-sample distribution — it is **NOT IMPLEMENTED**, only checked at
three households. This supersedes MEASURE-MAP-1, which was **NOT accepted**
on review (wrong specification answered, `max` conflated with `log-sum-exp`,
an inverted couples derivation, an invalid hash). **GATE-1's own hard gate
(step 2, mechanical term-by-term comparison) PASSED, 99/99 rows, zero
disagreement** — `docs/normative/baseline_f1_term_check_v1.md`: every S11
parameter the provenance record classifies UTILITY is a leisure term (age,
age², child shifter, intercept, `theta_l`) or the named consumption exception
(`beta_c`, singles' `theta_c_singles`); every parameter appearing in the
utility block at `engine_jax.py:303-308` (singles) / `519-530` (couples)
other than consumption is UTILITY; no OPPORTUNITY-classified parameter
appears there. Only after that PASS: `MNL/gate/measure_map_accepted.json` was
written (never by hand-edit outside this gate) pinning the accepted document's
commit and SHA-256, and the validator it targets,
`scripts/welfare/baseline_f1.py:validate_measure_map_gate` (read, not
modified), was invoked standalone and returned `status:
MEASURE-MAP-1R_ACCEPTED` — confirming the committed document bytes match the
pinned digest and the worktree has no divergence from that commit. **This is
a check-only confirmation that BASELINE-F-1 may now execute; the full-sample
run itself (BASELINE-F1-RUN) is a separately authorized action and was NOT
performed here.** Job_Market_paper `main` was fast-forwarded to
`docs/w1-reference-domain-fork` (`834055e..d58f93b`, fast-forward only,
confirmed possible before attempting); the branch and `main` now coincide.
MNL stays on `welfare/baseline-f1` (commit `d729cf89`), with nothing else in
MNL touched by this pass. R5 (`JMP_counterfactual_attainment_design_v1.md`)
is authorized to begin drafting from this acceptance onward; no counterfactual
execution is authorized. **No welfare number is promoted, no provisional
label is lifted beyond the E1 unlock itself, and no full-sample or
decomposition execution is authorized by any of this.** The prior header,
carried forward — 2026-09-11, at **REC-1 r3 / P1 closure** — R3 (close the FORK-1 record before citation) is now **CLOSED**, not partially open: `docs/normative/scripts/fork_derived_numerals_v1.py` (commit `8dcc476b6008d6926f8f46d99c32d26571976dfa`) reproduces every [derived here] numeral in `JMP_W1_reference_domain_fork_v1.md` against the accepted S11 parameter tables, **158/158, script exit 0**. Four Table-1 cells that had been double-rounded in the first pass are corrected (memo revision r3, cause and source logged in the memo's own revision-log table) and every numeral is promoted from memo-grade to **CITABLE**. **P1 is RESOLVED**: S11 is the accepted baseline for both singles and couples per R-286 and R-291 below; the provisional 2,275-couple `couples_clean_baseline/r240_step3_parameter_table_v1.csv` was used only by the non-accepted MEASURE-MAP-1 return and is not an input to the fork memo. `W1_latent_set_identification_note_v1.md` and `JMP_W1_fork_ruling_v1.md` are now tracked and committed for the first time, alongside the script and its CSV. Still on branch `docs/w1-reference-domain-fork`, NOT merged to `main`; `docs/normative/JMP_measure_map_v1.md` remains excluded, pending Goal 1 acceptance. The prior header, carried forward — 2026-09-11, at the **FORK-1 / BASELINE-F-1 rulings** — the W1 reference-domain fork (Mapping F, full feasible set, vs. Mapping M, market-job reference) is RULED ON. `docs/normative/JMP_W1_reference_domain_fork_v1.md` recommends Mapping F as primary (its first-pass recommendation was M; the flip is logged in the memo's own revision log and followed a prior the Deputy introduced, verified in the memo, that Mapping M's kappa-to-infinity limit is Haydar-Maniquet Measure 6 over the market-job universe, not a household-specific access object). The Deputy's FORK-1 ruling (Appendix A of `JMP_W1_fork_ruling_v1.md`) ADOPTS R1 (Mapping F primary) and R2 (the old ex-ante RURO/inclusive-value W1-EA functional is RETIRED from the baseline — historical artefacts only, not current Haydar-Maniquet W1, not current headline levels, not current P/A/B/D shares). R3 (close the FORK-1 record before citation) **was, at this ruling, PARTIALLY OPEN** (see the REC-1 r3 / P1 closure entry above, now superseding this): the couples max-L reference configuration is resolved term-by-term on the accepted S11 parameter tables (woman-only at 5h is the reference except in a narrow young-woman/large-age-gap corner, threshold beta_f/beta_m > 4.278), the revision log and the memory-replacement check are closed, and the co-author worked-example caveat is retained pending its primary source; the REC-1 deterministic derived-numeral script was at that point NOT yet closed (154/158 numerals reproduced; 4 Table-1 rounding cells were off and awaited authorization to correct), so numerals were memo-grade and not citable in the manuscript. R4 (MEASURE-MAP-1) returned a correspondence audit that was NOT accepted on review (wrong specification answered, `max` vs `log-sum-exp` conflated, function-vs-executed-artifact confusion, an inverted couples derivation, an invalid hash) and is reissued as MEASURE-MAP-1R, the active gate. R5 (counterfactual attainment design) is QUEUED behind MEASURE-MAP-1R. R6 (seminar) is ADOPTED: the seminar may present the Haydar-Maniquet W1 definition and the empirical-domain result "W1 = staying-home equivalent under Mapping F", but may NOT present old W1-EA welfare/decomposition magnitudes as the paper's W1 results, and quantitative P/A/B/D shares require the revised F pipeline to be executed and independently checked first. Two escalations follow in `docs/normative/JMP_W1_BASELINE_F1_authorization_and_Cobs_ruling_v1.md`: **E1 (BASELINE-F-1, the literal closed-form Mapping-F W1 at the observed bundle) is AUTHORIZED IN PRINCIPLE, with full-sample execution LOCKED until MEASURE-MAP-1R is independently accepted** — implementation, tests, output schema and small deterministic household checks may proceed now; **E2 (the consumption argument C_i^obs) is CONFIRMED** as a consistency requirement, not a free normative choice, to be the exact consumption argument of the estimated systematic utility at the observed labour state, with its provenance closed by MEASURE-MAP-1R. All of this lives on branch `docs/w1-reference-domain-fork` (commits `f634623`, `358a2e8`), NOT merged to `main`. **No welfare number is promoted, no provisional label is lifted, and no counterfactual or decomposition execution is authorized by any of this.** The prior header, carried forward — 2026-09-09, at Goal-1 **R-291** — the **v5 pair is on disk and the v4 review is discharged**. v5 is a source-reconciled rewrite, not a patch layer: `manuscript/JMP_working_paper_for_seminar_v5.tex/.pdf/.bib` (34 pages) and `reports/JMP_research_story_report_v5.html`, both generated from ONE source, `reports/research_story_build/v5_sections.py` + `build_v5.py`, so prose, equations, tables, figures and bibliography cannot drift between the two outputs. **ONE MODEL, EVERYWHERE.** The reported specification is the S11 record — tau = 1, theta_c = 0 EXACTLY, beta_c ESTIMATED — and the welfare object is stated as what it is, a weighted power mean of ORDER beta_c, with the arithmetic-mean derivation, the derivative, the figure and the Q&A answer replaced TOGETHER. `beta_c = 1` and `theta_c = 0.168` are now admissible only inside the history section or as an explicit counterfactual comparison, and that is MECHANICALLY ENFORCED. **THE COUPLES BUDGET-CHANNEL SPLIT IS BOUND (R-291 / PRICE-D).** The "reported jointly, split pending" treatment is REMOVED: two counterfactual panels repriced through the tax-benefit system on the corrected 58/46/6 partition, index-specific, summing to the joint contribution at 1.4e-17 and to the induced four-player contribution EXACTLY. Raw Gini couples C_D 0.067827 = resources 0.043551 + composition 0.024276 (64.2 / 35.8 of the channel; 32.9 and 18.3 per cent of total); equivalized 0.072396 = 0.041364 + 0.031032 (57.1 / 42.9). Singles for comparison 24.38 / 9.82 of I00 (71.3 / 28.7 of the channel), on an EARLIER partition of the budget fields, which is disclosed. Three statements are reported as findings: resources lead the channel in BOTH populations on the household basis under all six indices; the resources share is widest under GE(2) and narrowest under the Gini in both, so this channel is far more index-sensitive than the four-way decomposition above it; and equivalizing raises the composition share in both under all six, which is what the coalition-consistent scale DOES — the scale is owned by the composition operator, so equalizing composition also puts every household on a common scale, and for singles that REVERSES the ordering (resources lead in only two of six equivalized indices). **ONE CLAIM IN THE R-291 ADDENDUM IS NOT REPORTED AS A FINDING, AND WHY IS STATED IN THE PAPER:** "composition carries a markedly larger share of the budget channel for couples" holds at the Gini (35.8 against 28.7) and REVERSES under the other five indices, and under all six once equivalized. **THE W1/W4 BRIDGE AND CR1 ARE CLOSED, NOT PENDING.** The premise audit locates the failure in premise (b): the opportunity kernel is NOT unit mass on the domain H uses (median 0.0773 singles, 147.96 couples), so the old signed gaps were SCALE ARTEFACTS; after normalizing on exactly that domain the gap is non-negative for EVERY household in both samples and the ratio is at most one, median W4/W1 0.9843 singles and 0.6242 couples. CR1 parameter intervals from 100 draws are reported on the headline Gini shares and are NEVER merged with the RQMC integration band. **A REPORTABLE SOURCE INCONSISTENCY WITHOUT NUMERICAL CONSEQUENCE:** three values of lambda_c were in circulation because each panel recomputes it as a mean over its OWN rows — estimation frames 1938.238719 / 4247.875047, welfare panel 1774.518218 / 3821.448012. Under theta_c = 0 the constant is alternative-invariant and cancels EXACTLY from the money metric, verified to 2.7e-15 over four widely separated values; NOTHING reported changes and the paper now reports ONE constant per population. **THE GATE SITUATION CHANGED AND THE REASON IS SUBSTANTIVE.** `consistency_gate_spec_v1.md` asserts `beta_c == 1` and an F35 parameterisation in which four band coefficients sit beside a separately estimated 35-hour peak; the S11 record contradicts BOTH — beta_c is estimated and g^H has FIVE band coefficients over one residual reference of width 26.5 h/week. Running that gate against v5 would FAIL the paper for describing the model it estimates. `reports/canonical_notation_v5.md` records the disposition clause by clause (two clauses of s1.1 and the whole of s1.4 superseded, everything else carried forward) and `reports/run_v5_gate.py` enforces the carried-forward rules plus the new SINGLE-CURRENT-MODEL check: **16 items, 16 PASS**. `reports/run_v5_gate_negative_control.py` is the gate’s own test — it injects one violation at a time into a copy and checks the intended item flips to FAIL, **8 of 8 caught**. THREE RULES FAILED THAT CONTROL ON FIRST RUN AND WERE STRENGTHENED: the causal disclaimer accepted either of two sentences where both are needed; the machine-label rule was case-folded, so it flagged the ordinary word "pending"; and the control itself read a CRASHING gate as "no failures", which is now a hard error. `reports/check_v5_pdf_layout.py` measures the RENDERED page rather than trusting the log: it caught a page where longtable mis-accounted the remaining height with a float present and clipped a whole paragraph, and it was itself validated by reverting the fix and confirming it flags the page. **WHAT IS NOT CLAIMED.** The novelty sentence is the licensed conjunction only, the allocation rule is credited as inherited to Owen / Shorrocks / Audoly, Muehlhan (2023) and Creedy-Herault (2011) are engaged as the closest precedents from what the review establishes about them, and the five forbidden priority claims appear ONLY inside the "what is new / what is inherited" box that disowns them. Pay neutrality is stated as the property actually established — the reference is DIRECTLY pay-neutral and earning opportunities reach the measure through ATTAINMENT — with no axiomatic claim in either direction. **TWO FIGURES WERE DROPPED RATHER THAN SHIPPED STALE:** the matched-pair illustration is rendered at the superseded numeraire, and the figD descriptive family is on the predecessor 1,555 / 2,275 frames; figV01-figV09 replace them on the estimation frame. **THE ONE CAPABILITY GAP IS THE NOTEBOOK.** `MNL/experiments/JMP_SEMINAR_SPRINT/JMP_research_lab.ipynb` is specification-driven in its utility construction and carries an estimated consumption weight wherever the specification leaves it free, but the artifacts it loads PRE-DATE S11. Re-pointing is mechanical, re-running is not, and it is left honestly labelled rather than re-pointed and unverified. **No provisional label is lifted here.** The prior header, carried forward — 2026-09-09, at Goal-1 **R-286** — the preference-figure family is REGENERATED AT THE FINAL PARAMETERS and indexed. Seven panels, figP01-figP07, drawn at the S11 SCALE-1 specifications of record (tau = 1, theta_c = 0 exactly, beta_c ESTIMATED at 2.038732 singles / 2.101720 couples) on the corrected criterion-A frames, at `MNL/experiments/JMP_SEMINAR_SPRINT/figures/preferences_final/` from `runs/preference_figures_final/`. It SUPERSEDES the 2026-09-08 family, and not by a redraw: five inputs changed, including BOTH consumption normalizers (1911.108058 -> 1938.238719 singles, 3821.448012 -> 4247.875047 couples), so the old panels are at the wrong coordinate as well as the wrong parameters. THREE GATES, ALL PASS. The closed forms are not a re-implementation: they call the engine's own Box-Cox module, and the parity gate runs the engine's own `_compute_utility_singles` / `_compute_utility_couples` through the real loaders on whole households at the S11 parameter vector, worst absolute difference 3.553e-14 against a 1e-10 tolerance; `box_cox_derivative_theta`, which carries the known Taylor-branch defect, is NOT on this path. The invariance gate puts the leisure-normalizer coordinate change at 4.165e-14 over five normalizers, four households and two grids while the leisure weight itself moves by up to 97 per cent. The third gate is figP07's. The three known defects of `RURO_post_estimation_styled.py` are avoided BY CONSTRUCTION and each is named with its line numbers: no elasticity is computed anywhere (that script's are invented constants), the level sets come from an exact analytic inversion so there is no contour grid to transpose, and every curve sits at a MEDOID with its own age and children rather than at a leisure-weight intercept standing in for median characteristics. FOUR THINGS FOLLOW FROM theta_c = 0. du/dC = beta_c/C, so lambda_c cancels out of the marginal utility of consumption entirely. Nothing is ever truncated. One nat acquires a euro price, C' = C exp(1/beta_c) — a fixed PROPORTION of consumption, +63.31 % singles and +60.93 % couples — which is figP05, and which under beta_c = 1 as a numeraire would have been a definition rather than an estimate. And figP07 exists at all: at theta_c = 0 the W1-EA inversion reads EXACTLY as a weighted power mean of ORDER beta_c of the alternatives' consumption, so beta_c is simultaneously the order of the mean and the elasticity of an alternative's implied weight in its own consumption. At beta_c = 1 the measure is the ARITHMETIC mean; at beta-hat_c an alternative paying twice the median carries 4.11x the weight rather than 2x, and across the observed p5-p95 span the weight ratio is 95.6:1 rather than 9.4:1, so the estimated curvature RAISES W1-EA and tilts it toward the best-paying reachable alternatives. That identity is algebraic and must hold for ANY positive non-consumption weights, so it is tested that way — 1000 cases, 7.11e-16 against a 1e-12 tolerance. **TWO LIMITS ARE STATED ON THE FIGURES THEMSELVES.** The power-mean representation is a consequence of theta_c = 0 and does not exist at theta_c != 0, so figP07 could not have been drawn at the superseded coordinate; and figP07 is the aggregator's KERNEL, NOT a recomputation of W1-EA — the W1-EA LEVELS of record remain those on the superseded fast-lane frame at beta_c = 1, theta_c = 0.0075810. **No welfare level is recomputed, promoted or retracted, and no provisional label is lifted here.** figP06 does NOT settle boundary status and says so: the one active coordinate, beta_l_age2_sf at +1, stays at its bound under re-coordination because the box transforms with the SAME k. 243 `pff_` registry keys; the superseded `pf_` keys are RETAINED unchanged and must not be quoted for current results. The family is already consumed downstream — R-285 embeds all seven panels in the v4 working paper behind `check_figures_v1.py`, and all seven embedded PNGs are BYTE-IDENTICAL to this family's paper renderings. The prior header, carried forward — 2026-09-05, at Goal-1 **R-254** — the seminar package is INTEGRATED and the economics review is APPLIED. The bounded economics / claim-to-evidence review (deputy s10) returned four MATERIAL corrections and two minors; all six are applied and all are enforced mechanically rather than asserted. (3) The deck conflated a reference-only range with an eight-row envelope: changing only the reference convention moves the preference share 6.30 -> 10.89 raw and 7.98 -> 10.97 equivalized, while 3.3-11.0 is over all eight rows and also carries the model and basis choices. (4) The coalition-consistent equivalization is now justified by FACTOR CONSISTENCY - composition is part of the endowments-and-needs factor, so the scale is recomputed at the coalition's composition - with exact closure demoted to a consequence and a check, never the reason. (10) The underemployment evidence is reclassified as EXTERNAL DESCRIPTIVE CONTEXT, NOT A MODEL COMPARISON: the Spearman association and the "bands the model prices as thin" sentence are removed, and Figure 16 panel (a) is REDRAWN from a scatter against the estimated offer weight to a by-band bar chart with no model quantity on either axis, because the model has no desired-hours object to compare against. (12) The 35-hour attribution claim is corrected - the specification PLACES the peak in the offer block and the data do not nonparametrically identify the attribution - and 41 reader-facing occurrences of process vocabulary are gone from the main text and the deck. TWO RESULTS ARE ADMITTED TO THE MAIN TEXT. The common-choice-set benchmark is §6.7: it fits the marginals as well or better while 129.1 nats worse, the availability constants reappear in its utility index at essentially their availability values and the leisure sex gap reverses (+0.428 to -1.991), but the preference SHARE barely moves (6.4 % against 6.3 %) - so the omission UNDERSTATES inequality and RELABELS within the environment rather than turning opportunity into taste, and the misattribution to tastes is on the BEHAVIOURAL side. The nested geographic split of job access is §7.9: geography is 87.62 % (±3.89) of the job-access channel raw and 101.92 % (±3.11) equivalized, 13.05 % (±0.74) and 9.87 % (±0.55) of baseline inequality, with the residual SIGNED and near zero - and the reading is explicitly about the composition of the access block (household-varying geography against a sex-only residual term), NOT a finding that region drives welfare inequality and NOT a causal region effect. The integration verifier now runs SEVEN checks - terminology, headline, figures, benchmark, review corrections, geographic split, and a check that nothing from an unreturned lane is quoted - and returns PASS on all seven. THREE PLACEHOLDERS REMAIN AND ARE NAMED: the DADS external earnings comparison (no matched employer-employee file is accessible), the matched-pair re-selection (the opportunity-distance ranking behind Figures 14-15 omits the region term; the repair has NOT passed its own proof and the pair is NOT re-selected), and parameter-uncertainty propagation with subgroup decompositions (not run; the standing statement that sampling uncertainty is not estimated is unchanged). **No provisional label is lifted here.** Next action: Beamer. The prior header, carried forward — the common-opportunity benchmark, the paper's second headline question, is CLOSED. `RUM_BENCHMARK_STALE_FOR_FINAL_REPORTING_PENDING_CORRECTED_FRAME_UPDATE` is **DISCHARGED**: the benchmark is estimated on the corrected frame against corrected S8 (headline negLL 18151.8526 on 16 free coordinates, df 25, 2 x gap 258.176; companion 18152.2763 on 10, df 31, 2 x gap 259.023), its four-cell decomposition is computed on the same support with the same inversion and both references, and the MISCLASSIFICATION QUANTITY is defined, identity-checked at residual 0.0, and reported. THE RESULT IS NOT THE EXPECTED ONE AND IS REPORTED AS IT CAME OUT: on the money-metric welfare decomposition the conventional analyst does NOT turn opportunity into taste. Of the 35.5% the model attributes to job access plus earning opportunities, -4.0% surfaces as preference contribution, +36.0% reappears as endowments-and-needs, and the rest leaves the measured total, which falls by 24% raw and 15% equivalized. The benchmark's error is UNDERSTATEMENT and relabelling inside the environment. Where the misclassification IS large is the POSITIVE side: the hours constants the benchmark estimates as TASTES are the same numbers the model estimates as AVAILABILITY (mean absolute difference 0.037 over five bands; the 35-hour peak 2.579 as availability, 2.548 as taste), and the leisure-intercept sex gap REVERSES (+0.428 to -1.991). The fit battery measures the literature's own negative finding on this project's data: the benchmark matches every aggregate marginal as well as the model does, in places better, while its log-likelihood is 129 nats worse, so the likelihood comparison is DESCRIPTIVE PROVENANCE and never a contest. `tables/headline_decomposition_v1.csv` is BYTE-UNTOUCHED and the integration verifier still returns PASS on all three checks; the benchmark block lives in `tables/rum_benchmark_decomposition_v1.csv`. Every benchmark number carries `FINAL_RUM_BENCHMARK_PROVISIONAL_PENDING_ECONOMICS_REVIEW`; **no provisional label is lifted here** and the retained positive model is untouched. The prior header, carried forward — 2026-09-04, at Goal-1 **R-245** — the seminar-sprint close-out. **The sprint's scientific core is COMPLETE and the next action is the bounded economics review.** Corrected S8 is the frozen positive model and nothing in the sprint moved it: the age-bound diagnostic (SPR-7) returned `RETAIN_S8_CLOSE`, and both external validation halves that could have disturbed it either confirmed the model (the 35-hour concentration) or were HELD rather than reported (the underemployment axis, `WISHMORE_CODING_UNRESOLVED`). `SPRINT/tables/headline_decomposition_v1.csv` is the SOURCE OF RECORD for every welfare magnitude in the paper — the paper sentence is generated from it so it cannot drift. Singles are the quantitative welfare headline; couples are a sensitivity-bounded companion, by the pre-fixed rule, because the male-leisure sensitivity is MATERIAL in `C_P`. The figure queue is refreshed once and is now exhaustive: 16 paper, 8 couples paper, 9 supporting, 1 withdrawn, **0 missing, 0 unclassified**. The seminar working paper is on disk at `JMP/manuscript/JMP_working_paper_for_seminar_v1.md`. Every welfare number still carries `FINAL_SINGLES_PROVISIONAL_PENDING_ECONOMICS_REVIEW` / `FINAL_COUPLES_PROVISIONAL_PENDING_ECONOMICS_REVIEW`; **no provisional label is lifted here.** The prior header, carried forward for the programme-mode context it still governs — 2026-09-03, at Goal-1 **R-233** (**the programme is in RESEARCH-FIRST / PRESENTATION-FIRST SEMINAR SPRINT mode** — *"MORE RESEARCH, LESS GOVERNANCE"*: a narrow documentation cap, the GPU hands-on research environment as priority zero, a closed singles candidate set, one welfare measure, clean both-flexible couples only, and external identification paused but not abandoned. R-230 and R-231 are **FILLED** — the CPL-A adapter is accepted with C1 escalated, and the HP/HO disposition plus the PKG-03 C1–C4 conventions are ruled: **C1 is the O-5 convention**, C2/C3 are flexible-only, the C4 channel inventory is deferred. One item is **RETURNED**: the seminar parameter count of 35 is not reproduced by any count on disk (51 coordinates / 41 free / 10 pinned / 39 interior), so no parameter count is published until the deputy resolves it. Carried forward unchanged from R-232: **S9 SELECTION IS COMPLETE and corrected S8 is FORMALLY RETAINED as the final positive model** — W3 joined HP and HO as NOT IDENTIFIED, so no heterogeneity extension earned admission and, under the deputy's §9 rule, no S9 extension is invented; "S9" names the selection, not a specification. LOC4 remains the benchmark for the specification-sensitivity finding. `TORCH_BACKEND_JMP_READY` becomes `PENDING_PARITY_EXPORT`; the export itself is a later card, at freeze. No welfare number is promoted and no provisional label is lifted.)
...
|---|---|---|
| **`FORK1_RULING_ACCEPTED`** + **`FORK1_R3_CLOSED`** + **`FORK1_P1_RESOLVED`** + **`MEASURE_MAP_1R_ACCEPTED`** + **`E1_BASELINE_F1_UNLOCKED`** + **`E2_COBS_CONFIRMED`** | `docs/normative/JMP_W1_fork_ruling_v1.md` (Goal 1's implementation record for the Deputy's FORK-1 ruling R1-R6), `docs/normative/JMP_W1_BASELINE_F1_authorization_and_Cobs_ruling_v1.md` (E1/E2), `docs/normative/JMP_measure_map_v1.md` (MEASURE-MAP-1R return, R4) and `docs/normative/JMP_measure_map_acceptance_v1.md` (Goal 1's acceptance of it). R1 (Mapping F primary) and R2 (old W1-EA retired from the baseline) are ADOPTED. **R3 CLOSED** at commit `8dcc476` — REC-1's script reproduces 158/158 memo numerals, exit 0. **P1 RESOLVED** — S11 is the accepted baseline (R-286, R-291); the provisional R-240 couples table was used only by the now-superseded MEASURE-MAP-1. **MEASURE-MAP-1 NOT accepted and superseded; MEASURE-MAP-1R IS ACCEPTED** — every stored W1-W6 object is classed DIFFERENT OBJECT from the observed-bundle theory measure (an inclusive attained value, never u(z_obs)); literal W1-F and W4 agree to machine precision on the three authorized household checks; NOT IMPLEMENTED full-sample. **GATE-1 step 2 (mechanical term-by-term check, hard gate) PASSED, 99/99 rows, no disagreement** (`docs/normative/baseline_f1_term_check_v1.md`, commit `d58f93b`): every UTILITY-classified S11 parameter is a leisure term or the named consumption exception (`beta_c`, `theta_c_singles`); every parameter in the utility block (`engine_jax.py:303-308` singles, `519-530` couples) other than consumption is UTILITY. **E1 (BASELINE-F-1) is now UNLOCKED** — `MNL/gate/measure_map_accepted.json` (commit `d729cf89`) pins the accepted document/commit/hash, and the validator `scripts/welfare/baseline_f1.py:validate_measure_map_gate` confirms it, status `MEASURE-MAP-1R_ACCEPTED`. Full-sample execution (BASELINE-F1-RUN) is a separately authorized next action, not yet run. **E2 (C_i^obs): CONFIRMED** — raw household EUROMOD-priced disposable consumption after the stored take-up rule, unequivalised, `c_norm` in the engine; frozen contract in the accepted map §4. Appendix A of the ruling documents controls on any conflict with this summary | **FORK-1 / E1 / E2 rulings**, 2026-09-11; **R3/P1 closure**, 2026-09-11; **MEASURE-MAP-1R acceptance / GATE-1**, 2026-09-11 |
| **`V5_PAIR_OF_RECORD`** | `manuscript/JMP_working_paper_for_seminar_v5.tex/.pdf/.bib` and `reports/JMP_research_story_report_v5.html`, generated together from `reports/research_story_build/v5_sections.py` + `build_v5.py`. One model (S11: tau = 1, theta_c = 0, beta_c estimated), one welfare formula (power mean of order beta_c), one result population (1,540 / 2,223), in both outputs. Numbers resolve against `reports/numbers_of_record_v5.json`; a token with no source fails the build rather than taking a predecessor value | **R-291** |
| **`V5_GATE_PASSING`** + **`V5_GATE_NEGATIVE_CONTROL_PASSING`** | `reports/run_v5_gate.py`, 16 items, all PASS, written to `reports/consistency_gate_v5.md`; and `reports/run_v5_gate_negative_control.py`, 8 injected violations, all caught. The gate includes the SINGLE-CURRENT-MODEL check: `beta_c = 1` and `theta_c = 0.168` may appear only inside the history section or as an explicit counterfactual comparison | **R-291** |
| **`CONSISTENCY_GATE_SPEC_V1_PARTLY_SUPERSEDED`** | `reports/consistency_gate_spec_v1.md` s1.1 (two clauses: `beta_c == 1`; `theta_c` estimated) and the whole of s1.4 (the F35 peak parameterisation). The S11 record contradicts both, so `run_consistency_gate.py` is **NOT** re-run against v5 and continues to guard its own v1/v2 artifacts. `reports/canonical_notation_v5.md` is the clause-by-clause disposition and the notation authority for v5 | **R-291** |
| **`COUPLES_NESTED_D_COMPLETE`** | the couples subdivision of resources against household composition, from `s12_couples_nested_D_attributions_v1.csv`: two panels REPRICED through the tax-benefit system on the corrected 58/46/6 partition, all 32 states finite and positive, resources + composition - C_D at 1.4e-17 and the discrepancy from the induced four-player C_D exactly 0.0. This DISCHARGES the "report D jointly, no imputed split" instruction. The SINGLES subdivision remains on an EARLIER partition and is reported for comparison with that disclosed | **R-291** |
| **`W1_W4_BRIDGE_CLOSED`** + **`CR1_HEADLINE_INTERVALS_REPORTED`** | the premise audit locates the failure in premise (b), the unit mass of the opportunity kernel on the domain H uses; after normalizing on exactly that domain the gap is non-negative for every household in both samples and the ratio is at most one. CR1 intervals from 100 draws are reported on the headline Gini shares and are never merged with the RQMC band | **R-291** |
| **`LAMBDA_C_CONSTANT_OF_RECORD`** | the estimation-frame value, 1938.238719 (singles) / 4247.875047 (couples). Three values were in circulation because each panel recomputes lambda_c over its own rows; under theta_c = 0 the constant is alternative-invariant and cancels exactly from the money metric, verified to 2.7e-15. **No reported quantity depends on the choice** | **R-291** |
| **`NOTEBOOK_PREDATES_S11`** | `MNL/experiments/JMP_SEMINAR_SPRINT/JMP_research_lab.ipynb`. Its utility construction is specification-driven and carries an estimated consumption weight wherever the specification leaves it free, but the frames, specification and result files it loads are the generation preceding S11. This is the ONE capability gap v5 reports. It is a replay/refit interface from the frozen priced frame onwards and was never source-to-results | **R-291** |
| **`PREFERENCE_FIGURE_FAMILY_FINAL_PARAMETERS`** + **`PREFERENCE_FIGURES_PF_SUPERSEDED`** | the seven preference panels figP01-figP07 at `MNL/experiments/JMP_SEMINAR_SPRINT/figures/preferences_final/`, produced by `runs/preference_figures_final/`. Drawn at the S11 SCALE-1 specifications of record (tau = 1, theta_c = 0, beta_c estimated) on the corrected criterion-A frames, behind three gates — engine parity 3.553e-14, normalizer invariance 4.165e-14, and the W1-EA power-mean identity 7.11e-16 over 1000 cases. Each panel ships a paper PDF, a slide PDF, PNGs, a plotted-numbers CSV and a caption stating population, units, what is plotted and status; every printed number carries a `pff_` registry key. The 2026-09-08 family at `runs/preference_figures/` is **superseded** and marked `SUPERSEDED.md`; its `pf_` keys are retained as history and **must not be quoted for current results**. These are POSITIVE-MODEL preference objects: **no welfare level is recomputed, promoted or retracted**, and figP07 is the W1-EA aggregator's kernel, not a recomputation of W1-EA | **R-286** |
...
| **`SEMINAR_PACKAGE_INTEGRATED_R254`** | the working paper and the seminar deck content. Audience-facing terminology throughout, self-contained captions, the economics review's four material corrections applied, and every numeral traced to a named artefact. Enforced by `SPRINT/runs/headline_table/verify_paper_integration_v1.py`, seven checks, **PASS** | **R-254** |
| **`RUM_BENCHMARK_IN_MAIN_TEXT`** | §6.7 of the working paper and Slide 18 of the deck. The common-choice-set benchmark is admitted as the paper's own counterfactual about method. Its zeros on job access and earning opportunities are the benchmark's definition made arithmetic. The result is NOT the one the premise predicts and is reported as it came out | **R-253** / **R-254** |
| **`GEO_NESTED_ACCESS_PROVISIONAL_PENDING_ECONOMICS_REVIEW`** | §7.9 and Figures 18-19. The nested split of job access into geographic/local-market access and a residual. `C_geo + C_oth = C_acc` to 0.0 on every row and levels one and two are bitwise unchanged, so it is a NESTING of the accepted decomposition, not a re-decomposition. The asymmetry is STRUCTURAL - household-varying geography against a sex-only residual access term - and the reading says so | **R-251.4** / **R-254** |
| **`E1_PAIR_RANKING_IS_REGION_FREE`** | Figures 14-15 only. The stored profile artefact computes its market-access index from region columns that are identically zero on this frame, so the participation numbers plotted there and the opportunity distance that ranks the pairs omit each household's own region term. A repaired builder is specified and has **NOT** passed its own proof; the pair is **not** re-selected. The estimation, the exact table and the geographic split all rebuild the indicators and are unaffected. Disclosed in §7.8 | **R-254** |
...
| ~~`RUM_BENCHMARK_STALE_FOR_FINAL_REPORTING_PENDING_CORRECTED_FRAME_UPDATE`~~ | the RUM benchmark — set at **R-222 s8**, and **DISCHARGED at R-250.2**: the single rerun is done, on the corrected frame, against corrected S8. Both variants converge under the four-leg standard, all nine pre-estimation N-checks pass and the fidelity gate is BITWISE | set **R-222 s8**, discharged **R-250.2** |
| **`RUM_BENCHMARK_CORRECTED_FRAME_COMPLETE`** | the benchmark of record — headline variant (uniform hours factor and uniform employment margin, the six constants relocated into utility) with the availability-retaining variant as companion | **R-250.2** |
| **`RUM_MISCLASSIFICATION_QUANTIFIED`** | the misclassification quantity — defined as the change in the preference contribution, identity-checked at residual 0.0 against 1.4e-14 on every arm and basis, reported on both references and both bases with paired jackknife bands | **R-250.2** |
| **`RUM_LIKELIHOOD_COMPARISON_IS_DESCRIPTIVE_PROVENANCE_ONLY`** | the benchmark-versus-model likelihood gap — never a model-selection verdict; the fit battery measures why | **R-250.2**, on the memo's standing ACW constraint |
| **`N6_THETA_LEG_AT_OPTIMISER_RESOLUTION`** | the constant-invariance theta leg — NOT MET at its declared 1e-10 tolerance and REPORTED, not waived: the null control moves theta-hat further than the shifted run does and the objective is bitwise equal | **R-250.2**, under R-121 B6 |
| **`FINAL_RUM_BENCHMARK_PROVISIONAL_PENDING_ECONOMICS_REVIEW`** | every benchmark number, without exception | **R-250.2** |
...
| **FORK-1 — W1 reference-domain fork (full feasible set F vs. market-job reference M) and BASELINE-F-1** | **R1/R2 ADOPTED** (Mapping F primary; old W1-EA retired from the baseline). **R3 CLOSED** — REC-1's deterministic script reproduces 158/158 memo numerals, exit 0, against the accepted S11 tables (four Table-1 rounding cells corrected as memo revision r3); numerals are CITABLE. **P1 RESOLVED** — S11 is the accepted baseline (R-286, R-291); the provisional R-240 couples table was used only by the now-superseded MEASURE-MAP-1. **MEASURE-MAP-1R ACCEPTED** (R4) — every stored W1-W6 is DIFFERENT OBJECT from the observed-bundle theory measure; literal W1-F/W4 agree to machine precision on 3 authorized household checks; NOT IMPLEMENTED full-sample. **GATE-1 step 2 term-by-term check PASSED, 99/99, no disagreement.** **E1 (BASELINE-F-1) UNLOCKED** — `MNL/gate/measure_map_accepted.json` written and independently validated (`validate_measure_map_gate` returns `MEASURE-MAP-1R_ACCEPTED`); full-sample execution (BASELINE-F1-RUN) not yet run. **E2 (C_i^obs provenance) CONFIRMED.** Job_Market_paper branch `docs/w1-reference-domain-fork` = `main` (fast-forwarded, commit `d58f93b`); MNL branch `welfare/baseline-f1` (commit `d729cf89`) | `docs/normative/JMP_W1_reference_domain_fork_v1.md`; `docs/normative/JMP_W1_fork_ruling_v1.md`; `docs/normative/JMP_W1_BASELINE_F1_authorization_and_Cobs_ruling_v1.md`; `docs/normative/JMP_measure_map_v1.md`; `docs/normative/JMP_measure_map_acceptance_v1.md`; `docs/normative/baseline_f1_term_check_v1.md`; `MNL/gate/measure_map_accepted.json` |
...
**What is deferred BEHIND the review, not abandoned.** ~~The RUM final rerun~~ — **DONE at R-250.2**, ahead of the review rather than behind it, because the benchmark is the paper's second headline question and the review needs it on the table; the label is discharged and the numbers are in decision-note §35 and sprint decision-log Entry 14. The **Torch parity export**
...
| **Preference-figure family of record (R-286)** | `experiments/JMP_SEMINAR_SPRINT/figures/preferences_final/` — figP01-figP07, each as paper PDF, slide PDF, PNGs, plotted-numbers CSV and `captions/<stem>.md`; `figure_index_preferences_final_v1.csv` is the index |
| **Preference-figure producer and its gates (R-286)** | `experiments/JMP_SEMINAR_SPRINT/runs/preference_figures_final/` — `pff_model_v1.py` (the utility of record, the engine-parity gate and the power-mean gate), `run_pff_step1_reference_v1.py`, `run_pff_step2_invariance_v1.py`, `make_pff_figures_v1.py`, `append_pff_registry_v1.py`, and the step records `pff_step1`..`pff_step4`. Re-running the four steps reproduces every CSV and PNG byte-identically |
| **Superseded preference family (pre-R-286)** | `experiments/JMP_SEMINAR_SPRINT/runs/preference_figures/` — retained as the record of what the earlier family printed; its `SUPERSEDED.md` names the five inputs that changed |
...
**Deputy reading of the v5 pair.** The paper and the report are on disk, built
from one source, and every mechanical check passes: 16 of 16 gate items, 8 of 8
injected violations caught by the gate's negative control, no ink outside the
print area on any of the 34 pages, no undefined citations, and every reported
number bound to `reports/numbers_of_record_v5.json`. What remains is a reading,
not a build.

Four items are for the deputy specifically, because each is a judgement the
mechanical checks cannot make.

1. **The R-291 addendum's cross-population claim is NOT reported as a finding.**
   "Household composition carries a markedly larger share of the budget channel
   for couples" holds at the Gini and reverses under the other five indices, and
   under all six once equivalized. The paper states the Gini figures, names the
   reading they invite and says why it is not adopted. If the deputy wants it
   reported as a Gini-specific finding instead, that is a wording change.

2. **The couples budget-channel split is now published**, which the earlier
   instruction ("report D jointly, no imputed split") deferred until a repricing
   existed. It exists and is gated. The joint contribution remains the headline
   component and the split is nested inside it.

3. **The corrected W1/W4 bridge is published** as a property comparison between
   two references, with the failed premise named. The earlier instruction
   withheld any ratio until the reconciliation closed; it has closed.

4. **The notebook is left as it stands**, honestly labelled, rather than
   re-pointed at the current artifacts and shipped unverified. Re-binding and
   re-running it is the one capability gap and the natural next card.

Still open, and unchanged by v5 because neither is a writing problem:

- **the observation rule.** The sample screens on the observed hours and wage of
  employed deciders, so the estimation sample is selected on an outcome of the
  modelled process. The conditional sampled-set likelihood is the right object
  given a sampled choice set; it is not automatically the right object given an
  outcome-selected sample, and which correction the screen requires, if any, is
  not established. Stated in the paper as unresolved.
- **the sampling law.** The sampled-set derivation conditions on a labelled
  collection of slots. The diagnostics are consistent with independent draws but
  do not establish the law by construction. Stated in the paper as unresolved.

Beamer typesetting is unaffected and remains available; the deck content is
frozen at R-254 and its numbers are now superseded wherever they quote a welfare
magnitude, so the deck must be re-bound to the v5 registry before it is set.
