...
**Beamer.** The seminar deck CONTENT is complete and frozen at R-254 —
`JMP/manuscript/JMP_seminar_deck_content_v1.md`, 22 slides at 41.0 minutes with a
4.0-minute slack for a 45-minute slot, plus a marked 25-minute cut at 23.0 minutes
across 16 slides and three backup slides. Every slide carries its message, the
figure or table it carries, speaker notes and a timing, and every number on every
slide is bound in the deck's own provenance table. What remains is **typesetting**:
turning that content into Beamer, with the figures already on disk at PNG and PDF.

Nothing in the scientific core is open. The three named placeholders — the DADS
external earnings comparison, the matched-pair re-selection, and
parameter-uncertainty propagation with subgroup decompositions — are all
**disclosed in the paper's own placeholder table** and none of them blocks the
seminar. No provisional label is lifted, and lifting them is the economics
review's call, not the sprint's.
