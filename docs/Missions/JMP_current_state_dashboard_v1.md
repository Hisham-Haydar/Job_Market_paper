...
|---|---|---|
| **`FORK1_RULING_ACCEPTED`** + **`FORK1_R3_CLOSED`** + **`FORK1_P1_RESOLVED`** + **`E1_BASELINE_F1_AUTHORIZED_IN_PRINCIPLE_LOCKED`** + **`E2_COBS_CONFIRMED`** | `docs/normative/JMP_W1_fork_ruling_v1.md` (Goal 1's implementation record for the Deputy's FORK-1 ruling R1-R6) and `docs/normative/JMP_W1_BASELINE_F1_authorization_and_Cobs_ruling_v1.md` (the Deputy's follow-on ruling on E1/E2). R1 (Mapping F primary) and R2 (old W1-EA retired from the baseline) are ADOPTED. **R3 (close the FORK-1 record before citation) is CLOSED** at commit `8dcc476` — the REC-1 deterministic derived-numeral script (`docs/normative/scripts/fork_derived_numerals_v1.py`) reproduces every [derived here] numeral in the fork memo, **158/158, exit 0**, against the accepted S11 parameter tables; four Table 1 cells that were off by double-rounding in the first pass are corrected (memo revision r3) and logged with cause and source. **P1 (the two couples leisure-curvature tables) is RESOLVED**: S11 is the accepted baseline for both singles and couples (dashboard R-286, R-291); the provisional 2,275-couple R-240 table was used only by the non-accepted MEASURE-MAP-1 and is not an input to the fork memo. MEASURE-MAP-1 is NOT accepted; MEASURE-MAP-1R is the active scientific correspondence gate (R4). **E1 (BASELINE-F-1, the literal Mapping-F W1 at the observed bundle): AUTHORIZED IN PRINCIPLE, FULL-SAMPLE EXECUTION LOCKED until MEASURE-MAP-1R is accepted** — implementation, tests, schema and small deterministic checks may proceed now; the full singles/couples distribution may not be computed. **E2 (the consumption argument C_i^obs): CONFIRMED** as a consistency requirement, not a free normative choice — it must be the exact consumption argument of the estimated systematic utility at the observed labour state, with provenance closed by MEASURE-MAP-1R. Appendix A of the ruling documents controls on any conflict with this summary | **FORK-1 / E1 / E2 rulings**, 2026-09-11; **R3/P1 closure**, 2026-09-11 |
...
| **FORK-1 — W1 reference-domain fork (full feasible set F vs. market-job reference M) and BASELINE-F-1** | **R1/R2 ADOPTED** (Mapping F primary; old W1-EA retired from the baseline). **R3 CLOSED** — REC-1's deterministic script reproduces 158/158 memo numerals, exit 0, against the accepted S11 tables (four Table-1 rounding cells corrected as memo revision r3); numerals are CITABLE. **P1 RESOLVED** — S11 is the accepted baseline (R-286, R-291); the provisional R-240 couples table was used only by non-accepted MEASURE-MAP-1. **MEASURE-MAP-1 NOT accepted**; MEASURE-MAP-1R is the active gate (R4). **E1 (BASELINE-F-1) AUTHORIZED IN PRINCIPLE, FULL-SAMPLE EXECUTION LOCKED pending MEASURE-MAP-1R acceptance. E2 (C_i^obs provenance) CONFIRMED.** Branch `docs/w1-reference-domain-fork` (commit `f634623` fork memo, commit `358a2e8` BASELINE-F-1/E1/E2 ruling, commit `8dcc476` R3/P1 closure), NOT merged to `main` | `docs/normative/JMP_W1_reference_domain_fork_v1.md`; `docs/normative/JMP_W1_fork_ruling_v1.md`; `docs/normative/JMP_W1_BASELINE_F1_authorization_and_Cobs_ruling_v1.md`; `docs/normative/scripts/fork_derived_numerals_v1.py` |
