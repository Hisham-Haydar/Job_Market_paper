...
|---|---|---|
| **`FORK1_RULING_ACCEPTED`** + **`E1_BASELINE_F1_AUTHORIZED_IN_PRINCIPLE_LOCKED`** + **`E2_COBS_CONFIRMED`** | `docs/normative/JMP_W1_fork_ruling_v1.md` (Goal 1's implementation record for the Deputy's FORK-1 ruling R1-R6) and `docs/normative/JMP_W1_BASELINE_F1_authorization_and_Cobs_ruling_v1.md` (the Deputy's follow-on ruling on E1/E2). R1 (Mapping F primary) and R2 (old W1-EA retired from the baseline) are ADOPTED. R3 (close the FORK-1 record before citation) is PARTIALLY OPEN — the REC-1 deterministic derived-numeral script is not yet closed, and P1 (the two couples leisure-curvature tables) affects Mapping-M sensitivity only, not the Mapping-F baseline. MEASURE-MAP-1 is NOT accepted; MEASURE-MAP-1R is the active scientific correspondence gate (R4). **E1 (BASELINE-F-1, the literal Mapping-F W1 at the observed bundle): AUTHORIZED IN PRINCIPLE, FULL-SAMPLE EXECUTION LOCKED until MEASURE-MAP-1R is accepted** — implementation, tests, schema and small deterministic checks may proceed now; the full singles/couples distribution may not be computed. **E2 (the consumption argument C_i^obs): CONFIRMED** as a consistency requirement, not a free normative choice — it must be the exact consumption argument of the estimated systematic utility at the observed labour state, with provenance closed by MEASURE-MAP-1R. Appendix A of the ruling documents controls on any conflict with this summary | **FORK-1 / E1 / E2 rulings**, 2026-09-11 |
...
| **FORK-1 — W1 reference-domain fork (full feasible set F vs. market-job reference M) and BASELINE-F-1** | **R1/R2 ADOPTED** (Mapping F primary; old W1-EA retired from the baseline). **R3 PARTIALLY OPEN** — REC-1 (deterministic derived-numeral reproduction script) not yet closed; P1 (couples leisure-curvature table discrepancy) affects Mapping-M sensitivity only. **MEASURE-MAP-1 NOT accepted**; MEASURE-MAP-1R is the active gate (R4). **E1 (BASELINE-F-1) AUTHORIZED IN PRINCIPLE, FULL-SAMPLE EXECUTION LOCKED pending MEASURE-MAP-1R acceptance. E2 (C_i^obs provenance) CONFIRMED.** Branch `docs/w1-reference-domain-fork` (commit `f634623` fork memo, commit `358a2e8` BASELINE-F-1/E1/E2 ruling), NOT merged to `main` | `docs/normative/JMP_W1_reference_domain_fork_v1.md`; `docs/normative/JMP_W1_fork_ruling_v1.md`; `docs/normative/JMP_W1_BASELINE_F1_authorization_and_Cobs_ruling_v1.md` |
