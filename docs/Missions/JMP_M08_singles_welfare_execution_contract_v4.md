# JMP-M08 Singles Welfare Execution Contract v4 (DRAFT — UNCOMMITTED)

## Supersession note — v3 → v4 (Stage-A correction item 2)

**This document supersedes
`docs/Missions/JMP_M08_singles_welfare_execution_contract_v3.md`. v3 remains on
disk, unchanged, as immutable history.** v4 is the contract of record for
Stage A.

**v4 exists for exactly one reason:** to complete Stage-A correction item 2
(Goal-1 ruling **R-83**), which requires aligning two derivative statements with
the corrected operative V20a definition at §5A.8/§5A.8.1. **The only changes
from v3 are these two sites and this note.** Specifically:

1. **§5.4.1**, V20a row, "v2 disposition" cell: the sentence *"Its
   household-hash identity of the resolved equalised objects is exactly the
   executable test that 'all assigned access, ability, and preference
   arguments are equalised as frozen'. Tolerance `1e-9` unchanged"* is replaced
   with a statement that gate §5.4(1)'s mechanical form is V20a as defined at
   §5A.8/§5A.8.1: exact hash identity on the serialized canonical operands, or
   max-abs-difference `≤ 1e-9` on a common evaluation grid — never a tolerance
   on raw hash equality.
2. **§8.2**, item 2: the sentence *"V20a keeps `1e-9` as a hash-identity
   tolerance under gate §5.4(1)"* is replaced with the same corrected
   statement.

**Nothing else changes.** §5A.8/§5A.8.1's V20a definition itself (the object of
alignment) is untouched — it was already correct as written in v3. No operator,
gate, tolerance value, channel, threshold, or decision is added, removed, or
redefined. **Nothing in v4 is computed.** No welfare computation, no code
change, no data change, no MNL write, no commit accompanies this document.
Stage A still owns the freeze. **Residue scan:** §10.6.

---

## Supersession note — v2 → v3 (Stage-A correction cycle)

**This document supersedes
`docs/Missions/JMP_M08_singles_welfare_execution_contract_v2.md`. v2 remains on
disk, unchanged, as immutable history.** v3 is the contract of record for Stage A.

**v3 exists for exactly one reason:** to apply the four consolidated corrections
required by the independent Stage-A contract review

> `Job_Market_paper docs/Missions/JMP_M08_stageA_contract_review_v1.md`
> sha256 `4733876f3a6e8ca499e93c732bea3e847939bf4907c676c974bb184fde838f11` (15,932 bytes; recomputed on disk for this pass)
> — verdict **PASS-WITH-REQUIRED-CORRECTIONS**, findings **A2.1**, **A5.1–A5.4**,
> **A6.1**, **A6.2** — accepted by Goal-1 **R-80**, which authorizes a **single
> correction cycle → contract v3**.

**The only changes from v2 are those four corrections (C1–C4) and this note.**
Specifically, and bindingly:

- the **deputy's decomposition architecture is unchanged** — `W2` primary, `W5`
  secondary, `W3` validation-only; the three-channel `{A,B,P}` game; no fourth
  channel; `R_bg^k = I^k({A,B,P})` and the two §3.4 identities; the five §3.5
  gates; `s_opp` and the 2pp S-10 trigger;
- the **access, ability and preference operator definitions at §5A are
  unchanged** (ruling §4.10) — C1 corrects only *which object* the preference
  operator's step-(b) operand **is named as** in the D7/U13 register rows, and
  aligns them with the already-ratified §5A.9.1 (R-72 S2). No operator is
  redefined, no reference is moved, no support is changed;
- **every frozen value is unchanged** — `1e-12` (V1/V17/V18), `1e-9` (V20a,
  inversion bracket, `ε_Shapley` coefficient), `1e-6` (reprice parity, inversion
  residual), `1e-8`/`1e-6` (engine parity), ESS `30`, `0.5` nats (U7/V22), the 2pp
  `s_opp` trigger, the D16 material-loading list, `Ā =
  type_conditional_median_opportunity`, `R_h` as the `W5` measure-side reference,
  `held`, `dwt = db090`;
- **no new decision, threshold, operator, measure, channel, or scope is
  introduced.** C3 *removes* a self-adoption and replaces it with a citation; C4
  *removes* text; C1 and C2 correct identification, arithmetic and
  cross-reference errors;
- the **execution block is unchanged and is restated plainly at §5A.8.1, §5.4.2
  and §8.4**: no welfare execution until the Goal 1 Manager ratifies the V6/V22
  materiality numbers and the U6 draw-growth decision on the implementer's
  Stage-A proposals, and the §8.4 conditions are met.

**Correction map:** §10.1. **Residue scans:** §10.2 (C1), §10.3 (C4).
**Worktree effect of this pass:** §10.4.

**Nothing in v3 is computed.** No welfare computation, no code change, no data
change, no MNL write, no commit accompanies this document. Stage A still owns
the freeze.

---

**Supersession (carried forward from v2, unchanged).** v2 **superseded
`docs/Missions/JMP_M08_singles_welfare_execution_contract_v1.md`**. **v1 remains
on disk, unchanged, as immutable history.** *(v3 note: the contract of record for
Stage A is now **v3**; the sentence below is retained as v2 wrote it.)* The only
substantive change from v1 is the incorporation of the Deputy Programme
...
design memos, transcribed at §5A → **v2**, under the
decomposition-architecture ruling → **v3**, under the Stage-A review's four
consolidated corrections as accepted by **R-80** (§10) → **this v4**, under
Stage-A correction item 2 as accepted by **R-83** (§10.6).
...
| **D5** | **Access/ability/preference channel membership of the 47-coordinate certified vector — REPLACED IN v3 (A5.1)** | **Exact partition, extracted from the committed spec and the accepted θ̂ artifacts: preference 18 / ability 6 / access 23 = 47.** **Preference (18):** `beta_l0_{sm,sf,m,f}`, `beta_l_age{,2}_{sm,sf,m,f}`, `beta_l_nkids_{sf,f}`, `theta_l_{sm,sf,f}`, `theta_c_singles`. **Ability (6):** `beta_w0` (common anchor), `beta_w_educL`, `beta_w_educH`, `beta_w_pexp`, `beta_w_pexp2`, `sigma`. **Access (23):** `beta_E`, `beta_h_{pt1,pt2,ft,lh}`; `beta_E_gsur`, `beta_E_drgn2..8`, `beta_E_drgur`, `beta_E_drgmd`, `beta_E_y2015`, `beta_E_y2017`; `beta_occ_{2,3,4}_{m,f}`. `theta_l_m = −0.8`, `beta_ll = 0` and `beta_c = 1.0` are **estimation-fixed values OUTSIDE the 47-coordinate vector** and are **not** channel-membership coordinates. Full factor-by-argument-cell register, status classes, and the reconciliation of v2's erroneous "20" at **§3.1b** | **`[v3]` EXTRACTED, not asserted:** MNL (read-only, tracked) `scripts/bpool/specs/estimation_spec_joint_pooled_v1_bll0_tlmpin.yaml` + `scripts/welfare/configs/theta_hat_p2a_singles2016_v1.csv`; `Job_Market_paper docs/results/FR_P2a_phase5_parameter_reporting_map_v1.csv`. Channel routing per **D1** (§5A.1, R-71) and the access-purity rule **D6**. Source paths, digests and the reconciliation arithmetic at **§3.1b**. *v2's source line — counts "cross-checked against `RURO_welfare_gate_report_W3_v1.md` … preference 20 / ability 6 / access 23 names — MATCH" — is **WITHDRAWN**: those counts sum to 49, the claim was false, and that report was produced on the pooled couples-inclusive build at working-tree HEAD `7cac2e3` (V12)* |
...
| D7 | Equalisation operators (qualitative) | **Access:** set access blocks to a common offer environment; hold ability+preference at actual; recompute. **Ability:** neutralise wage-technology's dependence on own education/experience/residual productivity (`beta_w0` stays anchor); hold access+preference; recompute. **Preference — `[v3]` CORRECTED (A2.1):** substitute the **frozen `dwt`-weighted taste-covariate references** (`āge`, `āge²`, `ēduc_leisure`, `n̄kids`; §5A.6 step (a)) **and** the **ratified singles-female accepted coefficient block** as `θ̄^pref` (§5A.9.1 step (b)); **re-evaluate `u_i` on the FIXED alternative support** at the accepted `θ̂`; recompute. The **singles-male accepted block is the mandatory pre-registered mirror sensitivity** (V23). The operator **does not alter the alternative support, `c_ij`, `π`, or any measure reference** (V2/V3/V4), and it **never re-estimates a coefficient**. **`R_h` is NOT the preference-equalisation operand** — it is exclusively the measure-side reference preference used in constructing `W5` (U13, as corrected). Operative definitions are §5A, **unchanged** (ruling §4.10) | `JMP_welfare_spec_v5.md` §2; scaffold contract §3.2/§7; ability/opportunity-cut memo §5; §5A.6, §5A.9.1; **R-72 S2**; **Stage-A review A2.1** |
...
### 3.1b **`[v3]` D5 in full — the 47-coordinate factor-by-argument-cell register**

**This subsection is NEW IN v3 and exists solely to discharge Stage-A review
finding A5.1.** It replaces v2's D5 count claim, closes **V12**, and confirms
**V7(a)**. It introduces no operator, no channel, no threshold and no
decision: the channel routing is **D1** (§5A.1, ratified R-71) and the
access-purity rule **D6**, both unchanged; only the *membership arithmetic* is
corrected.

#### 3.1b.1 Extraction provenance — read-only, recorded

Every row below was **extracted** from committed artifacts. Nothing is inferred
from prose, and no count is carried over from a report.

| Role | Path | sha256 | Verification |
|---|---|---|---|
| **Certified specification** (defines the free parameter vector, the bounds, the fixed values, and every factor/argument slot) | MNL (tracked) `scripts/bpool/specs/estimation_spec_joint_pooled_v1_bll0_tlmpin.yaml` | `492bcfa9c766bfcb5d8536f5e920cc0b00ffa600b7b89db60b250365f331f211` | git blob `90792aaef0bfa477e63a71af48c38860028b66eb`, **identical at MNL `520441a`** (this contract's front-matter pin) **and at MNL `5b0e3d29e28126e1b3ee0340a243c09755da0b3b`** (HEAD after the R-79 parity closure). Its 16-hex truncation `492bcfa9c766bfcb` is the `spec_hash` printed in the accepted F5 report header quoted at §6.5.3 |
| **Accepted θ̂ (P2a singles-2016), welfare-facing copy** | MNL (tracked) `scripts/welfare/configs/theta_hat_p2a_singles2016_v1.csv` | `0684ee52a3c290749c0e9c30c272db1956990a84a6f4a77f328305e91f0d417c` | git blob `d49fe4e236ce5a0aa78a0925ecd1311736b2b086`, identical at `520441a` and `5b0e3d2`. **47 data rows** |
| **Accepted θ̂, estimation-side copy** | MNL (tracked) `outputs/p2a_singles2016/region_live_v1/phase3_estimation_v1/complete/theta_estimated.csv` | `8b2f3fcbc7427712ac05576e9da32ece87101381054a619db47d337c4dd3eb97` | git blob `8205ee566828b8d4a2a72fd32c5577f8767b1663`. Parameter-name column **identical, in identical order**, to the welfare-facing copy |
| **Accepted Phase-5 parameter reporting map** (supplies the per-coordinate `status`, `active_bound_side`, `pin_reason` classification) | `Job_Market_paper docs/results/FR_P2a_phase5_parameter_reporting_map_v1.csv` | `89a0465cc55f4bc05898559120591e4c28db15a18992bd2b33ba6538ce7b8481` | **exact match to the digest already bound at §3.2a(U1)**. **47 data rows**; `parameter_name` column identical, in identical order, to both θ̂ copies |

**Read-only discipline.** No MNL file was written, and no computation was
performed: the register is a transcription of names, spec slots and recorded
status flags. Digests were recomputed for verification only.

**Corroboration of D17, incidental.** The reporting map's `ci_robust_low` for
`beta_l0_sf` is the value D17 already records for the vigilance coordinate. D17
is unchanged.

#### 3.1b.2 The reconciliation — where v2's "20" came from, and why it is wrong

v2's D5 asserted **preference 20 / ability 6 / access 23 = "MATCH"**. Those
counts sum to **49**, not 47, which is exactly the contradiction V12 recorded and
finding A5.1 requires resolved. The arithmetic resolves exactly, in two errors
that partially offset:

| Step | Effect on the preference count |
|---|---|
| v2's D5 preference **name list** (`beta_l0_{sm,sf,f}`, `beta_l_age{,2}_{sm,sf,m,f}`, `beta_l_nkids_{sf,f}`, `theta_l_{sm,sf,f}`, `theta_c_singles`) enumerates | **17** |
| **Error 1 — one true coordinate OMITTED:** `beta_l0_m` is a free coordinate of the certified vector (it appears in the spec's `initial_values` and `bounds`, and in all three accepted θ̂ artifacts) but is absent from v2's list | **+1 → 18** |
| **Error 2 — three NON-coordinates COUNTED:** `theta_l_m = −0.8` (spec `fixed_params`), `beta_ll = 0` (removed from the vector; spec comment *"beta_ll REMOVED — fixed at 0.0 … not estimated"*) and `beta_c = 1.0` (spec `utility.consumption.fixed_value`, the scale numeraire) are **estimation-fixed values outside the free vector**. None appears in any accepted θ̂ artifact | **v2 added +3 → 20** |
| **Corrected preference count** | **18** |

`18 + 6 + 23 = 47` — **exact**. Equivalently, `49 − 3 + 1 = 47`.

**Binding consequence, stated so it cannot recur.** `theta_l_m`, `beta_ll` and
`beta_c` are **not channel-membership coordinates**. They are inputs to the
evaluated functional form, held at their fixed values in every coalition. They
are correctly discussed at §5A.6 §3.3 (the `held` switch) and §5A.9.5 (the
caveat block) — those passages are **unchanged** — but they are **never counted**
in a channel register.

#### 3.1b.3 The register — 47 coordinates, exact partition

Channel assignment is **D1** (§5A.1) plus **D6** access-purity, both unchanged.
"Argument cell" is the slot the channel's operator substitutes; where the cell
carries no household-varying argument, the operator's action on it is the
**identity** and the coordinate is marked **degenerate**.

**Preference — `u_i` cells (18).** Operator `P`: step (a) substitutes the frozen
taste-covariate references; step (b) substitutes the ratified singles-female
coefficient block `θ̄^pref` (§5A.6, §5A.9.1; C1).

| # | Coordinate | Spec slot | Factor · argument cell | Substituted by | Status class |
|---|---|---|---|---|---|
| 1 | `beta_l0_sm` | `utility.leisure.intercept` (singles-male block) | `u_i` · leisure intercept — no household argument | step (b) block substitution | interior · **S-10 scenario-2 coordinate** |
| 2 | `beta_l_age_sm` | `utility.leisure.shifters[age_norm]` (sm) | `u_i` · `age_norm` | `āge` (a) + block (b) | interior |
| 3 | `beta_l_age2_sm` | `utility.leisure.shifters[age_norm2]` (sm) | `u_i` · `age_norm2` | `āge²` (a) + block (b) | **ACTIVE BOUND — upper, `1.0`** |
| 4 | `theta_l_sm` | `utility.leisure.box_cox_exponent` (sm) | `u_i` · leisure curvature — no household argument | step (b) | interior |
| 5 | `beta_l0_sf` | `utility.leisure.intercept` (sf) | `u_i` · leisure intercept | step (b) | interior · **D17 vigilance coordinate** |
| 6 | `beta_l_age_sf` | shifter `age_norm` (sf) | `u_i` · `age_norm` | `āge` (a) + block (b) | interior |
| 7 | `beta_l_age2_sf` | shifter `age_norm2` (sf) | `u_i` · `age_norm2` | `āge²` (a) + block (b) | **ACTIVE BOUND — upper, `1.0`** |
| 8 | `beta_l_nkids_sf` | shifter `n_children` (sf; `gender_specific: true`) | `u_i` · `n_children` | `n̄kids` (a) + block (b) | interior |
| 9 | `theta_l_sf` | `utility.leisure.box_cox_exponent` (sf) | `u_i` · leisure curvature | step (b) | interior |
| 10 | `theta_c_singles` | `utility.consumption.singles_box_cox_exponent` | `u_i` · consumption curvature | step (b) | interior |
| 11 | `beta_l0_m` | `utility.leisure.intercept` (couples-male) | `u_i` — **couples-only** | — | **PINNED · couples-only, INACTIVE for singles** |
| 12 | `beta_l_age_m` | shifter `age_norm` (couples-male) | `u_i` — couples-only | — | **PINNED · couples-only, INACTIVE** |
| 13 | `beta_l_age2_m` | shifter `age_norm2` (couples-male) | `u_i` — couples-only | — | **PINNED · couples-only, INACTIVE** |
| 14 | `beta_l0_f` | `utility.leisure.intercept` (couples-female) | `u_i` — couples-only | — | **PINNED · couples-only, INACTIVE** |
| 15 | `beta_l_age_f` | shifter `age_norm` (couples-female) | `u_i` — couples-only | — | **PINNED · couples-only, INACTIVE** |
| 16 | `beta_l_age2_f` | shifter `age_norm2` (couples-female) | `u_i` — couples-only | — | **PINNED · couples-only, INACTIVE** |
| 17 | `beta_l_nkids_f` | shifter `n_children` (couples-female) | `u_i` — couples-only | — | **PINNED · couples-only, INACTIVE** |
| 18 | `theta_l_f` | `utility.leisure.box_cox_exponent` (couples-female) | `u_i` — couples-only | — | **PINNED · couples-only, INACTIVE** |

**Ability — `g^W` cells (6).** Operator `B` substitutes `ē`, `p̄ₑₓₚ`, `p̄ₑₓₚ²`
(§5A.3 §2.1–§2.2).

| # | Coordinate | Spec slot | Factor · argument cell | Substituted by | Status class |
|---|---|---|---|---|---|
| 19 | `beta_w0` | `wage_opportunity.mean_shifters[intercept]` | `g^W` · `μ_i` intercept — **common anchor**, no household argument | — (**anchor: `B` does not substitute it**, D7) | interior · **DEGENERATE identity cell** |
| 20 | `beta_w_educL` | `mean_shifters[educL]` | `g^W` · `μ_i`, `educL` dummy | `ē` (dwt share vector) | interior |
| 21 | `beta_w_educH` | `mean_shifters[educH]` | `g^W` · `μ_i`, `educH` dummy | `ē` (dwt share vector) | interior |
| 22 | `beta_w_pexp` | `mean_shifters[pexp_years]` | `g^W` · `μ_i`, `pexp` | `p̄ₑₓₚ` | interior |
| 23 | `beta_w_pexp2` | `mean_shifters[pexp_years2]` | `g^W` · `μ_i`, `pexp²` | `p̄ₑₓₚ²` (**index-mean**, R-72 S1) | interior · **W-4 flagged; S-10 scenario-3 coordinate** |
| 24 | `sigma` | `wage_opportunity.variance.parameter` | `g^W` · dispersion — one common `σ`, no household argument | — (§5A.3 §2.3) | interior · **DEGENERATE identity cell** |

**Access — `g^E`, `g^H`, `g^Occ` cells (23).** Operator `A` substitutes `ḡsur`,
the dwt share-weighted region index, and the sex-pooled occupation availability
(§5A.2 §3.2–§3.3).

| # | Coordinate | Spec slot | Factor · argument cell | Substituted by | Status class |
|---|---|---|---|---|---|
| 25 | `beta_E` | `hours_opportunity.shifters[working]` | `g^E` · employment-margin level; argument is the **row attribute** `working`, not a household covariate | — | interior · **DEGENERATE identity cell** |
| 26 | `beta_h_pt1` | `hours_opportunity.shifters[working_pt1]` | `g^H` · hours band | — (U12 §3.4: `g^H` carries no household covariates) | interior · **DEGENERATE identity cell** |
| 27 | `beta_h_pt2` | `shifters[working_pt2]` | `g^H` · hours band | — | interior · **DEGENERATE identity cell** |
| 28 | `beta_h_ft` | `shifters[working_ft]` | `g^H` · hours band | — | interior · **DEGENERATE identity cell** |
| 29 | `beta_h_lh` | `shifters[working_lh]` | `g^H` · hours band | — | interior · **DEGENERATE identity cell** |
| 30 | `beta_E_gsur` | `market_opportunity.shifters[gsur × working]` | `g^E` · GSUR level | `ḡsur` (dwt mean) | interior |
| 31 | `beta_E_drgn2` | `market_opportunity.shifters[reg2 × working]` | `g^E` · region indicator | dwt **share-weighted index** (R-71 R4) | interior |
| 32 | `beta_E_drgn3` | `shifters[reg3 × working]` | `g^E` · region indicator | share-weighted index | interior |
| 33 | `beta_E_drgn4` | `shifters[reg4 × working]` | `g^E` · region indicator | share-weighted index | interior |
| 34 | `beta_E_drgn5` | `shifters[reg5 × working]` | `g^E` · region indicator | share-weighted index | interior |
| 35 | `beta_E_drgn6` | `shifters[reg6 × working]` | `g^E` · region indicator | share-weighted index | interior |
| 36 | `beta_E_drgn7` | `shifters[reg7 × working]` | `g^E` · region indicator | share-weighted index | interior |
| 37 | `beta_E_drgn8` | `shifters[reg8 × working]` | `g^E` · region indicator | share-weighted index | interior |
| 38 | `beta_E_drgur` | `shifters[drgur × working]` | `g^E` · urbanisation indicator | share-weighted index | interior |
| 39 | `beta_E_drgmd` | `shifters[drgmd × working]` | `g^E` · urbanisation indicator | share-weighted index | interior |
| 40 | `beta_E_y2015` | `shifters[year_2015_indicator × working]` | `g^E` · survey-year indicator — **identically zero** on the 2016-only sample | — | **PINNED · DEGENERATE identity cell** (`pin_reason: structurally_inapplicable__identically_zero_covariate_2016_only`) |
| 41 | `beta_E_y2017` | `shifters[year_2017_indicator × working]` | `g^E` · survey-year indicator — identically zero on 2016-only | — | **PINNED · DEGENERATE identity cell** (same `pin_reason`) |
| 42 | `beta_occ_2_m` | `occupation_opportunity.shifters[loc4_2, applies_to: male]` | `g^Occ` · **`dgn` (sex)** argument — sex selects the block | sex-pooled availability `p̄(loc4 \| ·)` (U12 §3.3) | interior |
| 43 | `beta_occ_3_m` | `[loc4_3, male]` | `g^Occ` · `dgn` | sex-pooled availability | interior |
| 44 | `beta_occ_4_m` | `[loc4_4, male]` | `g^Occ` · `dgn` | sex-pooled availability | interior |
| 45 | `beta_occ_2_f` | `[loc4_2, applies_to: female]` | `g^Occ` · `dgn` | sex-pooled availability | interior |
| 46 | `beta_occ_3_f` | `[loc4_3, female]` | `g^Occ` · `dgn` | sex-pooled availability | interior |
| 47 | `beta_occ_4_f` | `[loc4_4, female]` | `g^Occ` · `dgn` | sex-pooled availability | interior |

**Totals — reconciled.**

| Cut | Counts | Total |
|---|---|---|
| **Channel** | preference **18** · ability **6** · access **23** | **47** ✓ |
| **Status class** (accepted reporting map) | interior **35** · active bound **2** · pinned **10** | **47** ✓ |
| **Status × channel** | preference 8 interior + 2 active-bound + 8 pinned = 18; ability 6 interior = 6; access 21 interior + 2 pinned = 23 | **47** ✓ |
| **Pins, by reason** | 8 × `structurally_inapplicable__unreferenced_by_singles_objective` (couples-only, rows 11–18) · 2 × `structurally_inapplicable__identically_zero_covariate_2016_only` (rows 40–41) | **10** ✓ |
| **Degenerate identity cells** (coordinate exists; operator action is the identity) | rows 19, 24, 25, 26, 27, 28, 29, 40, 41 | **9** |
| **Outside the vector, NOT counted** | `theta_l_m = −0.8`, `beta_ll = 0`, `beta_c = 1.0` | **3** |

**The two active-bound coordinates are `beta_l_age2_sm` and `beta_l_age2_sf`**,
both at the **upper** bound `1.0`, both **preference**, both **singles**. This is
exactly what §5A.6 §3.4 states ("two active upper bounds at 1.0"), and §5A.6
§3.4's two consequences — the boundary-aware-inference restriction and the
neutralisation of the exposure once `age_norm2` is equalised — stand **unchanged**.

#### 3.1b.4 Cells declared by D1 that carry **no coordinate** in the certified spec

D1's cell routing is a routing of **cells**, not a claim that every cell is
occupied. Enumerating the certified spec against D1 shows five declared cells
with **no θ coordinate**. Recording them is what makes the partition of §3.1b.3
exact; **none of them changes an operator, a channel assignment, or a gate.**

| Declared cell | D1 channel | Finding on the certified spec | Disposition |
|---|---|---|---|
| `u_i` · **education-in-leisure** (R9 §3.1(a) names a `beta_l_educH_*` taste shifter; R-72 **S3** routes education-in-leisure → preference) | preference | `utility.leisure.shifters` carries **only** `age_norm`, `age_norm2`, `n_children`. **No education-in-leisure coordinate exists.** | The `ēduc_leisure` reference of §5A.6 §3.2 multiplies **no coefficient**; its substitution is the identity. **S3's routing decision stands** and binds any future specification that adds the term. Recorded as a specification limit |
| `g^E` · **education-interacted cells** (GSUR×educ and any `educ`-interacted hours/market term) | ability | `market_opportunity.shifters` carries **no** `educ` interaction; `hours_opportunity.shifters` carries **no** `educ` term | **U12 §3.4's Stage-A enumeration duty is DISCHARGED: none found.** §5A.5.1's four-cell GSUR×educ slot table is retained **unchanged** as the governing rule and is **coordinate-empty** under this spec |
| `g^Occ` · **`educ3` conditioning argument** | ability | `occupation_opportunity` conditions on `applies_to: male \| female` only; **no `educ3` term** among the 47 | The `educ3` conditioning of §5A.4's `Π` is carried by the **availability table's conditioning argument**, not by a θ coordinate. **V13 binds the engine object at Stage A**; that duty is unchanged and is now explicitly load-bearing |
| `g^W` · **`δ_occ[loc4_j]` occupation mean-shift** | ability | `wage_opportunity.mean_shifters` = `intercept`, `educL`, `educH`, `pexp_years`, `pexp_years2`. **No occupation term in the wage index** among the 47 | §5A.3 §2.4's `A`–`B` interaction term has **no coordinate carrier** under the certified spec. §2.4 is retained **unchanged** (it is the governing rule, and LOC4 reopens the cell per §2.3's forward statement). **Stage A must bind this against the engine under V13 / V7(b) and record the finding** |
| `π` | **none** (sampling artifact) | not a θ coordinate; never touched by any operator | **V3** (π-invariance), unchanged |

#### 3.1b.5 **V7(a) — CONFIRMED against the corrected register**

V7(a) requires: *"the 47 coordinates form an exact partition over
preference/ability/access cells — no coordinate in two channels, none omitted."*

- **None omitted:** rows 1–47 enumerate the parameter-name column of the accepted
  θ̂ artifacts in order; the count is 47 in all three artifacts.
- **No coordinate in two channels:** each row carries exactly one channel.
- **No coefficient's argument slots split across channels.** What splits across
  channels is the **observable**, through *distinct coefficients in distinct
  factors* — which is D6's own "one variable, two cells, two channels, never
  double counted" and creates no shared coordinate:

| Observable | Preference carrier | Ability carrier | Access carrier |
|---|---|---|---|
| **Age** | `beta_l_age_*`, `beta_l_age2_*` (`age_norm`, `age_norm2` in `u_i`) | `beta_w_pexp`, `beta_w_pexp2` (`pexp`, `pexp²` in `g^W`) | **none** — verified against the spec, per §5A.2 §3.5's halt requirement |
| **Education** | **none in this spec** (§3.1b.4 row 1) | `beta_w_educL`, `beta_w_educH` | **none** — access-purity D6; the `g^E`/`g^Occ` education cells are ability-assigned and coordinate-empty (§3.1b.4) |
| **Gender** | the `sm`/`sf` group blocks (gender-in-tastes) | — | `beta_occ_*_m` / `beta_occ_*_f` (gender-in-offers) |

  The shared-cell machinery of §5A.5.1 (GSUR×educ) and §5A.4 (the `Π`
  conditionals) is retained **unchanged** as the governing rule; under the
  certified 47-coordinate spec it is coordinate-empty (§3.1b.4). **V7(a) holds
  exactly. V7(b) — the bitwise `log g^W` invariance test — is unchanged and
  remains the operative non-double-counting check.**

#### 3.1b.6 **V12 — CLOSED**

V12 required Stage A to *"reconcile the arithmetic against the frozen spec YAML
and the accepted θ̂ (handoff §1) and **halt** if the declared membership is not an
exact partition."*

**The reconciliation has been performed against exactly those two sources**
(§3.1b.1), it resolves exactly (§3.1b.2), and the declared membership **is** an
exact partition (§3.1b.3, §3.1b.5). **V12 is CLOSED. The halt does not fire.**

The **49-name count is not imported**: it originated in
`RURO_welfare_gate_report_W3_v1.md`, produced on the pooled couples-inclusive
build at working-tree HEAD `7cac2e3`, which V12 itself flags as *"indicative for
P2a singles and … not imported as validated"*. v2's D5 nonetheless recorded it as
"MATCH". **That claim is withdrawn** and is replaced by §3.1b.3.
...
| U8 | Shapley-exhaustiveness numeric tolerance | v1 proposal only; no constant anywhere in `scripts/welfare/` | An explicit numeric epsilon | **Yes, narrow** — needed for the §5.4 gates to be checkable in code | **RESOLVED — FROZEN AS `ε_Shapley` BY WRITTEN GOAL-1 RATIFICATION. `[v3]` CORRECTED (A6.1).** The decomposition-architecture ruling §3.5(2)–(3) writes the gates in terms of `ε_Shapley` without fixing its value. **This contract does NOT fix it.** The value is fixed by **`docs/Missions/JMP_M08_goal1_rulings_document_v1.md`** entries **R-60** (*"U8 FROZEN (convention class): ε = 1e-9·max(1,\|I(Ω)\|)"*) and **R-76** (*"ε_Shapley for the §3.5 arithmetic and total-accounting gates = the U8 frozen convention 1e-9·max(1,\|I^k(∅)\|)"*). This contract **cites** that ratification; it does not make it (§3.2a(U8), §5.4.2). The **integration stability tolerance remains separate and governing** for each `I^k(S)` and is **not** replaced by `ε_Shapley` |
...
| U13 | **`[v3]` RE-TITLED (A2.1):** Reference preference `R_h` — the **measure-side** reference preference for `W5` *(v2 titled this "for preference-equalisation", which mis-identified the decomposition operand)* | Horizontal reference `R^h` named as "the natural choice" | A hard freeze | Nonblocking for `W3`; **blocking before Stage F** (as a `W5` **measure** reference) | **RESOLVED** — R-59 ratifies the named default `R_h` **as the `W5` measure-side reference preference only**; verbatim source in §3.2a(U13). **The preference-equalisation operand is NOT `R_h`**: it is the frozen taste-covariate references plus the ratified singles-female `θ̄^pref` block (§5A.6, §5A.9.1, R-72 S2), with the singles-male block as the mandatory mirror |
...
#### U8 — Shapley-exhaustiveness epsilon · **RESOLVED — FROZEN AS `ε_Shapley` BY WRITTEN GOAL-1 RATIFICATION**

**`[v3]` REWRITTEN under Stage-A review finding A6.1.** The review found that v2
self-adopted this tolerance — *"the ruling … states the two arithmetic gates in
terms of `ε_Shapley` without fixing its value; **this contract fixes it**"* — and
ruled, correctly, that **the contract cannot ratify its own proposed tolerance**;
an absent threshold must trigger a bounded design conflict, not an invention
(charter discipline). **That language is WITHDRAWN.** The value is not fixed by
this contract. It is fixed by a written Goal-1 ratification, cited below.

**The ratifying instrument, verified on disk for this pass:**

| Field | Value |
|---|---|
| Path | `Job_Market_paper docs/Missions/JMP_M08_goal1_rulings_document_v1.md` |
| SHA-256 | `e663802712b2e004711beb327432a43e64501a47d82fcd7d6a2589c3e689ae2d` |
| Bytes | 8,686 |
| Class | Goal-1 Manager governance record — the **written primary text** for rulings that previously existed only in the Goal-1 Manager chat |
| Issued | 2026-08-07, expressly *"to close the auditability gap recorded in `JMP_M08_goal1_rulings_register_v1.md`"* and to supply *"the written ratification of ε_Shapley (R-60.5, R-76.2) **required by Stage-A review finding A6.1**"* |
| Tracking | Untracked at this HEAD; rides the Stage-A freeze commit |
| Precedence | *"Where this summary and a binding deputy instrument differ, the deputy instrument controls."* No such difference arises here: the deputy ruling supplies the **symbol** `ε_Shapley` and leaves the **value** open; this document supplies the value |

**Entry R-60 (2026-08-06), quoted verbatim** — the convention itself:

> **U8 FROZEN (convention class):
> ε = 1e-9·max(1,|I(Ω)|)** for Shapley numerical-identity checks — a
> numerical convention analogous to the finite-sample correction, not a
> scientific threshold.

**Entry R-76 (2026-08-07), quoted verbatim** — the attachment of that convention
to the decomposition-architecture ruling's gates, and the per-measure anchoring:

> **ε_Shapley for the §3.5 arithmetic and total-accounting gates =
> the U8 frozen convention 1e-9·max(1,|I^k(∅)|) — hereby the written
> Goal-1 ratification (with R-60.5) that contract v3 cites.**

**The ratified value, therefore:**

```text
ε_Shapley  =  1e-9 · max(1, |I^k(∅)|)          for each k ∈ {W2, W5}
```

**Three things this citation settles, so nothing is left self-adopted:**

1. **The value** — `1e-9 · max(1, |·|)` — is R-60's frozen U8 convention, and its
   **class** is fixed there too: a *numerical convention*, "not a scientific
   threshold". The rationale is the ratifier's, not this contract's.
2. **The attachment** — that this convention *is* `ε_Shapley` for gates
   §5.4(2)–(3) — is R-76, which names those gates directly.
3. **The anchoring** — the reference magnitude `|I^k(∅)|` rather than R-60's
   `|I(Ω)|` — is **also R-76's**, which writes `|I^k(∅)|` explicitly. v2 had
   re-anchored this on its own reasoning ("v1's `|I(Ω)|` phrasing had no
   per-measure referent once two measures are decomposed"); **that reasoning is
   now surplus, and the re-anchoring rests on R-76, not on this contract.**

*Search record, retained from v2 and unchanged:* search of
`MNL scripts/welfare/` (`*.py`, `*.yaml`) for `shapley` returns **zero matches** —
no Stage-One implementation exists, and no competing constant exists to be
overridden.

**Stage-A verification duty.** Confirm the digest above against the file at
freeze, and confirm that R-60 and R-76 read as quoted. **If the document is
absent or the entries do not read as quoted, U8 returns to PENDING and
decomposition execution is blocked** — the review's stated alternative.
...
**U13 — reference preference `R_h`** (line 281, in the `reference_preference` block):

```yaml
      W5: "R_h"                                       # horizontal reference preference
```

**`[v3]` CORRECTED (Stage-A review A2.1).** Ratified: the named default **`R_h`**
(the horizontal reference preference) **as the measure-side reference preference
used in constructing `W5`**, and for that purpose only.

**Read the quoted YAML line as it is keyed.** The scaffold contract's block is
`reference_preference`, and the key on the quoted line is the **measure** `W5`.
`R_h` is therefore a *measure* reference, on the Exercise-A welfare side. It is
**not** the Exercise-B preference-equalisation operand.

**The preference-equalisation operand, stated once and bindingly** (R-72 S2,
already ratified and transcribed at §5A.9.1; operator definition at §5A.6, both
**unchanged**):

| Preference-operator step | Operand | Status |
|---|---|---|
| **(a)** covariate arguments | the **frozen `dwt`-weighted taste-covariate references** `āge`, `āge²`, `ēduc_leisure`, `n̄kids` (§5A.6 §3.2; R-71 R6; index-mean convention R-72 S1) | ratified, frozen, hash-recorded before execution |
| **(b)** group-specific coefficient block `θ̄^pref` | the **ratified singles-female accepted coefficient block** | ratified (R-72 S2, §5A.9.1) |
| **(b) mirror** | the **singles-male accepted coefficient block** | **mandatory** pre-registered mirror sensitivity, recorded under **V23** before any coalition value |

The operator **re-evaluates `u_i` on the FIXED alternative support** at the
accepted `θ̂`. It does **not** change the alternative support, the row count,
`c_ij`, `π`, or any measure reference (V2/V3/V4, confirmed unchanged by ruling
§4.8), and it **never re-estimates a coefficient** (charter §5.3, §10; V19).
Wherever v2's D7 said the preference operator "revalues feasible sets", read:
*re-evaluates utility on the fixed alternative support*.

**Why the two objects must not be identified** (ability/preference operator memo,
ratified at R-72, quoted in the Stage-A review at A2.1):

> `θ̄^pref` ≠ `R^h`; the two are distinct objects and must not be identified.

Reusing `R^h` in the preference operator would collapse the Exercise-A welfare
reference into the Exercise-B decomposition operator, violating the scaffold's
measure/decomposition non-double-interpretation guard.

**Consequences recorded, so nothing else moves.** `R_h` remains frozen and
unchanged as the `W5` measure-side reference; `W5` remains the **secondary**
decomposition (D19); V2's reference-invariance gate covers `R_h` as a measure
reference (§5A.8 V2, as corrected under C2(d)). No operator, coalition, gate,
tolerance or measure changes under this correction — only the *naming of the
operand* in D7 and U13, which §5A had already stated correctly.
...
| **V20a** — analytic degeneracy: resolved equalised `u` block, taste covariates and every `g` factor **hash-identical across households** under `{A,B,P}`, tolerance `1e-9` | **RETAINED — as the mechanical form of gate §5.4(1) operator completeness.** **`[v4]` CORRECTED (Stage-A correction item 2):** gate §5.4(1)'s mechanical form is V20a as defined at §5A.8/§5A.8.1: **exact hash identity on the serialized canonical operands, OR max-abs-difference `≤ 1e-9` on a common evaluation grid — never a tolerance on raw hash equality.** Failure action unchanged (gate) | gate **§5.4(1)**; assertion transcribed at §5A.8 |
...
#### 5.4.2 **`[v3]` `ε_Shapley` — cited, not fixed here (A6.1)**

**`ε_Shapley = 1e-9 · max(1, |I^k(∅)|)`**, for each `k ∈ {W2, W5}`.

**This contract does not fix that value.** It is fixed by the written Goal-1
ratification `docs/Missions/JMP_M08_goal1_rulings_document_v1.md` (sha256
`e663802712b2e004711beb327432a43e64501a47d82fcd7d6a2589c3e689ae2d`), entries
**R-60** (the frozen U8 convention and its convention class) and **R-76** (its
attachment to the ruling's §3.5 arithmetic and total-accounting gates as
`ε_Shapley`, anchored on `|I^k(∅)|`). Both entries are quoted verbatim at
**§3.2a(U8)**. *v2's "this contract fixes it" language is **WITHDRAWN** under
Stage-A review finding A6.1.*

It governs gates (2) and (3) **only**. The separate numerical-integration
stability tolerance remains governing for each `I^k(S)`, as the ruling's closing
sentence requires and §3.2a(U8)'s scope limit records.
...
| **V2** | Reference coalition-invariance | Each measure's reference construction (own-set baseline for `W^3`, `Ā`, `J`, `o`) and the `c_ij` matrix are hash-identical across all eight coalitions. **`[v3]` REFERENCE INVENTORY EXPANDED (A5.4)** — the parenthetical is completed so that **both decomposed measures' operative references are named explicitly**: (a) **`W2`'s own-set best-paid reference** — *the reference of the PRIMARY decomposition* (D19, D2), which v2's parenthetical omitted; (b) **`W5`'s reference ability set `Ā` = `type_conditional_median_opportunity`** (U11) — *the reference of the SECONDARY decomposition*; (c) `W3`'s own-set laissez-faire baseline — retained, now as a **validation** object (§5.1); (d) `J` (`W6`) and `o` (`W4`, the `working == 0` lowest-draw predicate, §3.2a(U2)) — welfare-family references; (e) `R_h`, the **`W5` measure-side reference preference** (U13 as corrected at C1); and (f) the `c_ij` matrix. **All six are hash-identical across all eight coalitions.** *(§5A.0's closing note already identified (a) and (b) as "the operative frozen references"; v2's V2 parenthetical did not. It does now.)* | **exact** (hash-identical). **`[v2]` UNCHANGED — confirmed** as the operative form of ruling §3.3's fixed-object list and ruling §3.2's "references frozen across coalitions". **`[v3]`** the tolerance class, the assertion and the failure action are **unchanged**; only the inventory of named references is completed | Gate. Prevents the reference co-moving with the channel and cancelling the effect. |
...
| **V6** | Counterfactual ESS | Re-run the contract §6.1(ii) ESS diagnostic (`ESS_i`, max normalised weight) under **every non-baseline coalition**. The counterfactual changes the IS *target* while the proposal is unchanged, so baseline ESS does not transfer. Baseline singles ESS is already weak (W3 gate report: median 20.3 / 18.8; 1,918/2,243 and 2,493/2,764 below the threshold of 30). | ESS threshold **30** (ratified); **materiality/escalation number PROPOSED-AT-STAGE-A, under the §5A.8.2 EXECUTION BLOCK**. **`[v2]`** this is one of the two surviving Stage-A numbers (§8.4) and is part of the *separate integration machinery* the ruling §3.5 keeps governing over each `I^k(S)` | **`[v3]` REPLACED (A5.3), aligned with V22.** Apply the frozen escalation rule to the counterfactual, not only the baseline. Run the **mandatory** `V_i^dir` redraw cross-check on **each coalition's** flagged subset at the **frozen `0.5`-nat** agreement tolerance (U7 = R-60/R-71/R-78; V22). **Halt** under the ratified rule — flagged-set widening beyond the Stage-A materiality number, or persistent `V_i^dir`/`V_i^IS` disagreement at `0.5` nats (charter §11). *v2's action — "if the flagged set widens materially **and `V_i^dir` remains blocked**, halt" — is **SUPERSEDED and WITHDRAWN**: `V_i^dir` is **UNBLOCKED** and **mandatory** under **R-71** (as R-77 froze the parity axis), so its "remains blocked" precondition can no longer be satisfied and the stale conjunction would have disarmed the halt.* |
| **V7** | No double counting with the wage/occupation technology | (a) the 47 coordinates form an exact partition over preference/ability/access cells — no coordinate in two channels, none omitted; (b) the per-row `log g^W` contribution is **bitwise identical** between baseline and access-equalised runs, confirming `δ_occ`, `μ_i`, `σ` are untouched by the access operator. | (a) exact partition; (b) **bitwise**. **`[v3]` V7(a) CONFIRMED against the corrected register (A5.1):** the exact partition is **§3.1b.3** and the confirmation — no coordinate in two channels, none omitted, and **no coefficient's argument slots split across channels** under the certified spec — is **§3.1b.5**. **V7(b) is unchanged** and remains the operative bitwise non-double-counting check | Gate. Membership is frozen and unchanged after results (charter §9.10). **`[v3]`** the frozen membership is **§3.1b.3** |
...
| **V12** | Block-count reconciliation — **`[v3]` CLOSED (A5.1)** | The W3 gate report records `welfare.blocks` as preference 20 / ability 6 / access 23 = **49 names** against a **47**-coordinate certified vector. Stage A must reconcile the arithmetic against the frozen spec YAML and the accepted θ̂ (handoff §1) and **halt** if the declared membership is not an exact partition. That report was also produced on the pooled couples-inclusive build at working-tree HEAD `7cac2e3`; per charter §2 its membership counts are indicative for P2a singles and are not imported as validated. | exact partition. **`[v3]` DISCHARGED at §3.1b:** the reconciliation was performed against exactly the two named sources (§3.1b.1), resolves exactly — v2's "20" counted three estimation-fixed non-coordinates and omitted `beta_l0_m`, giving **18 / 6 / 23 = 47** (§3.1b.2) — and the declared membership **is** an exact partition (§3.1b.3). The 49-name count is **not imported** | *(halt, per the assertion)* — **`[v3]` the halt DOES NOT FIRE**; V12 is **CLOSED** (§3.1b.6) |
...
| **V20a** | **`[v2]` RETAINED — the mechanical form of gate §5.4(1) operator completeness. `[v3]` MADE EXECUTABLE (A5.2)** | **Over the canonical equalisation objects of §5A.8.1** — the common substituted argument vector, the selected preference block, the frozen occupation marginal, and the factor specifications and coefficients — **EITHER (i) exact hash identity on the canonical serialization, OR (ii) max-abs-difference `≤ 1e-9` on the common evaluation grid.** *(v2's form — *"resolved equalised `u` block, taste covariates, and every `g` factor hash-identical across households"* — is superseded: it attached a tolerance to hash equality and asserted identity of row-evaluated arrays that V4 deliberately keeps household-specific.)* | **Test (i): exact** (no tolerance). **Test (ii): `1e-9`** max-abs-difference. **`[v3]`** the `1e-9` **value is unchanged**; only the object it is attached to is now defined. The economic gate — operator completeness, ruling §3.5(1) — is unchanged | gate (unchanged) |
...
removed — V20b is retired.** **`[v3]` Both remain PROPOSED-AT-STAGE-A under the
explicit execution block of §5A.8.2.**

### 5A.8.1 **`[v3]` V20a made executable — the canonical equalisation objects (A5.2)**

**This subsection is NEW IN v3** and discharges Stage-A review finding A5.2. It
defines *what V20a is evaluated on*. **It changes no tolerance, no gate, and no
operator.** The economic gate is unchanged: gate §5.4(1) operator completeness,
ruling §3.5(1) — *"all assigned access, ability, and preference arguments are
equalised as frozen."*

**Why v2's form was not executable** (review A5.2, adopted):

1. **Hash identity is exact and admits no numerical tolerance.** v2 required
   objects *"hash-identical across households"* while assigning tolerance `1e-9`.
   The two test concepts are incompatible as written.
2. **Row-evaluated `g`-factor arrays need not be identical across households.**
   **V4 deliberately keeps the household-specific alternative support unchanged**
   — that is a ratified invariance, not a defect. Wages, hours, occupations and
   row order differ across households, so `g`-factor arrays evaluated *per row*
   legitimately differ **even when every household-specific argument assigned to
   `A`, `B` and `P` has been equalised**. A literal row-array identity test would
   fail on a correct run.

**The canonical equalisation objects.** V20a is evaluated on these four objects
and on nothing else. Each is a resolved *operand*, not a row-evaluated array:

| # | Canonical object | Content | Fixed by |
|---|---|---|---|
| **CE-1** | **Common substituted argument vector** | the frozen reference values written into every equalised argument slot under `{A,B,P}`: `x̄^acc` (`ḡsur`; the dwt share-weighted region and urbanisation index vectors), `ē`, `p̄ₑₓₚ`, `p̄ₑₓₚ²`, `āge`, `āge²`, `ēduc_leisure`, `n̄kids` | §5A.2 §3.2; §5A.3 §2.2; §5A.6 §3.2; R-71 R4/R5/R6; R-72 S1 |
| **CE-2** | **Selected preference block `θ̄^pref`** | the ratified **singles-female** accepted coefficient block (mirror: singles-male), as a **selection among accepted coordinates** — never a re-fit | §5A.9.1; R-72 S2; C1; V19; V23 |
| **CE-3** | **Frozen occupation marginal** | `p̄̄(loc4)` — the `{A,B}` conditional of the frozen joint `Π` | §5A.4; R-71 R5 |
| **CE-4** | **Factor specifications and coefficients** | the factor forms of `g^E`, `g^H`, `g^Occ`, `g^W`, `u_i` and the accepted `θ̂` coordinates entering them, excluding CE-2's substituted block | certified spec (§3.1b.1); handoff §1 |

**Canonical serialization.** Before either test, CE-1…CE-4 are serialized under a
**canonical ordering**: coordinates in the parameter-name order of the accepted
θ̂ artifacts (§3.1b.1); argument slots in the register order of §3.1b.3;
`loc4` categories in ascending code order; floats in IEEE-754 binary64,
byte-exact, no text formatting. The serialization is recorded with the run.

**The test — choose ONE per object, and never mix the two concepts:**

| Test | Applies to | Criterion |
|---|---|---|
| **(i) Exact hash identity** | the **serialized canonical operands** CE-1…CE-4 — one serialization per household under `{A,B,P}` | the digest is **identical across all households**. **Exact. No tolerance is attached, permitted, or implied.** |
| **(ii) Max-abs-difference `≤ 1e-9`** | the **numerically evaluated** form of CE-1…CE-4, where evaluation is required | evaluated on a **common evaluation grid** — one grid, identical for every household, holding the row-varying attributes (`w_j`, `h_j`, `loc4_j`, `working_j`) fixed across households — and `max_{i,i'} \|value_i − value_{i'}\| ≤ 1e-9` |

**Binding prohibition.** A numerical tolerance is **never** attached to raw hash
equality. Test (i) is exact or it fails; test (ii) is numerical and carries
`1e-9`. A run reports which test it used, per object.

**Explicitly out of scope of V20a**, and unchanged: the household-specific
alternative support, the row count, the per-row `c_ij` matrix, `π`, and the
per-row evaluated `g`-factor arrays. Their invariance is **V2/V3/V4**'s business
(hash-identical **across coalitions**, not across households), and those three
gates are **UNCHANGED**. **V20a tests identity across *households*; V2/V3/V4 test
identity across *coalitions*. The two must not be conflated.**

**`1e-9` is unchanged.** It is v2's V20a tolerance, retained; it is **not**
`ε_Shapley` (§3.2a(U8), §5.4.2), which is a separately-anchored arithmetic
tolerance on the Shapley identities. The three `1e-9`-class objects — the
inversion bracket width (D11), V20a test (ii), and `ε_Shapley`'s coefficient —
remain distinct and non-competing, exactly as §8.2 item 2 records.

### 5A.8.2 **`[v3]` EXPLICIT EXECUTION BLOCK on the V6 / V22 / U6 numbers (A5.3)**

**Stated plainly, as the Stage-A review requires.**

**The V6 and V22 materiality numbers and the U6 draw-growth decision are
PROPOSED-AT-STAGE-A. They are NOT frozen. NO WELFARE EXECUTION MAY BEGIN — no
coalition value, no `I^k(S)`, no Shapley contribution, no `R_bg^k`, no `s_opp`,
no S-10 scenario — UNTIL THE GOAL 1 MANAGER RATIFIES THEM ON THE IMPLEMENTER'S
STAGE-A PROPOSALS.**

| Item | What is unfrozen | What IS frozen and is not reopened |
|---|---|---|
| **V6** | the counterfactual-ESS **materiality and escalation number** | the ESS threshold **30** (U5, ratified); the `0.5`-nat `V_i^dir` tolerance (U7); the redraw cross-check being **mandatory** (R-71) |
| **V22** | the **per-coalition** materiality number | ESS threshold **30**; `0.5` nats; per-coalition application of the cross-check |
| **U6** | the **draw-growth decision** — build the 2×/4× datasets, or carry Gate 1(i) forward BLOCKED under a pre-registered escalation rule | that Gate 1(i) is currently **BLOCKED** on disk (the multiplier datasets do not exist); that 2×/4× dataset creation is authorized as **Stage-D** work (R-70) |

**This is the same block v2 already imposed** at §8.4 condition 3 and §8.0; v3
states it inside the validation section so it cannot be read past, and adds no
new condition. **R-80** confirms the disposition: *"V6/V22/U6 numbers remain
under explicit execution block, ratified on Stage-A proposals pre-Stage-D."*

**What v3 does NOT do:** it does not propose values for V6, V22 or U6, does not
narrow the range from which the Goal 1 Manager may choose, and does not lift the
block. The implementer's Stage-A proposals are the input to that ratification;
this contract records the block, not the number.
...
alongside.** Evidence, in descending authority — **`[v3]` NONNUMERICAL
PROVENANCE ONLY (A6.2)**: this subsection establishes *which weighting
convention is the headline one*, and needs no welfare magnitude to do so. Item 1
below quoted three survey-weighted Gini values and an across-measure bracket
from earlier accepted F5 material; **those figures are DELETED** — they are
substantive welfare magnitudes, they contradict §9.3's *"No computation, no
welfare number"*, and they were never load-bearing for the convention:

1. **Governance memo** —
   `MNL docs/jmp_methodology/JMP_welfare_measurement_decisions_memo_v2.md`:1347:
   the memo's headline inequality line is stated as **survey-weighted** Ginis.
   *(The values themselves are deliberately not reproduced here.)*
...
| **U8** | PROPOSED-PENDING-RATIFICATION | **RESOLVED — `ε_Shapley` = `1e-9·max(1,\|I^k(∅)\|)`, by WRITTEN Goal-1 ratification** ✓ **`[v3]`** cited, not self-adopted (A6.1) | §3.2a(U8); **rulings document R-60 + R-76**; ruling §3.5 |
...
**Ruling-citation note — `[v3]` UPDATED, consequential to C3 (A6.1).** v2
recorded that *"of the Goal-1 rulings R-59…R-72, none exists as a standalone
document"* and that *"**no R-64 document exists**"*, each ruling being known
through citation or recital in a document it authorised (see
`docs/Missions/JMP_M08_goal1_rulings_register_v1.md`). **That is no longer the
case.** `docs/Missions/JMP_M08_goal1_rulings_document_v1.md` (sha256
`e663802712b2e004711beb327432a43e64501a47d82fcd7d6a2589c3e689ae2d`, 8,686 bytes)
is on disk and is the **written primary text for R-59…R-80**. It was issued
2026-08-07 expressly to close that auditability gap; it supplies **the written
R-64** §2.1's carry-forwards rest on, and the **written ratification of
`ε_Shapley`** (R-60 + R-76) that §3.2a(U8) now cites. It is a **Goal-1 Manager
governance record, not deputy text**, and it states its own precedence rule:
*"Where this summary and a binding deputy instrument differ, the deputy
instrument controls."* Stage A verifies its digest at freeze. **The
decomposition-architecture ruling remains different in kind: it is a binding
deputy instrument on disk**, at the path and digest in the front matter, cited
directly rather than through a recital.
...
2. ~~**U8 — Shapley-exhaustiveness epsilon.**~~ **RESOLVED — frozen as
   `ε_Shapley` = `1e-9 · max(1, |I^k(∅)|)` by the written Goal-1 ratification**
   `JMP_M08_goal1_rulings_document_v1.md` **R-60** + **R-76**, quoted verbatim at
   §3.2a(U8). **`[v3]`** v2 recorded this as "adopted" *by this contract*; under
   Stage-A finding A6.1 that self-adoption is **withdrawn** and replaced by the
   citation — the contract cites the ratification, it does not make it. The scope
   limit is unchanged: `ε_Shapley` governs the Shapley arithmetic **only**, and
   the separate numerical-integration stability tolerance continues to govern
   each `I^k(S)`.
   **v1's standing caveat is spent:** v1 noted U8's `1e-9` was "partially
   superseded" by the V20a/V20b split. **`[v4]` CORRECTED (Stage-A correction
   item 2):** V20a — as defined at §5A.8/§5A.8.1 — is exact hash identity on the
   serialized canonical operands, or max-abs-difference `≤ 1e-9` on a common
   evaluation grid, under gate §5.4(1) — never a tolerance on raw hash equality;
   **V20b is retired**; and `ε_Shapley` is a **separate, relativised** arithmetic
   tolerance. The three are now distinct and non-competing objects.
...
2. **Stage-A economics review** — the amended architecture must pass the existing
   ...
   **`[v3]` Status pointer, no change of condition.** That review has run and
   returned **PASS-WITH-REQUIRED-CORRECTIONS** (4 consolidated corrections);
   **R-80** accepted it and authorized the single correction cycle that produced
   **this v3**. The four corrections are applied (front matter; correction map at
   §10.1). What R-80 leaves outstanding on this condition is the **narrow
   four-item confirmation in the same review chat before freeze**. The condition
   itself is **unchanged and not discharged by this document.**
3. **Goal-1 ratification of the V6/V22 and U6 numbers** — the **V6**
   ...
   **`[v3]` This condition is an EXPLICIT EXECUTION BLOCK, stated plainly at
   §5A.8.2 (A5.3):** the three items are **PROPOSED-AT-STAGE-A, not frozen**, and
   **no welfare execution may begin** until the **Goal 1 Manager ratifies them on
   the implementer's Stage-A proposals**. v3 proposes no value for any of the
   three and does not lift the block.
...
*(§9 records the v1 → v2 pass and is retained unchanged. The v2 → v3 record is
§10.)*

---

## 10. **`[v3]` Correction record — v2 → v3 (Stage-A corrections C1–C4)**

Documentation-only. This section records what changed and why. It creates no
obligation that is not already in §§1–8.

### 10.0 The two instruments of this pass

| Instrument | Path | sha256 | Bytes | Role |
|---|---|---|---|---|
| **Stage-A contract review** | `docs/Missions/JMP_M08_stageA_contract_review_v1.md` | `4733876f3a6e8ca499e93c732bea3e847939bf4907c676c974bb184fde838f11` | 15,932 | Independent economics/decomposition review of v2. Verdict **PASS-WITH-REQUIRED-CORRECTIONS**; A1, A3, A4 PASS; A2, A5, A6 FAIL; **4 consolidated corrections**. Retained at this path by **R-80** |
| **Goal-1 consolidated rulings document** | `docs/Missions/JMP_M08_goal1_rulings_document_v1.md` | `e663802712b2e004711beb327432a43e64501a47d82fcd7d6a2589c3e689ae2d` | 8,686 | Written primary text for **R-59…R-80**. Supplies the written **R-64** and the written **ε_Shapley** ratification (**R-60** + **R-76**) that C3 cites |

Both were verified on disk for this pass. Both are **untracked** at this HEAD and
ride the Stage-A freeze commit.

**Authority for this pass:** **R-80** — *"Stage-A review PASS-WITH-4-CORRECTIONS
accepted; review memo retained at
`docs/Missions/JMP_M08_stageA_contract_review_v1.md`; single correction cycle
authorized → contract v3; A6.1 cured by this document (R-60.5 + R-76.2); V6/V22/U6
numbers remain under explicit execution block, ratified on Stage-A proposals
pre-Stage-D; narrow four-item confirmation in the same review chat before freeze."*

### 10.1 Correction map — finding → v3 section → what changed

| Correction | Finding | v3 section(s) | What changed |
|---|---|---|---|
| **C1** | **A2.1** — `R_h` incorrectly identified as the preference-equalisation operand | **D7**; **§3.2 U13 row**; **§3.2a(U13)** *(expanded)* | `R_h` restated as **exclusively the measure-side reference preference for `W5`**. The preference operator's operand is stated as the **frozen `dwt`-weighted taste-covariate references** (step a) **plus the ratified singles-female accepted coefficient block `θ̄^pref`** (step b), with the **singles-male block as the mandatory pre-registered mirror** (V23). D7's *"revalue feasible sets"* replaced by ***"re-evaluate `u_i` on the FIXED alternative support"***, with an explicit statement that the operator alters neither support nor measure references. U13's register title re-worded from *"for preference-equalisation"* to *"the measure-side reference preference for `W5`"*. The memo's *"`θ̄^pref` ≠ `R^h` … must not be identified"* quoted. **§5A.6 / §5A.9.1 are UNCHANGED — they already stated the operand correctly; C1 aligns D7/U13 to them** |
| **C2(a)** | **A5.1** — channel-membership register arithmetically unresolved (20+6+23 = 49 ≠ 47) | **D5** *(replaced)*; **§3.1b** *(new)*; **§5A.8 V7**, **V12** | D5 replaced by the **exact 47-coordinate factor-by-argument-cell partition**, **extracted** from the committed spec `estimation_spec_joint_pooled_v1_bll0_tlmpin.yaml` and the accepted θ̂ artifacts (paths + digests + git blobs at §3.1b.1). **preference 18 / ability 6 / access 23 = 47.** Register distinguishes **35 interior · 2 active-bound (`beta_l_age2_sm`, `beta_l_age2_sf`, upper `1.0`) · 10 pinned (8 couples-only inactive for singles + 2 identically-zero year dummies) · 9 degenerate identity cells · 3 estimation-fixed values outside the vector**. Five D1-declared cells found **coordinate-empty** (§3.1b.4). v2's "MATCH" against the 49-name gate-report line **withdrawn**. **V12 CLOSED** (§3.1b.6); **V7(a) CONFIRMED** (§3.1b.5) |
| **C2(b)** | **A5.2** — V20a not executable (tolerance attached to hash identity; row-array identity contradicts V4) | **§5A.8 V20a** *(row rewritten)*; **§5A.8.1** *(new)* | V20a defined over four **canonical equalisation objects** (CE-1 common substituted argument vector; CE-2 selected preference block; CE-3 frozen occupation marginal; CE-4 factor specs/coefficients) with a **canonical serialization**, and tested by **EITHER exact hash identity on the serialized operands (no tolerance) OR max-abs-difference `≤ 1e-9` on a common evaluation grid**. **Binding prohibition: a tolerance is never attached to raw hash equality.** Row-evaluated `g`-factor arrays and the household-specific support are **excluded** — they are V2/V3/V4's business (across *coalitions*), not V20a's (across *households*). `1e-9` unchanged; gate §5.4(1) unchanged |
| **C2(c)** | **A5.3** — V6 retains the superseded *"`V_i^dir` remains blocked"* failure action; V6/V22/U6 numbers unfrozen | **§5A.8 V6** *(action replaced)*; **§5A.8.2** *(new)*; **§8.4 condition 3** | V6's failure action replaced with the **V22-consistent operative rule**: mandatory `V_i^dir` redraw cross-check on **each coalition's** flagged subset at the frozen **`0.5`-nat** tolerance; halt under the ratified **flagged-set-widening / persistent-disagreement** rule. The stale *"and `V_i^dir` remains blocked"* conjunction is withdrawn (`V_i^dir` is **UNBLOCKED and mandatory** under R-71). **§5A.8.2 states the EXPLICIT EXECUTION BLOCK plainly:** V6/V22 materiality numbers and the U6 draw-growth decision are **PROPOSED-AT-STAGE-A**, and **no welfare execution begins until the Goal 1 Manager ratifies them on the implementer's Stage-A proposals** |
| **C2(d)** | **A5.4** — two mechanical cross-references inconsistent | **D21**; **§5A.8 V2** | D21's *"enumerated verbatim at §5.4"* → **§5.3** (§5.4 carries the five gates; §5.3 carries the residual source list). V2's reference inventory expanded to name explicitly the **`W2` own-set best-paid reference** (primary) and the **`W5` `Ā`** reference (secondary), alongside `W3`'s own-set baseline, `J`, `o`, `R_h`, and `c_ij`. Assertion, tolerance class and failure action unchanged |
| **C3** | **A6.1** — contract self-adopts an unratified Shapley tolerance | **§3.2 U8 row**; **§3.2a(U8)** *(rewritten)*; **§5.4.2** *(new)*; **§8.0** ruling-citation note; **§8.2 item 2** | *"this contract fixes it"* **WITHDRAWN**. `ε_Shapley = 1e-9·max(1,\|I^k(∅)\|)` now rests on the **written Goal-1 ratification** `JMP_M08_goal1_rulings_document_v1.md`, with **R-60** and **R-76 quoted verbatim**; the document's existence, digest, byte count, class and precedence rule are recorded. The **`\|I(Ω)\|` → `\|I^k(∅)\|` re-anchoring is also R-76's**, not this contract's, so v2's supporting reasoning is surplus and marked as such. §8.0's *"no R-64 document exists / none exists as a standalone document"* corrected as a consequence. **Stage-A duty added: if the document is absent or the entries do not read as quoted, U8 returns to PENDING and execution is blocked** |
| **C4** | **A6.2** — substantive welfare magnitudes present despite the no-results discipline | **§6.5.3 item 1** | The three survey-weighted Gini values and the across-measure bracket **DELETED**. Retained: the **nonnumerical provenance chain** — the accepted family report uses `dwt`; the committed config specifies `weight_col: "dwt"`; the committed implementation identifies the primary index as survey-weighted. The conclusion (`dwt` is the headline weighting convention; the §5 halt does not fire) is **unchanged** — the figures were never load-bearing for it |

**Consequential edits, each forced by a correction above and marked `[v3]` in
place:** the title and front matter (supersession note); the revision-lineage
line; §8.0's U8 row and ruling-citation note; §8.2 item 2; §8.4 conditions 2
(status pointer only — the condition is **not** discharged) and 3 (execution-block
pointer). **No other text was touched.**

**Noted and deliberately NOT actioned, because it is outside the four
corrections:** §8.4 **condition 1** (v4-ACCEPT) is left exactly as v2 wrote it,
including its on-disk finding that *"v4 does not exist"*. Goal-1 **R-77** records
a final E2 verification **ACCEPT** with the parity axis **FROZEN** and v4 as the
report of record, and **R-79** records parity closed at MNL `5b0e3d29`. **Stage A
should dispose of condition 1 against R-77/R-79 at freeze.** v3 does not
discharge it and does not restate the §8.4 status string.

### 10.2 **C1 residue scan — every `R_h` / `R^h` site in the contract**

The whole contract was scanned for `R_h`, `R^h`, "reference preference",
"preference-equalisation" and "revalue". **Seven sites. Three were operand
residue and are fixed; four are correct as written and are recorded here so the
scan is complete in both directions.**

| # | Site | Text class | Disposition |
|---|---|---|---|
| 1 | **D7**, Preference clause | operative definition | **RESIDUE — FIXED.** `R^h` removed as the operand; taste-covariate references + singles-female `θ̄^pref` + male mirror substituted; *"revalue feasible sets"* → *"re-evaluate `u_i` on the fixed alternative support"* |
| 2 | **§3.2 register, U13 row** (title and status) | operative register entry | **RESIDUE — FIXED.** Re-titled to *"the measure-side reference preference for `W5`"*; status now states explicitly that the preference-equalisation operand is **not** `R_h` |
| 3 | **§3.2a(U13)**, ratification sentence | operative resolution | **RESIDUE — FIXED.** Ratification restated as `R_h` = `W5` measure-side reference **only**; operand table for steps (a)/(b)/mirror added; the *"`θ̄^pref` ≠ `R^h`"* prohibition quoted |
| 4 | **§3.2a(U13)**, quoted YAML `W5: "R_h" # horizontal reference preference` | **byte-faithful quotation** of the scaffold contract | **CORRECT — RETAINED VERBATIM.** The key on the quoted line is the **measure `W5`**, inside the `reference_preference` block: this line *is* the measure-side reading. A reading note was added beside it; the bytes are unedited |
| 5 | **§3.2a(U14)**, quoted scaffold §7 — pinned params *"swapped for the reference preference"* | **byte-faithful quotation** | **CORRECT — RETAINED VERBATIM.** Describes the `held`/`swapped` switch over **pinned coordinates**, i.e. step (b)'s substitution map; under C1 read "the reference preference" there as **`θ̄^pref`**. Non-binding in M08 in any case (couples-side pins). No edit |
| 6 | **§5A.6 §3.3** (R9 verbatim) — *"swapped for the reference preference … removes those two coordinates from step (b)'s substitution map"* | **verbatim R9 transcription** | **CORRECT — NO CHANGE.** The sentence itself ties "reference preference" to **step (b)'s substitution map**, i.e. `θ̄^pref`. Consistent with C1 |
| 7 | **§5A.6 §3.4** — *"the reference preference **contains a bound-active coefficient**"* | **verbatim R9 transcription** | **CORRECT — NO CHANGE.** Refers to the **selected group block** (`θ̄^pref`), which is why the sentence is about `beta_l_age2_*`. Consistent with C1, and corroborated by §3.1b.3's two active-bound coordinates |

**Also checked and clean:** §5A.9.1 (R-72 S2, the ratified female block + male
mirror) — already correct, **unchanged**; §5A.8 **V18**/**V23** (preference fixed
point and reference-group pre-registration) — both already written against the
**group block**, not `R_h`, **unchanged**; §5A.10 (S-10 invariance) — already
written against `θ̄^pref`, **unchanged**; §9.2's retained-sites index — a line-number
index only, no operand claim.

### 10.3 **C4 residue scan — every substantive welfare magnitude in the contract**

The whole contract was scanned for Gini values, welfare levels, welfare euro
magnitudes, decomposition shares and `Ω`-valued numbers.

| Site | Content | Disposition |
|---|---|---|
| **§6.5.3, evidence item 1** | a quoted line from `JMP_welfare_measurement_decisions_memo_v2.md`:1347 carrying **three survey-weighted Gini values (`W1`, `W4`, `W6`) and the across-measure bracket** | **REMOVED — and deliberately not re-quoted here**, since restating the figures in the correction record would defeat the correction. They were the **only** substantive welfare magnitudes in the contract. Replaced by a nonnumerical statement that the memo's headline inequality line is survey-weighted. The source memo is unchanged and remains citable by path and line |

**Nothing else was removed. Considered and RETAINED, with the reason each is not
a welfare magnitude:**

| Retained | Where | Why it is not a welfare magnitude |
|---|---|---|
| `ils_dispy` / `ils_ben` max abs diff, `ils_origy` `0.00`, 8/100 rows | §1.2 | **Reprice-parity diagnostic** on tax-benefit incomes — the documented failure signature Stage B must reproduce. Not a welfare or inequality quantity |
| `1e-6` euros; `1e-8`/`1e-6`; `1e-9`; `1e-12`; `0.5` nats; ESS `30` | D9–D11, V1/V17/V18, V20a, U5, U7, §5.4.2 | **Frozen tolerances and thresholds** — expressly not welfare magnitudes |
| `≥1%` mean/median change, `≥0.005` absolute Gini change, `≥2pp` share change | D16 | **Pre-registered S-10 trigger thresholds** (charter/S-10 spec). Thresholds, not results |
| `beta_l0_sf` robust CI lower endpoint; the `0.05`-class bound | D17 | **Parameter** inference quantities from the accepted Phase-5 record, not welfare quantities |
| Baseline ESS medians and below-threshold counts | §5A.8 V6 | **Integration diagnostics** (effective sample size), not welfare quantities |
| `drgn1` household counts; 12,445 households; sample sizes | §6.5.2, D11 | **Sample counts.** §6.5.3 requires household counts be reported **unweighted**; counting is not a welfare result |
| The 47 coordinate names, spec slots, bounds, status classes | §3.1b | **Specification and parameter-status metadata** extracted read-only; no estimate value is reproduced in the register |

**§9.3's discipline line therefore now holds without exception:** *no
computation, no welfare number.*

### 10.4 Worktree effect of this pass

| File | Effect |
|---|---|
| `docs/Missions/JMP_M08_singles_welfare_execution_contract_v1.md` | **untouched** — immutable history |
| `docs/Missions/JMP_M08_singles_welfare_execution_contract_v2.md` | **untouched** — byte-identical to its pre-pass state, sha256 `c757fb53d153121f9cec8b66be02a95702530455b8a6df8e90101980382d4907`; retained as immutable history |
| `docs/Missions/JMP_M08_singles_welfare_execution_contract_v3.md` | **new** — this file |
| `docs/Missions/JMP_M08_stageA_contract_review_v1.md` | **present, unmodified** — transferred to this repository under standing rule 8b (canonical placement by file transfer + hash, never retyped) and retained by R-80 |
| `docs/Missions/JMP_M08_goal1_rulings_document_v1.md` | **present, unmodified** — transferred under the same rule; the written ratification C3 cites |
| MNL, `dclaborsupply-monorepo`, and everything else in this repository | **unchanged.** MNL was read **read-only** for the §3.1b extraction: the spec YAML and three θ̂ artifacts were read and digested; **nothing was written** |

**No computation, no welfare number, no decomposition number, no parameter
estimate reproduced, no MNL write, no code change, no data change, no commit.**

### 10.5 What v3 does not change

The deputy's decomposition architecture; the three operators and their operands
as defined at §5A; the eight coalitions and the exact Shapley weights; the five
§5.4 gates and the two §3.4 identities; `R_bg^k` and its prohibitions; the
headline quantities and `s_opp`; the S-10 four scenarios and the 2pp trigger; the
U4 subgroup dimensions, statistics and disclosure rules; the weighting
convention; the disclosure split; every frozen tolerance and threshold; the halt
conditions; and the contract status string at §8.4.

**v3 remains a DRAFT. Nothing here is FROZEN. Stage A owns the freeze, and no
numerical welfare execution is authorized until §8.4's three conditions are met.**

### 10.6 **`[v4]` Residue scan — every co-occurrence of `1e-9` with hash/identity language**

**Stage-A correction item 2 (R-83).** The whole document was scanned for every
co-occurrence of `1e-9` with "hash", "hash-identical", or "identity" language.
**Two sites were live residue and are fixed in this pass (§5.4.1, §8.2 item 2).
Every other site is recorded below with a one-line classification; none is a
further conflicting site.**

| # | Site | Text | Classification |
|---|---|---|---|
| 1 | **§5.4.1**, V20a row, "v2 disposition" cell | *"Its household-hash identity of the resolved equalised objects is exactly the executable test … Tolerance `1e-9` unchanged"* | **RESIDUE — FIXED in this pass.** Replaced with the corrected statement pointing to §5A.8/§5A.8.1 |
| 2 | **§8.2**, item 2 | *"V20a keeps `1e-9` as a hash-identity tolerance under gate §5.4(1)"* | **RESIDUE — FIXED in this pass.** Replaced with the same corrected statement |
| 3 | **§5A.8**, V20a row (the table row itself) | defines V20a as EITHER exact hash identity on the canonical serialization OR max-abs-difference `≤ 1e-9` on the common evaluation grid | **CORRECT AS WRITTEN — NO CHANGE.** This is the operative v3 correction (A5.2) that sites 1 and 2 were required to align with; it never attaches a tolerance to hash equality |
| 4 | **§5A.8.1** (the whole subsection, including its closing paragraph *"`1e-9` is unchanged … exactly as §8.2 item 2 records"*) | defines the four canonical equalisation objects (CE-1…CE-4), the canonical serialization, and the two-test criterion (exact hash identity, or max-abs-difference `≤ 1e-9` on a common evaluation grid) | **CORRECT AS WRITTEN — NO CHANGE.** This is the corrected operative definition itself, the object of alignment for this pass. Its cross-reference to §8.2 item 2 is now accurate as a result of this pass |
| 5 | **§5A.8**, "V20 as a whole (R9 §7, verbatim)" block quote — *"hash-identical across households under `{A,B,P}` (tolerance `1e-9`)"* | v1's original degeneracy-check text, quoted verbatim | **CORRECT AS RETAINED — NO CHANGE.** A byte-faithful historical quotation, immediately followed by a disposition note (already in v2/v3) stating its "degeneracy" framing is superseded and mapping it to gate §5.4(1). Not a live claim |
| 6 | **§9.2**, site 14 (v1→v2 replacement enumeration) — *"analytic degeneracy, `1e-9`" → "RETAINED as the mechanical form of gate §5.4(1); tolerance and failure action unchanged"* | historical v1→v2 transition map entry | **CORRECT — NO CHANGE.** Records that the `1e-9` value is unchanged across the transition; does not itself assert a tolerance on hash equality |
| 7 | **§10.1**, correction map, row **C2(b)** | describes v3's A5.2 fix: "tested by EITHER exact hash identity on the serialized operands (no tolerance) OR max-abs-difference `≤ 1e-9` … a tolerance is never attached to raw hash equality" | **CORRECT — NO CHANGE.** Accurately restates the corrected V20a definition as history of the A5.2 correction |
| 8 | **Front matter**, frozen-value list — *"`1e-9` (V20a, inversion bracket, `ε_Shapley` coefficient)"* | enumerates which frozen values are unchanged | **CORRECT — NO CHANGE.** Lists `1e-9` as a value attached to three distinct objects; asserts no hash-equality/tolerance conflation |
| 9 | **D11** | inversion-convergence bracket width `1e-9` | **NOT APPLICABLE — NO CHANGE.** A solver bracket-width tolerance, not a hash-identity object |
| 10 | Goal-1 rulings **R-60** / **R-76** (quoted verbatim at §3.2a(U8)), and derivative sites §5.4.2, §8.0, §8.2 item 2's `ε_Shapley` clause, §10.3's frozen-tolerances table | *"ε = `1e-9`·max(1,\|I(Ω)\|)"* for *"Shapley numerical-identity checks"* | **NOT APPLICABLE — NO CHANGE.** `ε_Shapley` is a separately-anchored arithmetic tolerance on the Shapley total-accounting identities (gates §5.4(2)–(3)), textually distinct from V20a's hash-identity gate (§5.4(1)) and already held distinct by §5A.8.1's closing paragraph. "Numerical-identity" here is the ratifying document's own term of art for an arithmetic closeness check, not a raw-hash equality test |

**No further conflicting site found.** No STOP is issued.
