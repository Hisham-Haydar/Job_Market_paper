"""Build V9 without writing any V8 path."""
...
redirects = {
    (PAPER / "JMP_working_paper_for_seminar_v6.bib").resolve():
        (PAPER / "JMP_working_paper_for_seminar_v9.bib").resolve(),
    (PAPER / "JMP_working_paper_for_seminar_v6.tex").resolve():
        (PAPER / "JMP_working_paper_for_seminar_v9.tex").resolve(),
    (HERE / "story_v6.generated.md").resolve():
        (HERE / "story_v9.generated.md").resolve(),
    (ROOT / "reports/JMP_research_story_report_v6.html").resolve():
        (ROOT / "reports/JMP_research_story_report_v9.html").resolve(),
    (ROOT / "reports/numbers_of_record_v6.json").resolve():
        (ROOT / "reports/numbers_of_record_v9.json").resolve(),
}
...
def mark_and_move_html_appendix(document: str) -> str:
    ...
    if not match:
        raise RuntimeError("rendered V9 provenance appendix block not found")
    appendix = (
        BEGIN_MARKER
        + '<section id="v9-provenance-appendix" data-audit-exclude="true" '
    ...
    if not appendix_link:
        raise RuntimeError("V9 provenance appendix navigation link not found")
    ...
    if document.count("</main>") != 1:
        raise RuntimeError("cannot place V9 provenance appendix at end of main")
...
def mark_and_move_markdown_appendix(document: str) -> str:
    ...
    match = pattern.search(document)
    if not match:
        raise RuntimeError("editable V9 provenance appendix block not found")
...
def v9_write_text(path, data, *args, **kwargs):
    ...
    if isinstance(data, str):
        ...
        data = data.replace(
            ...
            "and scalar values.",
            "REPORT-V9: full reader-facing economics with certified ex-ante "
            "welfare evidence and a separate technical appendix.",
        )
        data = data.replace(
            "v6; numerical evidence preserved from v5; diagnostic boundary added",
            "v9; full economics restored; certified ex-ante evidence added",
        )
...
sys.modules["v6_sections"] = v9_sections
sys.modules["v6_render_inputs"] = v9_render_inputs
Path.write_text = v9_write_text
try:
...
for old, new in redirects.items():
    if not new.is_file():
        raise SystemExit("missing V9 build target: " + str(new))
    if old.exists() and old.stat().st_mtime_ns > new.stat().st_mtime_ns:
        raise SystemExit("V8/V6 write escaped redirect: " + str(old))

print("V9 HTML, editable Markdown, paper source and numerical registry written.")
