...
{{PENDING-WELFARE}} The decomposition of well-being inequality into job access, earning opportunities, resources and needs awaits re-evaluation on the corrected frame and is not reported here.'''
VERSION = 'Status note, stated once. The corrected positive estimation is complete for singles and couples and is what this document reports. The welfare decomposition is not: re-evaluating it on the corrected frame is held by an unresolved reproducibility control, so no corrected welfare headline appears below. Where a welfare magnitude is unavailable on the corrected frame it is absent, never replaced by a historical value.'
...
The behavioural half of that answer is now available and is reported below: corrected samples, a single converged optimum for each population, fresh clustered inference, and a population-level fit that never uses sampled-menu probabilities. The welfare half is not. This document therefore states the welfare measure exactly, establishes its relationship to the normative principle, and reports the diagnostics that relationship makes computable — but it does not report a corrected percentage of inequality due to opportunities, a causal effect of geography, or a ranking of the opportunity share between singles and couples.
'''),
...
normalizer and household tested. One consequence is worth stating: a coefficient sitting at a bound stays at its transformed bound under the same map, so boundary activity is a property of the estimation box and is settled by re-estimating with wider bounds, not by changing units.
...
The estimated Hessian is **positive definite and full rank** in both populations — {{n:rank_singles}} and {{n:rank_couples}} — with smallest eigenvalues {{n:mineig_singles}} and {{n:mineig_couples}}. The condition numbers in raw coordinates are high, {{n:kappa_singles}} and {{n:kappa_couples}}, which is what one expects when coefficients are carried on very different natural units rather than a sign of weak identification: rank is full, the curvature is strictly positive, and the optimum is unique across starts. {{PENDING-COND}} Standard errors are household-clustered and were computed fresh at this optimum.
...
The two are nevertheless far apart at the estimated scale, because $W^{1\text{-EA}}$ integrates rather than maximises. The non-work atom carries a weighted median of roughly six per cent of the reference integral and never more than ninety per cent for any household; $\Delta_i$ is negative for every household; and $W^{4\text{-EA}}/W^{1\text{-EA}}$ has a median of 34.2 with a minimum of 2.79. The distinction is quantitative and large, and it is carried entirely by the smoothing. We report the coincidence as a property of the measure under freely available non-employment, not as a defect repaired by adding accessibility thresholds or job amenities.
