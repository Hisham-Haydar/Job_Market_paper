"""Build V10 without writing any V9 path.

V10 is the Stage A release-language correction of V9 (see
v10_sections.py). The paper TeX/bib outputs that the underlying v6 engine
...
redirects = {
    (PAPER / "JMP_working_paper_for_seminar_v6.bib").resolve():
        (SCRATCH / "v10_unused_paper.bib").resolve(),
    (PAPER / "JMP_working_paper_for_seminar_v6.tex").resolve():
        (SCRATCH / "v10_unused_paper.tex").resolve(),
    (HERE / "story_v6.generated.md").resolve():
        (HERE / "story_v10.generated.md").resolve(),
    (ROOT / "reports/JMP_research_story_report_v6.html").resolve():
        (ROOT / "reports/JMP_research_story_report_v10.html").resolve(),
    (ROOT / "reports/numbers_of_record_v6.json").resolve():
        (ROOT / "reports/numbers_of_record_v10.json").resolve(),
}

original_write_text = Path.write_text
BEGIN_MARKER = "<!-- V10_PROVENANCE_APPENDIX_BEGIN -->"
END_MARKER = "<!-- V10_PROVENANCE_APPENDIX_END -->"
...
def mark_and_move_html_appendix(document: str) -> str:
    ...
    if not match:
        raise RuntimeError("rendered V10 provenance appendix block not found")
    appendix = (
        BEGIN_MARKER
        + '<section id="v10-provenance-appendix" data-audit-exclude="true" '
    ...
    if not appendix_link:
        raise RuntimeError("V10 provenance appendix navigation link not found")
    ...
    if document.count("</main>") != 1:
        raise RuntimeError("cannot place V10 provenance appendix at end of main")
...
def mark_and_move_markdown_appendix(document: str) -> str:
    ...
    match = pattern.search(document)
    if not match:
        raise RuntimeError("editable V10 provenance appendix block not found")
...
def v10_write_text(path, data, *args, **kwargs):
    ...
    if isinstance(data, str):
        ...
        data = data.replace(
            ...
            "and scalar values.",
            "REPORT-V10: full reader-facing economics with certified ex-ante "
            "welfare evidence; V9 release-language inconsistency corrected in "
            "the technical appendix, no numerical or structural change from V9.",
        )
        data = data.replace(
            "v6; numerical evidence preserved from v5; diagnostic boundary added",
            "v10; identical numerical evidence to v9; ex-ante-metric status "
            "language corrected in the technical appendix",
        )
...
sys.modules["v6_sections"] = v10_sections
sys.modules["v6_render_inputs"] = v10_render_inputs
Path.write_text = v10_write_text
try:
...
for old, new in redirects.items():
    if not new.is_file():
        raise SystemExit("missing V10 build target: " + str(new))
    if old.exists() and old.stat().st_mtime_ns > new.stat().st_mtime_ns:
        raise SystemExit("V9/V6 write escaped redirect: " + str(old))

print("V10 HTML, editable Markdown and numerical registry written "
