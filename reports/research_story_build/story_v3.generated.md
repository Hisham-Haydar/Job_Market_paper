...
Build date: 09 September 2026. Intended first complete circulation: 10 September 2026.
...
# 2. From observed work to the implemented reference
...
![Motivation only: a schematic single-person consumption–leisure ambiguity. Axes have no empirical calibration; menus and preferences are illustrative. The same observed bundle does not identify its explanation. The conceptual distinction is related to Haydar–Maniquet’s companion theory paper; no fixed-leisure equivalent-income measure is computed.](C:/Users/hisham/Repo/Job_Market_paper/manuscript/figures/v3/motivation.png){width=95%}
...
![Implemented monetary reference for the anonymous single-adult and couple examples. Horizontal unit: flat monthly disposable consumption in euros; vertical measure: dimensionless ex-ante value. The reference curve meets attained value at equivalent income. Status: current evaluator at historical estimates and priced inputs; not corrected welfare results. Each curve uses its own baseline opportunity distribution and leisure utility.](C:/Users/hisham/Repo/Job_Market_paper/manuscript/figures/v3/reference_map.png){width=95%}
...
# 3. Related literature and the exact contribution
...
# 4. French data, timing and institutional setting
...
# 5. Sample construction for singles and couples
...
# 6. Descriptive distributions and what they measure
...
# 7. The household model and its maintained assumptions
...
![Indifference curves in consumption and leisure, single-adult households. Population: one representative single-adult household of each sex, the medoid of employed deciders on age, hours and consumption. Units: leisure in hours per week, consumption in euros per month. Status: model-implied preference object at the estimated parameters; the budget set, the opportunity density and the taste shock are not drawn, so a curve is not a set of attainable bundles.](C:/Users/hisham/Repo/Job_Market_paper/manuscript/figures/v3/figP01_indifference_curves_singles.png){width=95%}
...
![Indifference curves in consumption and joint leisure, couple households. Population: one representative couple household, the medoid on the same criteria, with the partner’s leisure held at its observed value. Units: leisure in hours per week per spouse, consumption in euros per month. Status: model-implied preference object at the estimated parameters; conditional slices, not attainable sets.](C:/Users/hisham/Repo/Job_Market_paper/manuscript/figures/v3/figP02_indifference_curves_couples.png){width=95%}
...
![Marginal utility of consumption and of leisure, in physical units. Population: the representative households of each block. Units: marginal utility of consumption per euro per month and of leisure per hour per week, both after the chain-rule conversion from the normalized coordinates. Status: model-implied at the estimated parameters; levels are not comparable across separately estimated utility scales.](C:/Users/hisham/Repo/Job_Market_paper/manuscript/figures/v3/figP03_marginal_utilities.png){width=95%}
...
![Marginal rate of substitution between leisure and consumption, by age and sex. Population: the representative household profile evaluated across the estimated age range, singles and couples. Units: euros per month of consumption per additional recurring weekly hour of leisure. Status: model-implied at the estimated parameters; a compensation along an indifference curve, not a behavioural response to a wage change.](C:/Users/hisham/Repo/Job_Market_paper/manuscript/figures/v3/figP04_mrs_by_age_sex.png){width=95%}
...
![The euro value of one nat of utility under log consumption. Population: the representative households of each block, evaluated across the supported consumption range. Units: euros of monthly disposable consumption per nat of utility; a nat is the natural unit of the unit-scale index. Status: model-implied at the estimated consumption weight. The value is proportional rather than fixed, so it rises with the consumption at which it is evaluated and has no single euro figure independent of a baseline.](C:/Users/hisham/Repo/Job_Market_paper/manuscript/figures/v3/figP05_euro_value_of_one_nat.png){width=95%}
...
![Leisure-normalizer sensitivity: the coefficients move, the indifference curves and the marginal rate of substitution do not. Population: the representative single-adult female household, with the reported deviation maximised over all representative households and every normalizer tested. Units: panel (a) a dimensionless weight on transformed leisure; panel (b) euros per month against hours per week; panel (c) euros per month per extra weekly hour. Status: an exact coordinate change of the estimated model, not a re-estimation; coefficients are coordinates and the curves are the economics.](C:/Users/hisham/Repo/Job_Market_paper/manuscript/figures/v3/figP06_normalization_sensitivity.png){width=95%}
...
# 9. Employment, hours, occupation and wage opportunities
...
# 10. Estimation under the sampling law
...
# 11. Parameter interpretation and three distinct uncertainties
...
# 12. Behavioural estimates and fit for both populations
...
|Moment|Singles observed|Singles model|Couples observed|Couples model|
|---|---|---|---|---|
...
# 13. Population predictions and the matched-household illustration
...
# 14. Welfare: the verified identity and its numerical implementation
...
# 15. Interpersonal comparison, household welfare and equivalization
...
# 16. Weighted inequality and the choice of index
...
# 17. Equalization operators and their pathways
...
|Operator|Replaced / profile|Household-specific remainder|Repricing|Reference map|
|---|---|---|---|---|
...
# 18. Grouped attribution and interactions
...
# 19. Welfare results, benchmarks and limits of the empirical answer
...
|Component|Singles Gini points|Singles share (per cent)|Couples Gini points|Couples share (per cent)|
|---|---|---|---|---|
...
|Population and index|Preferences|Access|Earning opportunities|Resources and needs|Access + earnings|
|---|---|---|---|---|---|
...
**Exhaustiveness is tested, not assumed.** The maximum absolute top-level identity residual is 2.8e-17 Gini units and the nested access/earnings/budget residual 5.6e-17; the fully-common state reaches 1.3e-15. The decomposition therefore closes on the corrected frame, which is a property to check rather than a justification of the grouping.
...
# 20. Research history and implications for interpretation
...
# 21. Reproducibility and what remains open
...
## 1. What does the title ask?

How household-specific job access and earning opportunities change inequality in the declared ex-ante monetary level, separately from preferences, resources and needs.

## 2. Does income alone reveal constraints?

No. Similar earnings and hours can reflect different tastes, opportunities and family resources.

## 3. What does RURO mean?

Random Utility–Random Opportunity: observed job choice reflects both utility and a structural opportunity measure.

## 4. Is joint estimation new here?

No. It is inherited, especially from the applied RURO literature; the contribution is the cross-sectional welfare estimand and attribution.

## 5. Is the first picture the welfare metric?

No. It motivates preference–opportunity ambiguity; the second picture draws the implemented flat-consumption reference.

## 6. Which year is the income reference?

The survey collection and income/policy years are distinct and bound to the source audit in the data section.

## 7. Is priced consumption observed expenditure?

No. It is a simulated disposable-resource output under the stated full-year job and policy assumptions.

## 8. Are couples independent singles?

No. They choose joint jobs under a shared non-linear household budget.

## 9. What does the missing leisure interaction mean?

Its coefficient is maintained at zero, not estimated to demonstrate independent choices.

## 10. Where do children enter?

Female leisure weights in both types, and separate policy/needs pathways; the counts are not interchangeable.

## 11. Is a child coefficient a percentage?

No. It adds to the leisure weight.

## 12. Is a leisure weight willingness to pay?

No. A marginal rate of substitution also uses consumption and leisure levels, curvatures and units.

## 13. Is an hours density height a mass?

No. Its integral over the interval is mass; the narrow full-time band is not an atom.

## 14. Does the access intercept shift every working comparison?

No. It cancels between working jobs and shifts work against non-work.

## 15. What distinguishes g, p and q?

Structural opportunity intensity, utility-weighted choice density and the analyst’s numerical proposal, respectively.

## 16. Are simulated jobs a recovered menu?

No. They are numerical alternatives representing integrals or sampled likelihood calculations.

## 17. Why correct the observed row?

Under the independent conditional sampling law its proposal density appears in the same candidate correction as every other row.

## 18. Are historical wage draws independent?

No. Their scrambled Halton dependence invalidates the simple iid conditional-sampling derivation for that historical frame.

## 19. What does household-robust inference allow?

Dependence within a household’s score, conditional on the model and shared generated inputs, with independent households.

## 20. What does an integration band measure?

Finite numerical integration uncertainty at fixed parameters, not parameter sampling uncertainty.

## 21. Is a reference range a confidence interval?

No. It records normative-reference sensitivity.

## 22. What exactly is equivalent income?

A flat monthly disposable-consumption amount at every alternative in the stated reference opportunity distribution.

## 23. What did the identity check establish?

The factorized closed form reproduces the executed inversion at historical parameters for both types; it did not re-estimate the model.

## 24. What is special about log consumption?

Equivalent income becomes a consumption average weighted by non-consumption utility and opportunities, not by choice probabilities.

## 25. Must wider opportunities raise W?

No. They change attained value and the reference map; added low-consumption weight can lower the implemented average.

## 26. Do opportunities matter for actual non-workers?

Yes. Their ex-ante evaluation includes working and non-working alternatives.

## 27. Why adjust for household needs?

A household amount is not automatically comparable across rosters; the declared scale changes with composition when that pathway is equalized.

## 28. Is the broad environment share the opportunity share?

No. The latter is access plus wage opportunities; the former also contains resources and needs.

## 29. Is Shapley the one-factor reduction?

No. It averages marginal equalization effects across the permitted orders, allocating interactions.

## 30. Must shares sum to one?

Only if the grand-coalition residual is zero; otherwise they sum to the reducible difference divided by baseline inequality.

## 31. Why not report old nested percentages?

Their resource/needs classification and pricing pathways are stale, so corrected attribution is not established.

## 32. What does the common-opportunity benchmark identify?

A structural model comparison after re-estimation, not a causal effect or a licensed nested likelihood-ratio test.

## 33. Do public wage tables identify preferences?

No. They benchmark distributions and support choices under different coverage.

## 34. Does singles device parity establish couples support?

No. Each specification and backend must be evaluated on its own supported path.
