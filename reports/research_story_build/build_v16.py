"""Build V16 without writing any earlier report path.

V16 is V15 with four corrected passages (see v16_sections.py): the opportunity-density
equation, two likelihood floor descriptions, and the final-route sentence. The appendix
markers keep their V15 names so that the unchanged V15 gallery pairs with this report
in the release gates. The paper TeX/bib outputs that the underlying v6 engine
...
SCRATCH = Path(
    ...
    r"\bba8c789-42df-4e9e-b873-394adc28d079\scratchpad"
)
SCRATCH.mkdir(parents=True, exist_ok=True)

redirects = {
    (PAPER / "JMP_working_paper_for_seminar_v6.bib").resolve():
        (SCRATCH / "v16_unused_paper.bib").resolve(),
    (PAPER / "JMP_working_paper_for_seminar_v6.tex").resolve():
        (SCRATCH / "v16_unused_paper.tex").resolve(),
    (HERE / "story_v6.generated.md").resolve():
        (HERE / "story_v16.generated.md").resolve(),
    (ROOT / "reports/JMP_research_story_report_v6.html").resolve():
        (ROOT / "reports/JMP_research_story_report_v16.html").resolve(),
    (ROOT / "reports/numbers_of_record_v6.json").resolve():
        (ROOT / "reports/numbers_of_record_v16.json").resolve(),
}
...
def v15_write_text(path, data, *args, **kwargs):
    ...
    if isinstance(data, str):
        ...
        data = data.replace(
            ...
            "and scalar values.",
            "REPORT-V16: opportunity-density equation corrected (access on market jobs only, "
            "non-employment normalised); likelihood floor descriptions and the final-route "
            "sentence corrected; numbers unchanged.",
        )
        data = data.replace(
            "v6; numerical evidence preserved from v5; diagnostic boundary added",
            "v16; numerical evidence identical to v15",
        )
...
sys.modules["v6_sections"] = v16_sections
