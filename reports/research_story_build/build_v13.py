...
V13 is the figures release (see v13_sections.py and make_v13_figures.py). The paper TeX/bib outputs that the underlying v6 engine
...
redirects = {
    (PAPER / "JMP_working_paper_for_seminar_v6.bib").resolve():
        (SCRATCH / "v13_unused_paper.bib").resolve(),
    (PAPER / "JMP_working_paper_for_seminar_v6.tex").resolve():
        (SCRATCH / "v13_unused_paper.tex").resolve(),
    (HERE / "story_v6.generated.md").resolve():
        (HERE / "story_v13.generated.md").resolve(),
    (ROOT / "reports/JMP_research_story_report_v6.html").resolve():
        (ROOT / "reports/JMP_research_story_report_v13.html").resolve(),
    (ROOT / "reports/numbers_of_record_v6.json").resolve():
        (ROOT / "reports/numbers_of_record_v13.json").resolve(),
}

original_write_text = Path.write_text
BEGIN_MARKER = "<!-- V13_PROVENANCE_APPENDIX_BEGIN -->"
END_MARKER = "<!-- V13_PROVENANCE_APPENDIX_END -->"
...
def mark_and_move_html_appendix(document: str) -> str:
    ...
    if not match:
        raise RuntimeError("rendered V13 provenance appendix block not found")
    appendix = (
        BEGIN_MARKER
        + '<section id="v13-provenance-appendix" data-audit-exclude="true" '
    ...
    if not appendix_link:
        raise RuntimeError("V13 provenance appendix navigation link not found")
    ...
    if document.count("</main>") != 1:
        raise RuntimeError("cannot place V13 provenance appendix at end of main")
...
def mark_and_move_markdown_appendix(document: str) -> str:
    ...
    match = pattern.search(document)
    if not match:
        raise RuntimeError("editable V13 provenance appendix block not found")
...
def v13_write_text(path, data, *args, **kwargs):
    ...
    if isinstance(data, str):
        ...
        data = data.replace(
            ...
            "and scalar values.",
            "REPORT-V13: decomposition architecture, Shapley and central-result figures regenerated "
            "from certified records; numbers unchanged.",
        )
        data = data.replace(
            "v6; numerical evidence preserved from v5; diagnostic boundary added",
            "v13; numerical evidence identical to v12 plus source-bound figure values",
        )
...
sys.modules["v6_sections"] = v13_sections
sys.modules["v6_render_inputs"] = v13_render_inputs
Path.write_text = v13_write_text
try:
...
for old, new in redirects.items():
    if not new.is_file():
        raise SystemExit("missing V13 build target: " + str(new))
    if old.exists() and old.stat().st_mtime_ns > new.stat().st_mtime_ns:
        raise SystemExit("V12/V6 write escaped redirect: " + str(old))

print("V13 HTML, editable Markdown and numerical registry written "
