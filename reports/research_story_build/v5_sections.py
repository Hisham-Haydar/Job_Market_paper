...
Dispersion in consumption is quantitatively large relative to dispersion in log well-being: $\operatorname{Var}(\log C)$ is roughly {{n:d2_varshare_logC_couples|.0f}}–{{n:d2_varshare_logC_singles|.0f}}% of $\operatorname{Var}(\log W)$, with the excess offset by a large negative covariance between consumption and the leisure valuation. This bounded decomposition exercise holds household resources, needs and composition fixed, leaving important sources of dispersion outside the P/A/B allocation.
...
QA = [
 ...
 ('What is the first figure showing?',
  r'''The deterministic W1 construction adapted from the companion theory paper, Haydar and Maniquet (2026). See the figure Own-set equal-consumption equivalents. Section 4 states the empirical home-reference implementation on the current estimated model and derives its closed form.'''),
 ('Does log consumption make welfare an arithmetic mean?',
  r'''The measure is not an arithmetic mean of anything: $W^1_{i,F}=C_i^{\mathrm{obs}}\exp[(L_i(j_i^{\mathrm{obs}})-L_i(o))/\beta_c]$ evaluates a single observed job against a single home state, with no averaging or integration over jobs. $\beta_c$ sets the rate at which the non-consumption gap between those two states converts into a proportional adjustment of the household's own observed consumption. The estimated values are {{n:beta_c_singles|.4f}} and {{n:beta_c_couples|.4f}}. See Section 4.'''),
 ('Does the normalizer $\\lambda_c$ affect welfare?',
  r'''No. It cancels under exact log consumption from every choice probability, but it does not appear in the $W^1_F$ closed form at all -- not as a cancelling term, simply absent. See Section 4.'''),
 ...
 ('Is the measure independent of pay?',
  r'''The money metric is derived from the own-set equal-consumption principle. Under the current empirical specification its direct reference collapses to the universally available non-employment state; opportunity heterogeneity therefore affects the current welfare measure through attained bundles. The home state carries no opportunity-density, proposal, shock or intensity argument. See Section 4, What the reference does and does not do to pay.'''),
 ...
 ('Is the movable share $\\Delta I$ small because job opportunities do not matter?',
  r'''No. The decomposition is bounded by design because household resources, needs and composition are held fixed. Within that bounded game, equalising P/A/B changes the Gini by {{n:d2_deltaI_pct_singles_eq|.1f}}–{{n:d2_deltaI_pct_couples_uneq|.1f}}% of its baseline level, depending on sample and reporting scale. Dispersion in consumption is quantitatively large relative to dispersion in log well-being: $\operatorname{Var}(\log C)$ is roughly {{n:d2_varshare_logC_couples|.0f}}–{{n:d2_varshare_logC_singles|.0f}}% of $\operatorname{Var}(\log W)$, with the excess offset by a large negative covariance between consumption and the leisure valuation. This bounded decomposition exercise holds household resources, needs and composition fixed, leaving important sources of dispersion outside the P/A/B allocation. See Section 5.'''),
 ('Which ordering is robust across unequivalised and equivalised reporting?',
  r'''Within this bounded decomposition exercise, earning-opportunity heterogeneity has a larger Shapley contribution than the model's coarse geographic/temporal access channel. The preference contribution is not sign-robust to equivalisation. See Section 5.'''),
