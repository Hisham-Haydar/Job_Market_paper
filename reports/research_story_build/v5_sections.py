...
Neither construction's reference depends on pay. The accepted literal construction's home reference (Mapping F, Deputy ruling R1) is a fixed singleton state whose systematic utility carries no opportunity-density, proposal, behavioural-shock or intensity argument, so none of those four objects enters it at all. {{report-only}}The ex-ante estimation-based reference above has the same property for a different reason: the wage density integrates to one on its support and the non-consumption index has no wage argument, so replacing it leaves the reference mass unchanged exactly, by construction -- not as a numerical approximation.{{/report-only}}
...
The ex-ante estimation-based measure of the previous section is less unequal than the priced disposable income it replaces, for both household types: the Gini falls from {{n:inc_gini_singles|.3f}} to {{n:wl_gini_raw_singles|.3f}} for single adults and from {{n:inc_gini_couples|.3f}} to {{n:wl_gini_raw_couples|.3f}} for couples. This is a different object from the verified $W^1_F$ levels above -- an ex-ante monetary level built from each household's own preferences and own reachable jobs, rather than a closed-form evaluation at the observed bundle -- and the two are not two estimates of one quantity.
...
SECTIONS = [
    ...
    {'key': 'appbridge',
     'title': 'Supplementary appendix. The bridge between the two monetary '
              'references',
     'body': APP_BRIDGE, 'appendix': True, 'paper': False},
