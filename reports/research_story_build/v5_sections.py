...
with four blocks. **Employment access** $\beta_E+x_i^{E\prime}\gamma$ carries the group unemployment measure, region and urbanisation, and multiplies the working indicator. **Occupation access** is $\xi_{k,g}$ with group one the omitted reference. The **hours density** $H(h)$ elevates five bands over a residual reference set of total width {{n:hours_reference_width}} hours per week; the narrow full-time band $[33.5,36.5)$ is a density elevation over an interval, not an atom at thirty-five hours. The **wage density** $\phi$ is log-normal with location $\mu_i(k)=\beta_{w0}+\beta_{wL}\mathrm{educL}_i+\beta_{wH}\mathrm{educH}_i+\beta_{wx}x_i+\beta_{wx^2}x_i^{2}+\delta_k$ and dispersion $\sigma$, truncated to $[{{n:wage_lo}},{{n:wage_hi}}]$ euros per hour and renormalized on that support.
...
The choice set is a continuum, so the likelihood is evaluated over sampled alternatives. For each household we draw {{n:n_draws}} alternatives from a proposal density $q_i$ and place the observed choice in the set as well, giving {{n:n_alt_rows}} rows. The contribution of household $i$ is the conditional probability of its observed package within that set, corrected for the proposal:
...
QA = [
 ...
 ('Can the notebook reproduce everything?',
  r'''From the frozen priced frame onwards, yes: refit, inference, population prediction, welfare, coalition states and figures. It cannot reconstruct the raw survey input, generate new alternatives or price them; those three stages run as separate authorised jobs against the microdata environment.'''),
]
