...
V14 restores the theory figure (see v14_sections.py and make_v14_theory_figure.py). The paper TeX/bib outputs that the underlying v6 engine
...
redirects = {
    (PAPER / "JMP_working_paper_for_seminar_v6.bib").resolve():
        (SCRATCH / "v14_unused_paper.bib").resolve(),
    (PAPER / "JMP_working_paper_for_seminar_v6.tex").resolve():
        (SCRATCH / "v14_unused_paper.tex").resolve(),
    (HERE / "story_v6.generated.md").resolve():
        (HERE / "story_v14.generated.md").resolve(),
    (ROOT / "reports/JMP_research_story_report_v6.html").resolve():
        (ROOT / "reports/JMP_research_story_report_v14.html").resolve(),
    (ROOT / "reports/numbers_of_record_v6.json").resolve():
        (ROOT / "reports/numbers_of_record_v14.json").resolve(),
}

original_write_text = Path.write_text
BEGIN_MARKER = "<!-- V14_PROVENANCE_APPENDIX_BEGIN -->"
END_MARKER = "<!-- V14_PROVENANCE_APPENDIX_END -->"
...
def mark_and_move_html_appendix(document: str) -> str:
    ...
    if not match:
        raise RuntimeError("rendered V14 provenance appendix block not found")
    appendix = (
        BEGIN_MARKER
        + '<section id="v14-provenance-appendix" data-audit-exclude="true" '
    ...
    if not appendix_link:
        raise RuntimeError("V14 provenance appendix navigation link not found")
    ...
    if document.count("</main>") != 1:
        raise RuntimeError("cannot place V14 provenance appendix at end of main")
...
def mark_and_move_markdown_appendix(document: str) -> str:
    ...
    match = pattern.search(document)
    if not match:
        raise RuntimeError("editable V14 provenance appendix block not found")
...
def v14_write_text(path, data, *args, **kwargs):
    ...
    if isinstance(data, str):
        ...
        data = data.replace(
            ...
            "and scalar values.",
            "REPORT-V14: deterministic own-set equal-consumption theory figure restored in Section 3; "
            "numbers unchanged.",
        )
        data = data.replace(
            "v6; numerical evidence preserved from v5; diagnostic boundary added",
            "v14; numerical evidence identical to v13",
        )
...
sys.modules["v6_sections"] = v14_sections
sys.modules["v6_render_inputs"] = v14_render_inputs
Path.write_text = v14_write_text
try:
...
for old, new in redirects.items():
    if not new.is_file():
        raise SystemExit("missing V14 build target: " + str(new))
    if old.exists() and old.stat().st_mtime_ns > new.stat().st_mtime_ns:
        raise SystemExit("V13/V6 write escaped redirect: " + str(old))

print("V14 HTML, editable Markdown and numerical registry written "
