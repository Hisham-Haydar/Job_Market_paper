"""Build V11 without writing any V10 or V9 path.

V11 is the Stage B economic-derivation release (see v11_sections.py). The paper TeX/bib outputs that the underlying v6 engine
...
redirects = {
    (PAPER / "JMP_working_paper_for_seminar_v6.bib").resolve():
        (SCRATCH / "v11_unused_paper.bib").resolve(),
    (PAPER / "JMP_working_paper_for_seminar_v6.tex").resolve():
        (SCRATCH / "v11_unused_paper.tex").resolve(),
    (HERE / "story_v6.generated.md").resolve():
        (HERE / "story_v11.generated.md").resolve(),
    (ROOT / "reports/JMP_research_story_report_v6.html").resolve():
        (ROOT / "reports/JMP_research_story_report_v11.html").resolve(),
    (ROOT / "reports/numbers_of_record_v6.json").resolve():
        (ROOT / "reports/numbers_of_record_v11.json").resolve(),
}

original_write_text = Path.write_text
BEGIN_MARKER = "<!-- V11_PROVENANCE_APPENDIX_BEGIN -->"
END_MARKER = "<!-- V11_PROVENANCE_APPENDIX_END -->"
...
def mark_and_move_html_appendix(document: str) -> str:
    ...
    if not match:
        raise RuntimeError("rendered V11 provenance appendix block not found")
    appendix = (
        BEGIN_MARKER
        + '<section id="v11-provenance-appendix" data-audit-exclude="true" '
    ...
    if not appendix_link:
        raise RuntimeError("V11 provenance appendix navigation link not found")
    ...
    if document.count("</main>") != 1:
        raise RuntimeError("cannot place V11 provenance appendix at end of main")
...
def mark_and_move_markdown_appendix(document: str) -> str:
    ...
    match = pattern.search(document)
    if not match:
        raise RuntimeError("editable V11 provenance appendix block not found")
...
def v11_write_text(path, data, *args, **kwargs):
    ...
    if isinstance(data, str):
        ...
        data = data.replace(
            ...
            "and scalar values.",
            "REPORT-V11: economic derivation restored in the body; welfare as two "
            "perspectives; decomposition holds resources, needs and composition "
            "fixed; implementation moved to the appendix; numbers unchanged.",
        )
        data = data.replace(
            "v6; numerical evidence preserved from v5; diagnostic boundary added",
            "v11; numerical evidence identical to v10 plus source-bound comparison "
            "and illustration keys; economic derivation restored",
        )
...
sys.modules["v6_sections"] = v11_sections
sys.modules["v6_render_inputs"] = v11_render_inputs
Path.write_text = v11_write_text
try:
...
for old, new in redirects.items():
    if not new.is_file():
        raise SystemExit("missing V11 build target: " + str(new))
    if old.exists() and old.stat().st_mtime_ns > new.stat().st_mtime_ns:
        raise SystemExit("V10/V6 write escaped redirect: " + str(old))

print("V11 HTML, editable Markdown and numerical registry written "
