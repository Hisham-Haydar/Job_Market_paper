...
import v15_sections as sections  # noqa: E402
...
REGISTRY = ROOT / "reports/numbers_of_record_v15.json"
REGISTRY_V10 = ROOT / "reports/numbers_of_record_v14.json"
OUT_JSON = ROOT / "reports/v15_number_to_source_results.json"
OUT_MD = ROOT / "reports/v15_number_to_source_results.md"
...
def main() -> int:
    ...
    claim("every V14 registry value unchanged in V15", not changed,
          "numbers_of_record_v14.json vs v15 (" + (", ".join(changed) or "no differences") + ")")
    ...
    claim("every scalar placeholder resolves", not missing, "V15 registry")
    ...
    theory = ROOT / "manuscript/figures/v5/theory_w1.png"
    theory_v3 = ROOT / "manuscript/figures/v3/theory_w1.png"
    caption_digits = sorted(set(re.findall(r"\d+", inputs.THEORY_CAPTION)))
    claim("theory figure carries no estimated or certified number",
          caption_digits == ["1", "2026", "3"] and not any(k.startswith("fig15") for k in registry),
          "caption digits " + ", ".join(caption_digits) + "; W^1 superscript, publication year, section number only")
    claim("theory figure restored: byte-identical to the V4-V11 stored image, not the V14 regeneration",
          sha(theory) == sha(theory_v3)
          == "976fe2787bf9bb269acfec07e4cf782f69486e665059ad7e3092afead1064f02"
          and sha(theory) != sha(ROOT / "manuscript/figures/v14/fig_v14_theory_own_set.png"),
    ...
    v15_theory = _b64.b64encode(theory.read_bytes()).decode()
    v14_theory_old = _b64.b64encode(
        (ROOT / "manuscript/figures/v14/fig_v14_theory_own_set.png").read_bytes()).decode()
    for surface in ("research_story_report", "results_gallery"):
        old = sorted(x for x in embedded(ROOT / f"reports/JMP_{surface}_v14.html") if x != v14_theory_old)
        new = embedded(ROOT / f"reports/JMP_{surface}_v15.html")
        others = sorted(x for x in new if x != v15_theory)
        claim(f"{surface}: every other figure byte-identical to V14 (theory figure swapped)",
              others == old and new.count(v15_theory) == 1 and v14_theory_old not in new,
              f"{len(old)} V14 non-theory images, {len(new)} V15 images")
    ...
    sources = {name: {"path": str(path.relative_to(WORKSPACE)).replace("\\", "/"), "sha256": sha(path)}
               for name, path in (("certified ex-ante result record", WEA), ("certified result memo", MEMO),
                                  ("matched-household record", MATCHED), ("V14 predecessor registry", REGISTRY_V10))}
    result = {"gate": "V15 number-to-source", "status": "PASS" if not failures else "FAIL",
              "placeholder_count": len(placeholders), "claims": claims, "sources": sources, "failures": failures}
    OUT_JSON.write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
    lines = ["# V15 number-to-source results", "", f"Overall: **{result['status']}**", "",
             "| Check | Source | Status |", "|---|---|---:|"]
    ...
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
    print(f"V15 NUMBER-TO-SOURCE {result['status']}: {len(claims)} checks, {len(failures)} failures")
    for x in failures:
        print("  FAIL:", x)
