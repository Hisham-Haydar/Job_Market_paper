"""V15 gate: no section title appears in both the body and the appendix.
...
OUT_JSON = ROOT / "reports/v15_section_title_results.json"
BEGIN = "<!-- V15_PROVENANCE_APPENDIX_BEGIN -->"
END = "<!-- V15_PROVENANCE_APPENDIX_END -->"
SURFACES = {
    "report": (ROOT / "reports/JMP_research_story_report_v15.html", r"h[1-3]"),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="jmp-v15-title-control-") as tmp:
    ...
    payload = {"gate": "V15 body/appendix section-title uniqueness", "status": status,
               "surfaces": results,
    ...
    print(f"V15 SECTION TITLES {status}: report shared {results['report']['shared']}, "
