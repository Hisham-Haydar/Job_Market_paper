...
import v16_sections as v11  # noqa: E402


REPORT = ROOT / "reports/JMP_research_story_report_v16.html"
GALLERY = ROOT / "reports/JMP_results_gallery_v15.html"
SOURCE = BUILD / "story_v16.generated.md"
OUT_JSON = ROOT / "reports/v16_reader_gate_results.json"
OUT_MD = ROOT / "reports/v16_reader_gate_results.md"
...
def main() -> int:
    ...
    equations = {
        ...
        "C opportunity factorisation": (r"g_i(j)=\left(g^{E}_i\cdot", r"g_i(o)=1"),
    ...
    check("inherited predecessor document removed from appendix",
          ...
          and "Scientific history" not in source, "generated V16 Markdown")
    ...
    check("V9 to V15 untouched", diff.returncode == 0, "git diff over V9-V11 report, gallery and source")
    check("no unresolved source tokens", "{{" not in source, "generated V16 Markdown")

    result = {"gate": "V16 reader-facing structure, figures and economic derivation", "status": "PASS" if not failures else "FAIL",
              "main_words": len(re.findall(r"\b[\w’'-]+\b", report_text)), "checks": checks, "failures": failures}
    OUT_JSON.write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
    lines = ["# V16 reader, structure, figures and economics gate results", "", f"Overall: **{result['status']}**", "",
             "| Check | Evidence | Status |", "|---|---|---:|"]
    lines.extend(f"| {r['check']} | {r['evidence'].replace('|', '/')} | **{r['status']}** |" for r in checks)
    if failures:
        lines.extend(["", "## Failures", ""] + [f"- {item}" for item in failures])
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
    print(f"V16 READER GATES {result['status']}: {len(checks)} checks, {len(failures)} failures")
