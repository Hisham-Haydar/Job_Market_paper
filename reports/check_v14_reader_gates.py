...
import v14_sections as v11  # noqa: E402


REPORT = ROOT / "reports/JMP_research_story_report_v14.html"
GALLERY = ROOT / "reports/JMP_results_gallery_v14.html"
SOURCE = BUILD / "story_v14.generated.md"
OUT_JSON = ROOT / "reports/v14_reader_gate_results.json"
OUT_MD = ROOT / "reports/v14_reader_gate_results.md"
...
def main() -> int:
    ...
    check("inherited predecessor document removed from appendix",
          ...
          and "Scientific history" not in source, "generated V14 Markdown")
    ...
    import v14_render_inputs as theory_inputs
    theory_caption = " ".join(theory_inputs.THEORY_CAPTION.split())
    check("theory figure caption present once in report and gallery",
          flat(report_text).count(theory_caption) == 1 and flat(gallery_text).count(theory_caption) == 1,
          "rendered captions")
    # Scoped exception: the author and construction names may appear only in the theory caption.
    report_text_scoped = flat(report_text).replace(theory_caption, " ")
    gallery_text_scoped = flat(gallery_text).replace(theory_caption, " ")
    check("theory figure at the start of Section 3, after the framing and before Perspective 1",
          'data-fig="fig_v14_theory_own_set"' in main_body
          and main_body.index('id="3-from-choices') < main_body.index('data-fig="fig_v14_theory_own_set"')
    ...
    check("V9 to V13 untouched", diff.returncode == 0, "git diff over V9-V11 report, gallery and source")
    check("no unresolved source tokens", "{{" not in source, "generated V14 Markdown")

    result = {"gate": "V14 reader-facing structure, figures and economic derivation", "status": "PASS" if not failures else "FAIL",
              "main_words": len(re.findall(r"\b[\w’'-]+\b", report_text)), "checks": checks, "failures": failures}
    OUT_JSON.write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
    lines = ["# V14 reader, structure, figures and economics gate results", "", f"Overall: **{result['status']}**", "",
             "| Check | Evidence | Status |", "|---|---|---:|"]
    lines.extend(f"| {r['check']} | {r['evidence'].replace('|', '/')} | **{r['status']}** |" for r in checks)
    if failures:
        lines.extend(["", "## Failures", ""] + [f"- {item}" for item in failures])
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
    print(f"V14 READER GATES {result['status']}: {len(checks)} checks, {len(failures)} failures")
