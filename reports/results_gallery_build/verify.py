...
NOR = ROOT / "reports/numbers_of_record_v7.json"
...
welfare_reference = (
    ...
    "metric through attained outcomes rather than through a direct opportunity-prospect term."
)
variance_language = (
...
checks["corrected_fit_ruling"] = all(x in plain for x in (
    "underprediction of the observed 37-hour mass point",
    "Singles MAE rises slightly",
    "earlier three-group excess-predictability claim is withdrawn",
    "withheld for both groups of men",
))
checks["decomp_caveat_exact"] = (
...
checks["short_hours_exact"] = (
    "The preliminary attainment exercise is evaluated on the already-priced estimation panel. "
    "That panel sparsely represents the lower part of the structural hours support. "
    "A separate integration audit shows substantial structural mass in that region. "
    "The direct importance of this limitation for the current attained-bundle decomposition "
    "has not been fully quantified, so the result remains explicitly preliminary."
) in plain
checks["wea_definition_only"] = all(x in plain for x in (
    "W_EA_flat is definition-only and numerically blocked",
    "No numerical result is reported",
))
