...
import check_v15_rendered_language as audit  # noqa: E402
import v15_sections as v11  # noqa: E402


REPORT = ROOT / "reports/JMP_research_story_report_v15.html"
GALLERY = ROOT / "reports/JMP_results_gallery_v15.html"
SOURCE = BUILD / "story_v15.generated.md"
OUT_JSON = ROOT / "reports/v15_reader_gate_results.json"
OUT_MD = ROOT / "reports/v15_reader_gate_results.md"
...
def main() -> int:
    ...
    check("inherited predecessor document removed from appendix",
          ...
          and "Scientific history" not in source, "generated V15 Markdown")
    ...
    import v15_render_inputs as theory_inputs
    # The report caption is LaTeX ($R_i$, $A=\{j,k\}$ ...), rendered statically as
    # \(R_i\) etc (MathJax has not executed in the saved HTML); the gallery caption
    # is deliberately plain text. Neither form can be matched verbatim against the
    # LaTeX *source* string, so both surfaces are scoped from a stable anchor phrase
    # that is identical in every form (source, report-rendered, gallery-rendered).
    theory_anchor = "Own-set equal-consumption equivalents: the theoretical construction from the companion theory paper."
    theory_window = 900

    def theory_block(text: str) -> tuple[str, str]:
        i = text.find(theory_anchor)
        if i < 0:
            return text, ""
        block = text[i:i + theory_window]
        end = block.find("Section 3.")
        if end >= 0:
            block = block[:end + len("Section 3.")]
        return text[:i] + " " + text[i + len(block):], block

    report_text_scoped, report_theory_block = theory_block(flat(report_text))
    gallery_text_scoped, gallery_theory_block = theory_block(flat(gallery_text))
    check("theory figure caption anchor present once in report and gallery",
          flat(report_text).count(theory_anchor) == 1 and flat(gallery_text).count(theory_anchor) == 1,
          "rendered captions")
    check("theory caption content matches on both surfaces (LaTeX vs plain-text rendering aside)",
          all(t in report_theory_block for t in ("R_i", "R_h", "z_i", "z_h", "W^1_i", "W^1_h",
                                                  "directly", "comparable", "Haydar and Maniquet"))
          and all(t in gallery_theory_block for t in ("R_i", "R_h", "z_i", "z_h", "W1_i", "W1_h",
                                                       "directly", "comparable", "Haydar and Maniquet")),
          "theory caption block, both surfaces")
    check("theory figure at the start of Section 3, after the framing and before Perspective 1",
          'data-fig="theory_w1"' in main_body
          and main_body.index('id="3-from-choices') < main_body.index('data-fig="theory_w1"')
          < main_body.index('id="perspective-1-att'), "section 3 order")
    check("restored image matches the V4-V11 stored copy (not the V14 regeneration)",
          hashlib.sha256((ROOT / "manuscript/figures/v5/theory_w1.png").read_bytes()).hexdigest()
          == hashlib.sha256((ROOT / "manuscript/figures/v3/theory_w1.png").read_bytes()).hexdigest()
          == "976fe2787bf9bb269acfec07e4cf782f69486e665059ad7e3092afead1064f02",
          "manuscript/figures/{v3,v5}/theory_w1.png")
    check("caption no longer claims the two individuals share preferences",
          not re.search(r"share the same\s+preferences", report_text + gallery_text, flags=re.I)
          and "each with their own preferences" in report_text, "section 3, both surfaces")
    check("label-overlap defect disclosed, not silently redrawn (release memo)",
          (ROOT / "reports/JMP_v14_to_v15_theory_figure_restore_v1.md").is_file()
          and "overlap" in (ROOT / "reports/JMP_v14_to_v15_theory_figure_restore_v1.md").read_text(encoding="utf-8").lower(),
          "release memo discloses the known label overlap")
    check("surrounding text: theoretical illustration and non-employment reference stated",
          "The figure is a theoretical illustration: it contains no estimated value, no household data and no result" in welfare
          and "Its reference is non-employment" in welfare, "section 3")
    stale_labels = [label for label in STALE_LABELS
                    if re.search(r"(?<![A-Za-z0-9])" + re.escape(label) + r"(?![A-Za-z0-9])",
                                 report_text_scoped + " " + gallery_text_scoped)]
    ...
    diff = subprocess.run(
        ["git", "diff", "--exit-code", "--",
         ...
         "reports/research_story_build/v13_sections.py", "reports/research_story_build/story_v13.generated.md",
         "reports/JMP_research_story_report_v14.html", "reports/JMP_results_gallery_v14.html",
         "reports/research_story_build/v14_sections.py", "reports/research_story_build/story_v14.generated.md",
         "manuscript/figures/v14/fig_v14_theory_own_set.png"],
        cwd=ROOT, capture_output=True, text=True)
    check("V9 to V14 untouched", diff.returncode == 0, "git diff over V9-V11 report, gallery and source")
    check("no unresolved source tokens", "{{" not in source, "generated V15 Markdown")

    result = {"gate": "V15 reader-facing structure, figures and economic derivation", "status": "PASS" if not failures else "FAIL",
              "main_words": len(re.findall(r"\b[\w’'-]+\b", report_text)), "checks": checks, "failures": failures}
    OUT_JSON.write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
    lines = ["# V15 reader, structure, figures and economics gate results", "", f"Overall: **{result['status']}**", "",
             "| Check | Evidence | Status |", "|---|---|---:|"]
    lines.extend(f"| {r['check']} | {r['evidence'].replace('|', '/')} | **{r['status']}** |" for r in checks)
    if failures:
        lines.extend(["", "## Failures", ""] + [f"- {item}" for item in failures])
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
    print(f"V15 READER GATES {result['status']}: {len(checks)} checks, {len(failures)} failures")
