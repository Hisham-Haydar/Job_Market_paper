"""V14 gate: every rendered figure caption uses the current access definition.
...
OUT_JSON = ROOT / "reports/v14_figure_caption_results.json"
SURFACES = {"report": ROOT / "reports/JMP_research_story_report_v14.html",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="jmp-v14-caption-control-") as tmp:
    ...
    import v14_render_inputs as _inputs
    theory_caption = _inputs.THEORY_CAPTION
    theory_alone = scan("<figure><img alt=\"theory\"><figcaption>" + theory_caption + "</figcaption></figure>")
    planted = report.replace(theory_caption, theory_caption + " Access is region, urban or rural location and year.", 1)
    if planted == report:
        raise SystemExit("theory caption not found in the rendered report")
    ...
    payload = {"gate": "V14 figure captions use the current access definition", "status": status,
               "surfaces": results,
    ...
    print(f"V14 FIGURE CAPTIONS {status}: report {results['report']['captions_scanned']} captions "
