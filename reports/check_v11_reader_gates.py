"""Reader-language, economic-derivation and boundary checks for V11."""
...
import v11_sections as v11  # noqa: E402


REPORT = ROOT / "reports/JMP_research_story_report_v11.html"
GALLERY = ROOT / "reports/JMP_results_gallery_v11.html"
SOURCE = BUILD / "story_v11.generated.md"
OUT_JSON = ROOT / "reports/v11_reader_gate_results.json"
OUT_MD = ROOT / "reports/v11_reader_gate_results.md"
...
def main() -> int:
    ...
    check("implementation detail present in appendix",
          all(t in appendix_text for t in ("Implementation record: estimation", "polishing contracts",
    ...
    check("Stage A superseded quotations preserved", appendix_text.count("Superseded before V9") >= 5,
    ...
    check("V9 and V10 untouched", diff.returncode == 0, "git diff over V9/V10 report, gallery and source")
    check("no unresolved source tokens", "{{" not in source, "generated V11 Markdown")

    result = {"gate": "V11 reader-facing economic derivation", "status": "PASS" if not failures else "FAIL",
              "main_words": len(re.findall(r"\b[\w’'-]+\b", report_text)), "checks": checks, "failures": failures}
    OUT_JSON.write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
    lines = ["# V11 reader and economics gate results", "", f"Overall: **{result['status']}**", "",
             "| Check | Evidence | Status |", "|---|---|---:|"]
    lines.extend(f"| {r['check']} | {r['evidence'].replace('|', '/')} | **{r['status']}** |" for r in checks)
    if failures:
        lines.extend(["", "## Failures", ""] + [f"- {item}" for item in failures])
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
    print(f"V11 READER GATES {result['status']}: {len(checks)} checks, {len(failures)} failures")
