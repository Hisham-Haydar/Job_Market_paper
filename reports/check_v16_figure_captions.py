"""V16 gate: every rendered figure caption uses the current access definition.
...
OUT_JSON = ROOT / "reports/v16_figure_caption_results.json"
SURFACES = {"report": ROOT / "reports/JMP_research_story_report_v16.html",
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="jmp-v16-caption-control-") as tmp:
    ...
    payload = {"gate": "V16 figure captions use the current access definition", "status": status,
               "surfaces": results,
    ...
    print(f"V16 FIGURE CAPTIONS {status}: report {results['report']['captions_scanned']} captions "
