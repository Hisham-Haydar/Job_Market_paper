...
import v13_sections as sections  # noqa: E402
...
REGISTRY = ROOT / "reports/numbers_of_record_v13.json"
REGISTRY_V10 = ROOT / "reports/numbers_of_record_v12.json"
OUT_JSON = ROOT / "reports/v13_number_to_source_results.json"
OUT_MD = ROOT / "reports/v13_number_to_source_results.md"
...
def main() -> int:
    ...
    claim("every V12 registry value unchanged in V13", not changed,
          "numbers_of_record_v12.json vs v13 (" + (", ".join(changed) or "no differences") + ")")
    new_keys = sorted(set(registry) - set(carried))
    claim("V13 adds only figure-value registry keys",
          all(k.startswith("fig13_") for k in new_keys), ", ".join(new_keys))
    ...
    claim("every scalar placeholder resolves", not missing, "V13 registry")
    ...
    sources = {name: {"path": str(path.relative_to(WORKSPACE)).replace("\\", "/"), "sha256": sha(path)}
               for name, path in (("certified ex-ante result record", WEA), ("certified result memo", MEMO),
                                  ("matched-household record", MATCHED), ("V12 predecessor registry", REGISTRY_V10))}
    result = {"gate": "V13 number-to-source", "status": "PASS" if not failures else "FAIL",
              "placeholder_count": len(placeholders), "claims": claims, "sources": sources, "failures": failures}
    OUT_JSON.write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
    lines = ["# V13 number-to-source results", "", f"Overall: **{result['status']}**", "",
             "| Check | Source | Status |", "|---|---|---:|"]
    ...
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
    print(f"V13 NUMBER-TO-SOURCE {result['status']}: {len(claims)} checks, {len(failures)} failures")
    for x in failures:
        print("  FAIL:", x)
