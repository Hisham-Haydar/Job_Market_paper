"""Build V15 surfaces, run the complete release suite, and write one summary.

V15 restores the ORIGINAL theory figure (not the V14 regeneration) and corrects its caption. The deck, rehearsal script and
...
OUT_JSON = ROOT / "reports/v15_release_gate_summary.json"
OUT_MD = ROOT / "reports/v15_release_gate_summary.md"
...
def main() -> int:
    ...
    results = [
        run("report build", [py, "reports/research_story_build/build_v15.py"]),
        run("gallery build", [py, "reports/results_gallery_build/build_v15.py"]),
        run("figure captions use the current access definition, negative control", [py, "reports/check_v15_figure_captions.py"]),
        run("rendered-text language and negative control", [py, "reports/check_v15_rendered_language.py"]),
        run("Stage A ex-ante status and negative control", [py, "reports/check_v15_stage_a_status.py"]),
        run("body/appendix section-title uniqueness and negative control", [py, "reports/check_v15_section_titles.py"]),
        run("reader structure and economic derivation", [py, "reports/check_v15_reader_gates.py"]),
        run("number-to-source", [py, "reports/check_v15_numbers_against_source.py"]),
        run("offline HTML render", [py, "reports/research_story_build/check_v15_render.py"]),
        run("cross-surface synchronisation", [py, "reports/check_v15_synced_surfaces.py"]),
        ...
        run("Python source compilation", [py, "-m", "py_compile",
            "reports/research_story_build/build_v15.py", "reports/research_story_build/v15_sections.py",
            "reports/research_story_build/v15_render_inputs.py", "reports/results_gallery_build/build_v15.py",
            "reports/check_v15_figure_captions.py",
            "reports/check_v15_rendered_language.py", "reports/check_v15_stage_a_status.py",
            "reports/check_v15_section_titles.py", "reports/check_v15_reader_gates.py",
            "reports/check_v15_numbers_against_source.py", "reports/check_v15_synced_surfaces.py",
            "reports/research_story_build/check_v15_render.py", "reports/write_v15_release_manifest.py"]),
        run("V9 to V14 byte-preservation", ["git", "diff", "--exit-code", "--",
    ...
    OUT_JSON.write_text(json.dumps({"release": "V15", "status": status, "gates": results},
                                   indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
    lines = ["# V15 complete release gate summary", "", f"Overall: **{status}**", "",
