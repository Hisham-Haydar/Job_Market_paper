...
import check_v11_rendered_language as audit  # noqa: E402
...
GALLERY = ROOT / "reports/JMP_results_gallery_v11.html"
REGISTRY = ROOT / "reports/numbers_of_record_v11.json"
REGISTRY_V10 = ROOT / "reports/numbers_of_record_v10.json"
OUT_JSON = ROOT / "reports/v11_number_to_source_results.json"
OUT_MD = ROOT / "reports/v11_number_to_source_results.md"
...
def main() -> int:
    ...
    claim("every V10 registry value unchanged in V11", not changed,
          "numbers_of_record_v10.json vs v11 (" + (", ".join(changed) or "no differences") + ")")
    new_keys = sorted(set(registry) - set(carried))
    claim("new registry keys are only source-bound comparison and illustration keys",
          all(k.startswith(("att_", "mh_")) for k in new_keys), ", ".join(new_keys))
    ...
    claim("every scalar placeholder resolves", not missing, "V11 registry")
    ...
    sources = {name: {"path": str(path.relative_to(WORKSPACE)).replace("\\", "/"), "sha256": sha(path)}
               for name, path in (("certified ex-ante result record", WEA), ("certified result memo", MEMO),
                                  ("matched-household record", MATCHED), ("V10 registry", REGISTRY_V10))}
    result = {"gate": "V11 number-to-source", "status": "PASS" if not failures else "FAIL",
              "placeholder_count": len(placeholders), "claims": claims, "sources": sources, "failures": failures}
    OUT_JSON.write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
    lines = ["# V11 number-to-source results", "", f"Overall: **{result['status']}**", "",
             "| Check | Source | Status |", "|---|---|---:|"]
    ...
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")
    print(f"V11 NUMBER-TO-SOURCE {result['status']}: {len(claims)} checks, {len(failures)} failures")
    for x in failures:
        print("  FAIL:", x)
