"""Build the V19 rehearsal pack from the compiled V19 deck and its notes.

Same method as build_rehearsal_script_v16.py: "On slide" is the projected page,
"Say" is the frame's note with number macros expanded from deck_numbers_v19.tex.
Writes reports/rehearsal_pack_v19.md and refreshes reports/rehearsal_pack_v1.md
(the stable rehearsal-script path) to the V19 numbering: main slides are
numbered as on the footline (the title slide is unnumbered), backup slides as
B1, B2, ...  Refuses to write if an internal label from the verifier's V19
...
source = (HERE / "JMP_seminar_beamer_v19.tex").read_text(encoding="utf-8")
numbers = (HERE / "deck_numbers_v19.tex").read_text(encoding="utf-8")
...
document = pymupdf.open(HERE / "build/JMP_seminar_beamer_v19.pdf")
...
out = [
    ...
    "footline of JMP_seminar_beamer_v19.pdf; the title slide is unnumbered.", "",
...
out += ["<!-- Generated by beamer/build_rehearsal_script_v19.py from "
        "beamer/JMP_seminar_beamer_v19.tex and build/JMP_seminar_beamer_v19.pdf. -->", ""]
payload = "\n".join(out)

hits = []
for pattern, case_sensitive in gates.V16_INTERNAL + gates.V19_PROFILE["extra_labels"]:
    for found in re.finditer(pattern, payload, 0 if case_sensitive else re.I):
        ...
        if "JMP_seminar_beamer_v19" in context or "rehearsal_script_v19" in context:
...
for target in (ROOT / "reports/rehearsal_pack_v19.md", ROOT / "reports/rehearsal_pack_v1.md"):
    target.write_text(payload, encoding="utf-8", newline="\n")
print(f"Rehearsal script: {n_main} main + {len(frames) - n_main} backup -> "
      "reports/rehearsal_pack_v19.md and rehearsal_pack_v1.md")
