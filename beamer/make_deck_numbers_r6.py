...
# DECK-V17 (style revision): the V16 values plus the data-slide definitions.
V17_OUT_TEX = HERE / "deck_numbers_v17.tex"
V17_OUT_JSON = HERE / "build" / "v17_number_provenance.json"
V17_MACROS = dict(V16_MACROS, **{
    "VPolicyYear": ("policy_year", "d"),
    "VNDraws": ("n_draws", "d"),
    "VNOcc": ("n_occ", "d"),
    "VHoursFloor": ("hours_floor", "d"),
    "VHoursCap": ("hours_cap", "d"),
    "VWageLo": ("wage_lo", "d"),
    "VWageHi": ("wage_hi", "d"),
})
...
def main_v17() -> int:
    return main_v16(V17_MACROS, V17_OUT_TEX, V17_OUT_JSON, "v17")


def main_v16(macros=None, out_tex=None, out_json=None, tag="v16") -> int:
    macros = V16_MACROS if macros is None else macros
    out_tex = V16_OUT_TEX if out_tex is None else out_tex
    out_json = V16_OUT_JSON if out_json is None else out_json
    ...
    prov: dict = {"registry": str(V15_REGISTRY), "registry_sha256": sha256(V15_REGISTRY),
                  ...
                  "macros": {}}
    tex = [
        "%% deck_numbers_%s.tex -- GENERATED by make_deck_numbers_r6.py --%s." % (tag, tag),
        "%% Do not edit by hand.  Every numeral on a %s slide comes from here, and" % tag.upper(),
        "% every value here is one entry of reports/numbers_of_record_v15.json,",
        "% formatted only (no arithmetic).",
        "",
    ]
    for name, (key, spec) in macros.items():
    ...
    out_json.write_text(json.dumps(prov, indent=2, sort_keys=True), encoding="utf-8",
                        newline="\n")
    print("wrote %s (%d macros from %s)" % (out_tex.name, len(prov["macros"]),
                                           V15_REGISTRY.name))
    return 0
...
# ==========================================================================
# DECK-V19: V18 values, plus parameter identifiers for the backup tables and
# literature items verified against the literature corpus.
#
#   python make_deck_numbers_r6.py --v19
#
# The paper's own numbers still come only from the V15 registry.  Literature
# numbers come only from beamer/literature_benchmarks_v19.json, and every item
# there is re-verified against the corpus Markdown at build time: its quote must
# occur on the recorded PDF page (whitespace- and ligature-normalised), and every
# numeral of the displayed string must occur in the quote or recorded raw value.
# ==========================================================================
V19_OUT_TEX = HERE / "deck_numbers_v19.tex"
V19_OUT_JSON = HERE / "build" / "v19_number_provenance.json"
LIT_RECORD = HERE / "literature_benchmarks_v19.json"
LIT_ROOT = HERE.parent / "JMP_lit_collection" / "Markdowns"


def _lit_norm(s: str) -> str:
    import unicodedata
...
def verify_literature(record: dict | None = None) -> dict:
    """Return {macro: provenance} for every literature item, or raise on any failure."""
    record = json.loads(LIT_RECORD.read_text(encoding="utf-8")) if record is None else record
    out: dict = {}
    for item in record["items"]:
        paper = record["papers"][item["paper"]]
        md = LIT_ROOT / paper["md"]
        text = md.read_text(encoding="utf-8")
        if item.get("pdf_page") is not None:
            pages = re.split(r"<!-- PDF p\. (\d+) -->", text)
            chunks = {int(pages[i]): pages[i + 1] for i in range(1, len(pages) - 1, 2)}
            hay = chunks.get(int(item["pdf_page"]), "")
        else:
            hay = text
        quote = _lit_norm(item["quote"])
        if quote not in _lit_norm(hay):
            raise SystemExit("REFUSED: literature item %s: quote not found on PDF page %s of %s"
                             % (item["macro"], item.get("pdf_page"), paper["md"]))
        source_digits = set(re.findall(r"\d+(?:\.\d+)?", quote + " " + str(item.get("raw", ""))))
        shown = re.findall(r"\d+(?:\.\d+)?", item["display"])
        if item.get("formatting"):
            raw = float(str(item["raw"]).rstrip("%"))
            if format(100 * raw, ".1f") != item["display"]:
                raise SystemExit("REFUSED: literature item %s: display %s is not the recorded "
                                 "formatting of %s" % (item["macro"], item["display"], item["raw"]))
            shown = []
        missing = [d for d in shown if d not in source_digits]
        if missing:
            raise SystemExit("REFUSED: literature item %s shows %s not in its corpus quote"
                             % (item["macro"], missing))
        out[item["macro"]] = {"kind": "literature", "rendered": item["display"],
                              "raw": item.get("raw", item["display"]), "paper": paper["citation"],
                              "corpus_md": paper["md"], "pdf_page": item.get("pdf_page"),
                              "quote": item["quote"], "formatting": item.get("formatting")}
    return out
...
def build_v19() -> dict:
    ...
    for pop, p in _POP.items():
        for row in reg["gallery"]["coefficients"][p]:
            name = _macro_name("VCoef" + pop, row["parameter"]) + "Id"
            out[name] = {"kind": "coefid", "source": "gallery.coefficients.%s[%s].parameter"
                         % (p, row["parameter"]), "raw": row["parameter"],
                         "rendered": r"\texttt{%s}" % _tex_text(row["parameter"])}
    lit = verify_literature()
    clash = set(lit) & set(out)
    if clash:
        raise SystemExit("REFUSED: literature macro names clash with registry macros: %s" % clash)
    out.update(lit)
    return out
...
def main_v19() -> int:
    macros = build_v19()
    tex = ["% deck_numbers_v19.tex -- GENERATED by make_deck_numbers_r6.py --v19.",
    ...
    for name, rec in macros.items():
        tex.append(r"\newcommand{\%s}{%s}" % (name, rec["rendered"]))
    V19_OUT_JSON.parent.mkdir(exist_ok=True)
    V19_OUT_TEX.write_text("\n".join(tex) + "\n", encoding="utf-8", newline="\n")
    V19_OUT_JSON.write_text(json.dumps({"registry": str(V15_REGISTRY),
                                        "registry_sha256": sha256(V15_REGISTRY),
                                        "literature_record": str(LIT_RECORD),
                                        "literature_record_sha256": sha256(LIT_RECORD),
                                        "macros": macros}, indent=2, sort_keys=True),
                            encoding="utf-8", newline="\n")
    kinds: dict = {}
    for rec in macros.values():
        kinds[rec["kind"]] = kinds.get(rec["kind"], 0) + 1
    print("wrote %s (%d macros: %s)" % (V19_OUT_TEX.name, len(macros), kinds))
    return 0


if __name__ == "__main__":
    import sys
    if "--v19" in sys.argv[1:]:
