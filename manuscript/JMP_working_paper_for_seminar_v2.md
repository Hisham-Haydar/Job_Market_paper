...
**Seminar working paper, version 2.** France 2016 (EU-SILC / SRCV, priced through EUROMOD).
...
package, and differs from it in presentation and binding rather than in results.
Every numeral in this version is bound to an entry of
`JMP/reports/numbers_of_record_v1.json` — a generated registry recording, for
each quantity, the frozen artefact it came from, the column or key inside it, and
the value — and the third column of the self-check table names the entry for each
row. The integration verifier reads that column back and checks every key it
names against both the registry and the printed text, so a numeral that drifts
from its artefact fails the build rather than the reader.*
...
couples male leisure block moves it by up to 73 per cent while the environment side
moves by under 3.5 (§8.4). Re-aggregating by sex flips its **sign** for men between
the two reference conventions while the job-access ordering holds under both
(§7.10). And fitting a model with no opportunity object at all barely moves it —
6.4 against 6.3 per cent — while changing the level of inequality by a quarter
(§6.7). Five perturbations of different kinds, all moving the preference
contribution and all leaving the nested order and the environment's composition
alone. Read the preference share as the quantity this design pins least well, and
the environment's internal structure as the quantity it pins best.
...
**The parameterization, stated once.** The singles model is reported on its
**41 active coordinates**. Every one of the 41 is free and estimated; 41 is the
$k$ in every information criterion in this paper; and the phrase "41 estimated
structural parameters", wherever it appears, refers to exactly these. Of the 41,
two rest on an active box bound at the optimum and carry no standard error,
leaving **39 interior** coordinates — the constant $K_{\text{interior}}$ used for
inference, and the number of coordinates resampled in Appendix F.

That is the whole of the reported parameterization. The estimation repository
stores the 41 inside a longer vector that also carries coordinates this sample
cannot identify — the couples leisure block and two year dummies, all of which
have identically zero gradient on a singles-only sample and none of which is
estimated here. Those coordinates are **provenance, not parameters**: they are
recorded once in Appendix A.4, they are never displayed in a singles parameter
table, and no count taken from the storage vector is used anywhere in this paper.
...
**The forward pair** — preference distance 0.021 against an admissible median of
0.783; opportunity distance 0.502 against 0.209, a separation of 2.40×, clean,
and same-sex:

- **A** — a man aged 35–39, high education, no children, intermediate
  urbanisation; observed employed in occupation 4 at 40 hours and €18.37 an hour.
  Estimated leisure weight 4.611, employment probability **0.851**, median wage
  offer €11.38.
- **B** — a man aged 40–44, low education, no children, rural; observed employed
  in occupation 4 at 40 hours and €17.59 an hour. Estimated leisure weight 4.562,
  employment probability **0.349**, median wage offer €11.05.

Two single men, both working 40 hours in the same occupation in the same wage
quintile, whom the model gives essentially the same preference profile — leisure
weights of 4.61 and 4.56 — and very different opportunity: an employment
probability of **0.851 against 0.349**. Note where the difference is *not*: their
median wage offers are €11.38 and €11.05, within thirty cents of each other. The
whole gap is on the participation margin, which on this specification is where
region, urbanisation and the local unemployment rate enter — so this pair is the
household-level face of the geographic split of §7.9. That is panel (A) of
Figure T1, on real estimates.
...
- **A** — a woman aged 35–39, medium education, no children, intermediate
  ...
  Leisure weight 4.708, employment probability 0.861, median wage offer €10.16.
- **B** — a woman aged 45–49, medium education, five children, urban; observed
  ...
  employment probability 0.839, median wage offer €10.26.
...
**A defect was found in the pair-selection code and has been repaired; the pair
above is the repaired one.** The stored profile artefact these figures were drawn
from computed its market-access index from region columns that are identically zero
on this frame, rather than from the region indicators the estimator and the welfare
engine both evaluate — so the participation numbers plotted, and the
opportunity-distance measure that ranks the candidate pairs, omitted each
household's own region term. The gap was exactly that coefficient, at machine
precision. The builder now reads the live indicators, the ranking has been re-run
over the same 19,116 admissible pairs under the same rule, and the pair above is
what that ranking returns.

The repair matters, and it is worth saying how. The region-free ranking had
returned a pair separated by 2.08× on opportunity distance whose visible gap was
largely in the wage offer; the repaired ranking returns a **cleaner** pair,
separated by 2.40×, whose gap is almost entirely in the participation margin at
near-identical wage offers. **The restored variation is geographic**, which is the
same thing §7.9 measures in the aggregate. The reverse pair is unchanged in
identity — the same two women — with participation values that now carry their
region terms.

**No number in the welfare decomposition moved.** The estimation, the exact table,
the reference sensitivity and the geographic split all go through the loader that
rebuilds the indicators and were never affected; the defect was confined to this
illustration and to the ranking that chose which households to draw.
...
0.858 across environments and his money-metric welfare €1,302 to €1,432, a ratio of
1.10. The low-education man's runs **0.345 to 0.621** and **€419 to €864**, a ratio
of **2.06**.

That second range is large and should be read with care. It is not a statement that
moving would double this man's welfare. It is a statement about the *estimated
access block*: a household whose participation probability is already low is on the
steep part of the map from the access index to the employment margin, so a change in
the access environment moves it a long way — and this man's own environment, rural
Est, is the least favourable of the twenty-four. The comparison holds his
preferences, his characteristics and his priced budget fixed and changes only the
estimated access environment, so what it measures is the curvature of the model at
his position, not an effect of place on a person.
...
### 7.10 The decomposition re-aggregated by sex

The decomposition so far is a population statement. It can be re-aggregated inside
subgroups, and one such split belongs in the main results.

**What a subgroup number is, and what it is not.** Every reference object is
computed **once, over the whole singles population, and is common to every group** —
the substituted argument vector, the reference preference block, the frozen
occupation object with the population sex shares, and the endowments-and-needs
reference profile with its equivalence scale. A subgroup number therefore answers
*how much of this group's own money-metric inequality is removed by moving its
households to the common population reference*. It does **not** answer how much
would be removed by moving them to a reference computed inside the group; that is a
different estimand, it would not be comparable across groups, and it is not
computed. Exhaustiveness and the three accounting identities are re-checked inside
every group and pass in all 32 cells.

This is a **within-subgroup** decomposition, not a *contribution of the
characteristic*. Nothing here decomposes inequality into a between-sex and a
within-sex part, and nothing here is a statement about what sex causes. The two
questions are different and only the first is answered.

| group | $n$ | $I^{00}$ | preference share | job access | of which geographic |
|---|---:|---:|---:|---:|---:|
| all singles | 1,555 | 0.134277 | 6.30 ± 0.61 % | 14.90 ± 1.10 % | 13.05 ± 0.74 % |
| **women** | 841 | 0.126034 | 7.79 ± 0.14 % | **9.79 ± 0.65 %** | 13.08 ± 0.85 % |
| **men** | 714 | 0.140776 | $-2.69$ ± 0.35 % | **19.58 ± 1.08 %** | 16.22 ± 0.85 % |

*Final singles model, female reference, raw basis. Bands are the eight-scramble
jackknife — integration precision, carrying no parameter uncertainty.*
**Figure 20** carries all four panels.

**The substantive finding, and it is stable.** **Job access accounts for roughly
twice as large a share of men's measured inequality as of women's** — 19.58 per
cent against 9.79 under the female reference on the raw basis, and 18.44 against
9.02 under the male structural zero. The ordering holds under both reference
conventions and on both bases; the equivalized rows give 16.74 against 7.17 and
15.70 against 6.50. The geographic part of it is larger for men too, 16.22 against
13.08.

Some care about what this says. It is not that men face worse job access — the
decomposition is not a level comparison. It is that **the dispersion of job access
within men does more of the work in explaining inequality among men than the
dispersion of job access within women does among women**, where "does the work"
means the value of the equalisation operator, as everywhere else in §7.

**One subgroup number is reference-sensitive in sign, and no sign statement is made
about it.** The men's preference share is $-2.69$ per cent under the female-primary
reference and $+2.78$ under the male structural zero. That is not the
magnitude sensitivity already on record in §7.5; it is a **sign change**, and it is
exactly what a reference convention can do to a group whose own preference block is
not the reference block. **No statement about the sign of the male preference
contribution is licensed**, and none is made. Women's share moves far less, 7.79
against 10.76.

That the flip appears here rather than in the population number is itself
informative: the population preference share is positive under both conventions
(§7.5), so the sign stability of the headline does not extend to every subgroup of
it. The reference convention is a normative choice, and this is the sharpest
demonstration in the paper of how much can turn on it.

**Two further splits are reported in Appendix E rather than here** — urbanisation
class and the presence of children — because neither carries a finding that
survives the same scrutiny, and both are exploratory. Their numbers are in the
appendix table with the same construction and the same caveats.

**Bands here are integration precision only**, on the same convention as
everywhere else in this paper: they say how well the integral is resolved, not how
precisely the parameters are estimated. Sampling uncertainty in $\hat{\theta}$ is
not estimated anywhere in this paper (§3.6), and it is not estimated here.
...
term, a couples consumption curvature (log consumption by the certified
convention), and a male children-in-leisure term (children on the female leisure
only — the singles structural-zero convention).

**The first of those absences is a limitation of the current couples model and is
stated as one.** The cross-leisure interaction — the term
$\beta_{\ell\ell}\cdot \mathrm{BoxCox}(\ell_{\text{male}})\cdot
\mathrm{BoxCox}(\ell_{\text{female}})$, which is how spouses' leisure would enter
each other's marginal utility — is **absent from this specification**. It is not a
coordinate that was estimated and found small, and it is not a coordinate that was
estimated and then set to zero: it is not in the model at all, and the welfare
pipeline therefore evaluates couples utility at an effective
$\beta_{\ell\ell} = 0$ throughout. Nothing in this paper estimates spouse
cross-leisure complementarity, and no result here should be read as evidence for
or against it — including the results below on the male leisure block, which are
about a *level* term and not about an interaction.

The reason for the absence is identification, not convenience. With the
interaction free the joint information matrix is not positive definite on this
frame; pinning it restores a positive-definite interior block, and when it was
instead fixed at zero in an earlier pooled specification the weak direction did
not disappear but moved to the couples-male leisure coordinates — the same block
that carries the sensitivity reported in §8.4. So the absence and the named
weakness are two views of one identification problem, and neither is repaired by
relabelling the other.

**The extension this calls for, named rather than gestured at.** Identifying
$\beta_{\ell\ell}$ needs a second contrast on the joint leisure margin that this
design does not contain — one observation per household, and both spouses'
leisure entering a single index. The post-seminar extension is therefore a
**joint-leisure identification study**: re-estimate the couples block on a frame
carrying within-household variation in the joint leisure margin, and put
$\beta_{\ell\ell}$ through the same synthetic-recovery criterion the singles
heterogeneity extensions of §5.4 had to satisfy, before any cross-leisure
quantity is reported. Until that study returns, the couples module is reported
with the interaction absent and the effective $\beta_{\ell\ell} = 0$ stated
wherever couples utility is used.
...
**The cross-leisure interaction is absent and the effective
$\beta_{\ell\ell}$ is exactly zero** (§8.1) — so no couples number here bears on
spouse leisure complementarity, which this design does not estimate. The weakly
recovered male leisure block (three of four coordinates marginal at
...
**Spouse cross-leisure, in the couples module.** The couples specification
carries **no cross-leisure interaction**: the term is absent from the model, the
welfare pipeline runs at an effective $\beta_{\ell\ell} = 0$, and this paper
therefore estimates nothing about how one spouse's leisure enters the other's
marginal utility. The absence is forced by identification — the joint information
matrix is not positive definite with the interaction free — and it is bound to the
same weakness as the male leisure block (§8.1, §8.4). The named extension is a
joint-leisure identification study on a frame with within-household variation on
that margin, conditional on satisfying the same synthetic-recovery criterion
before any cross-leisure quantity is reported. Readers should treat every couples result here as conditional on the
interaction's absence rather than as evidence about its size.
...
### A.4 Provenance note: the storage vector

The reported parameterization of the singles model is the **41 active
coordinates** of A.1 — all free, all estimated, all displayed there with their
standard errors. This note records, once, how those 41 sit inside the object the
estimation repository stores, so that a reader comparing this paper against the
repository's own files is not surprised by a different length.

The stored vector has **51 coordinates**: the 41 active ones and **10 further
coordinates that this sample cannot identify** — the couples leisure block and
two year dummies. Each of the 10 carries an identically zero gradient on a
singles-only sample, each is held at a fixed value throughout, and none is
estimated, reported, or counted in any information criterion here. They exist
because one code path serves both the singles and the couples applications, and
the storage layout is shared; they are an artefact of that layout and carry no
economic content in the singles model.

Three consequences, so the distinction cannot leak into a reading of the results.
The likelihood, its gradient and its Hessian are identical whether evaluated on
the 41-coordinate active representation or on the 51-coordinate storage vector
with the 10 held — the compact evaluation reproduces the objective and gradient
bitwise, which is what licenses reporting the 41 alone. Every information
criterion in this paper uses $k = 41$. And the resampling of Appendix F draws the
**39 interior** coordinates, holding the 2 at-a-bound coordinates at
...
## Appendix E — The two exploratory subgroup splits

§7.10 reports the sex split, which is the one that carries a finding stable enough
for the main results. Two further splits were computed on exactly the same
construction and are reported here rather than there, because neither does.
Everything §7.10 says about the estimand applies unchanged: the reference objects
are **population-level and common to every group**, so each row answers how much of
*that group's own* inequality is removed by moving its households to the common
population reference. These are **within-subgroup** decompositions and not
contributions of the characteristic; nothing here is a between-group
decomposition, and nothing here is causal. Bands are integration precision.

**Final singles model, female reference, raw basis.**

| dimension | group | $n$ | weighted share | $I^{00}$ | preference share | job access | of which geographic |
|---|---|---:|---:|---:|---:|---:|---:|
| population | all singles | 1,555 | 100.0 % | 0.134277 | 6.30 ± 0.61 % | 14.90 ± 1.10 % | 13.05 ± 0.74 % |
| urbanisation | urban | 832 | 57.2 % | 0.129481 | 6.81 ± 0.62 % | 13.69 ± 1.11 % | 12.09 ± 0.73 % |
| urbanisation | intermediate | 328 | 21.1 % | 0.141058 | 5.98 ± 0.59 % | 16.09 ± 1.06 % | 14.00 ± 0.78 % |
| urbanisation | rural | 395 | 21.7 % | 0.138017 | 5.44 ± 0.64 % | 15.91 ± 1.08 % | 13.91 ± 0.70 % |
| children | with children | 428 | 23.7 % | 0.125806 | 8.31 ± 0.47 % | 11.30 ± 0.68 % | 11.76 ± 0.69 % |
| children | without children | 1,127 | 76.3 % | 0.136409 | 4.77 ± 0.57 % | 16.10 ± 1.21 % | 13.91 ± 0.78 % |

**Urbanisation.** Within-group inequality is 0.1295 urban, 0.1411 intermediate and
0.1380 rural, and the geographic access contribution is 12.09, 14.00 and 13.91 per
cent of each group's own baseline. The differences are small and the bands overlap.
One point of construction is easy to misread and is worth stating: these are
households **in one urbanisation class still being moved to the common population
reference**, which differs from their own class. The row is therefore not "what
geography does inside rural France"; it is what the population-level geographic
equalisation removes from rural households' own inequality.

**Children.** Households with children have lower measured inequality (0.1258
against 0.1364), a larger preference share (8.31 against 4.77 per cent) and a
smaller job-access share (11.30 against 16.10). The preference-share difference is
the one number here that is larger than its band by a comfortable margin, and it
has an obvious mechanical reading — the specification carries a children-in-leisure
term for single women and none for men, so the preference block simply has more
household-specific content to equalise among households with children. That is a
statement about the specification, not about parenthood, and it is why the split is
exploratory.

**Neither split is promoted.** Both are reported for completeness and because the
...
*The third column names the entries of `JMP/reports/numbers_of_record_v1.json` that carry
the row's numerals. That file is generated, not typed: each of its entries records the
artefact, the column or key, and the value, so a numeral in this paper can be traced to a
frozen artefact in one step. The integration verifier reads this column back, checks that
every key it names exists, and checks that the value the registry holds is the value the
paper prints. An em dash means the registry does not carry that row's numerals; the row is
then bound by its artefact source in the second column alone.*
...
| endowments-needs 58.2 ± 1.5; 53.2–68.9 % across rows | same, `C_needs_over_I00` | `C_needs_female_raw_share, C_needs_male_raw_share, C_needs_female_equivalized_share, C_needs_male_equivalized_share` |
| preference share 3.3–11.0 %, constant sign | same, `C_pref_over_I00` | `C_pref_female_raw_share, C_pref_male_raw_share, C_pref_female_equivalized_share, C_pref_male_equivalized_share` |
...
| the four cells; "not compensating variation and requires no tax reform"; the level-analogue mapping | `02_framework.md` §2.4 (quoted from the R-202 design record) | `state_I00_female_raw, state_I10_female_raw, state_I01_female_raw, state_I11_female_raw` |
...
| current pass: $I^{11}=0.000\mathrm{e}{+}00$ / $-2.035\mathrm{e}{-}31$ | `SPRINT/decision_log.md` Entry 7 §3 | `state_I11_female_raw, state_I11_male_raw` |
...
| draw-count rungs $R\in\{50,100,200,400\}$; largest move 0.21 SE ($R{=}400$), 0.20 ($R{=}200$), 0.56 ($R{=}50$) | `SPRINT/runs/drawcount_s6/s6_drawcount_estimation_v1.json`, `rungs.*.parameter_table` | `drawcount_ladder, drawcount_max_deviation_full_range, drawcount_max_deviation_geq_reference, drawcount_status` |
...
| hours-band constants recovered as tastes: mean absolute difference 0.037, maximum 0.075; fixed cost of work $+0.517$ against $-3.334$ | `rb_step4_misclassification_v1.json`, `positive_side_misclassification` | `rum_bench_const_mad, rum_bench_n_constants` |
| male-minus-female leisure intercept gap $+0.428$ against $-1.991$ | same, `sex_specific_leisure_block.leisure_intercept_gap_male_minus_female` | `rum_leisure_gap_final, rum_leisure_gap_benchmark` |
| baseline inequality 0.101841 against 0.134277 raw ($-24.2$ %) and 0.139869 against 0.165105 equivalized ($-15.3$ %) | `SPRINT/tables/rum_benchmark_decomposition_v1.csv`; `rb_step4_misclassification_v1.json`, `relative_change_in_measured_inequality` | `rum_inequality_drop_raw, rum_inequality_drop_equivalized, state_I00_female_raw, state_I00_female_equivalized` |
| preference share 6.4 % against 6.3 %; difference $+0.1$ points against a band of $\pm 0.4$ | same, `s_pref`, `M_share` | `rum_share_pref_RUMB_raw, rum_share_pref_RURO_raw, rum_share_pref_RUMB_equivalized, rum_share_pref_RURO_equivalized` |
| the three destinations of the omitted market-side share | same, `fraction_of_O_to_preferences`, `fraction_of_O_to_endowments_needs`, `fraction_of_O_leaving_the_total`, over the four arms | `rum_omitted_share_relabelled_as_preferences, rum_omitted_share_relabelled_as_needs, rum_omitted_share_leaves_measured_total` |
...
| numeral | source | record key |
|---|---|---|
| **every percentage in the paper** | `SPRINT/tables/headline_decomposition_v1.csv` (8 rows × 97 columns), mirrored at `headline_decomposition_v1.md` | `C_pref_female_raw_share, C_env_female_raw_share, C_acc_female_raw_share, C_earn_female_raw_share, C_needs_female_raw_share` |
| the four principal states with bands | `SPRINT/decision_log.md` Entry 7 §3 | `state_I00_female_raw, state_I10_female_raw, state_I01_female_raw, state_I11_female_raw, state_I00_female_raw__rqmc_band` |
| exhaustiveness at $0.000\mathrm{e}{+}00$ / $-2.035\mathrm{e}{-}31$; identities at machine precision | same | `state_I11_female_raw, state_I11_male_raw` |
| $I^{10}>I^{00}$ in all four arms; the Owen second-term explanation | same, Entry 7 §3 | `state_I10_female_raw, state_I00_female_raw, equalization_pref_only` |
...
| the emitted headline sentence and its guard | same, Entry 8 §3 | `C_env_female_raw_share, C_pref_female_raw_share, equalization_env_only, equalization_pref_only` |
...
| numeral | source | record key |
|---|---|---|
| geographic access 0.017528 ± 0.000887 raw and 0.016289 ± 0.000831 equivalized; other access 0.002476 ± 0.000891 and $-0.000307$ ± 0.000489 | `SPRINT/tables/nested_geographic_access_v1.csv`, female-primary rows | `geo_share_of_I00_raw, geo_share_of_I00_equivalized` |
| geographic share of job access 87.62 % ± 3.89 raw and 101.92 % ± 3.11 equivalized (integration band; the CR1 parameter interval is [74.02, 98.04] and [85.98, 111.07], Appendix F); male-reference 90.13 % ± 4.16 and 105.71 % ± 3.33 | same, `C_geo_share_of_C_acc` | `geo_share_of_C_acc_raw, geo_share_of_C_acc_equivalized` |
| geographic share of baseline inequality 13.05 % ± 0.74 raw and 9.87 % ± 0.55 equivalized; male-reference 12.24 % ± 0.65 and 9.13 % ± 0.47 | same, `C_geo_over_I00` | `geo_share_of_I00_raw, geo_share_of_I00_raw__band, geo_share_of_I00_equivalized, geo_share_of_I00_equivalized__band` |
...
| employment-opportunity mass 0.768–0.858 and 0.655–0.809; welfare €1,736–1,837 and €1,301–1,333; ratios 1.058 and 1.025 | `SPRINT/figures/figG02_regional_access_environments.csv`; `gn_step3_illustration_v1.json` | `regional_opportunity_mass_range_A, regional_opportunity_mass_range_B` |
| 24 access environments, 48 profile-by-environment cells; hours, occupation and wage-offer summaries each take one value per profile | same, `structural_constancy_under_a_geographic_counterfactual` | `n_regional_environments` |

### (g3) Appendix F — CR1 parameter-uncertainty propagation

| numeral | source | record key |
|---|---|---|
| $K = 100$ draws, acceptance rate 1.000; 39 interior coordinates drawn, 10 pinned and 2 at-a-bound held; rejection not clipping | `SPRINT/runs/param_uncertainty/pu_step1_draws_v1.json`; `SPRINT/tables/parameter_uncertainty_v1.md` | `n_params_interior, n_params_at_bound, n_params_provenance` |
| preference share 6.30 % with CR1 interval [4.22, 10.89]; environment share 93.70 % with [89.11, 95.78] | `SPRINT/tables/parameter_uncertainty_v1.csv`, raw basis | `s_pref_female_raw__cr1_interval, s_env_female_raw__cr1_interval` |
| equivalized preference share 7.98 % with [5.23, 12.21]; environment 92.02 % with [87.79, 94.77] | same, equivalized basis | — |
| geographic share of job access 87.62 % with **[74.02, 98.04]**; equivalized 101.92 % with [85.98, 111.07] | same, `C_geo_share_of_C_acc` | `C_geo_over_I00_female_raw__cr1_interval, geo_share_of_C_acc_raw` |
...
| the $C_{\mathrm{pref}}$ range 0.0014–0.0075 around 0.0052 | same, Entry 9 §8 | `couples_C_P_equivalized, couples_C_P_equivalized__rqmc_band` |
...
| 43,767 EU-LFS 2016 records aged 20–60; the band partitions coincide on integer hours; the model side reproduces the hours-fit figure to $5.6\times10^{-17}$ | `xh1_table_record_v1.json`, `lfs_source`, `band_alignment_finding`, `gate_fig02_reproduction` | `external_validation_n_rows` |
| relative offer weights 0.86 / 0.09 / 0.13 / 0.12 against the statutory band | same, `opportunity_weights` | — |
| the statutory band at 37.2 % (external), 34.3 % (observed sample), 34.6 % (model-implied); women 40.6 % vs men 34.6 % external, 36.7 % vs 32.4 % model-implied | `SPRINT/figures/figX1_external_hours_lfs_validation.csv`, panel b | `external_validation_statutory_band_lfs, external_validation_statutory_band_obs, external_validation_statutory_band_pred` |
...
| the delivered `wants_more_hours` field carries code 1 (No) and is the complement of its own name | `xh2_moments_record_v1.json`, `label_notes` `XH2-LABEL-1` and `wishmore_inversion_check`; checked, not asserted — the builder refuses to write unless the field reproduces the code-1 share on all seven bands | `external_validation_wishmore_status` |
| both exported CSV/JSON source pairs are now on this side (`SOURCE_EXPORTED_CSV`); each JSON reproduces every CSV cell, while the retained xlsx audit copies differ in 2 of 900 M1–M5 cells and 3 of 588 hours-band cells, worst 0.09 of an hour | same, `sources`, `export_pair_verification` and `transcription_verification` | — |
...
whose gap is geographic (§7.8). And the **sex subgroup decomposition** has returned
and is admitted to the main results at §7.10, with the two exploratory splits in
Appendix E.

**Closed at the previous pass.** The geographic split of job access is reported in
§7.9 with its own table, its generated reading and two figures; no share is quoted
