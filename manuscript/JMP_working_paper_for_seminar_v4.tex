...
**[AWAITING CORRECTED WELFARE NUMBERS]** The decomposition of well-being inequality into job access, earning opportunities, resources and needs awaits re-evaluation on the corrected frame and is not reported here.
\end{abstract}
...
The estimated Hessian is \textbf{positive definite and full rank} in both populations --- rank 41 of 41 and rank 47 of 47 --- with smallest eigenvalues 0.166893 and 0.0770744. The condition numbers in raw coordinates are high, about 384,000 and about 3.9 million, which is what one expects when coefficients are carried on very different natural units rather than a sign of weak identification: rank is full, the curvature is strictly positive, and the optimum is unique across starts. A scaled conditioning diagnostic, which reports the same curvature after putting the coordinates on comparable units, is commissioned and not yet on record. Standard errors are household-clustered and were computed fresh at this optimum.
