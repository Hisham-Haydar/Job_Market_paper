...
This appendix records the construction of the reported standard errors, the
treatment of constrained and non-estimated coordinates, the full-precision test
statistics, and the pre-registered diagnostics that the welfare stage inherits.
Numerical values are rendered from the accepted estimation and inference
artifacts; nothing here is recomputed from microdata. Passages marked
*(Interpretation)* are the authors' reading rather than a restatement of an
artifact.
...
All nineteen members of the attempt manifest were inspected. The set of recorded
gating failures is empty, and every diagnostic
...
**Clustering, correctly described.** The P2a sample is constructed as a
single-adult sample, so each household contributes one likelihood term and each
cluster has size one; this is a design feature of the sample, documented in the
sample section of the paper rather than in the inference artifacts recorded
here. Conditional on it, household-level clustering is not correcting for
within-cluster dependence. What the sandwich delivers is **robustness to
misspecification of the likelihood** — the divergence between the outer product
of scores and the negative Hessian — and it is described that way rather than as
a dependence correction. *(Interpretation.)*
...
section: where a coordinate is at an active bound, the unconstrained symmetric
normal approximation is not licensed in the constrained direction, so ordinary
two-sided Wald inference is not reported for it and the reported inference is
conditional on the observed active set.
...
*Source: `phase5_regional_tests.csv`, all rows and columns. Source-route note:
the H0-G robust p-value is carried along two accepted source routes that differ
in the sixteenth significant figure — `6.500461827702638e-08` on the table
route, used above, and `6.500461827702641e-08` on the claim route. Both imply
the same paper-facing rendering, `6.5e-08`, which is the value used in the prose
of the main section, and neither affects any reported verdict. No correction to
any source artifact is implied.*
...
location by education and potential experience. The occupation coefficients
reported in the main section belong to the opportunity-access block and are not
part of the wage-density location. The LOC4
...
Five limits, stated explicitly because each is a plausible misreading.
...
2. **A failure to reject is not a finding of zero.** The restrictions in H0-B
   and H0-C are not rejected. That records the strength of the evidence against
   those restrictions at the resolution and specification studied; it is not
   evidence that the NUTS-1 or urbanisation coordinates are zero. The battery
   likewise cannot rule out regional dimensions that are not among the tested
   coordinates, and it does not validate the parsimonious access specification
   against a richer external covariate set, since no such comparison was
   estimated.
3. **Agreement between variance estimators is not evidence of correct
...
4. **Statistical rejection is not causal identification.** All statements are
...
5. **No normative interpretation is carried in this appendix.** The
...
## A.10 Items not carried by the accepted inference artifacts

The following are outside the scope of the accepted estimation and inference
artifacts and are therefore **not** asserted on their authority anywhere in the
inference section or this appendix. They are listed so that a reader does not
mistake their absence for a claim, and so that the data and specification
sections can supply them from their own sources.

- Sample-construction filters for the P2a estimation sample beyond the household
  count of 1,555. The single-adult design of the sample — invoked in the main
  section and in A.3 in describing what household clustering does and does not
  do — is documented in the sample section of the paper, not in the inference
...
- The numerical values of the S-10 perturbations `Δⱼ` and sensitivity values
  `θ^sens_j`, which are recorded by the welfare analysis before its execution
...
- Estimates, standard errors and confidence-interval endpoints in the main
  ...
  and therefore renders as `0.855` at three decimals.
- Wald statistics in the main section's Table 1 are rendered to two decimal
  places, and p-values to three significant figures, or to two significant
  figures in scientific notation where the value is below 0.001. Table A.2 above
  gives the same statistics at full precision.
...
- Headline figures quoted at coarser precision in the prose of the main
  section — `beta_E_gsur = -1.105`, its robust interval
  `[-1.51, -0.70]`, robust Wald `37.45` and `29.21`, robust *p*-values `4.7e-05`
  and `6.5e-08` — carry the same renderings as the accepted result record.
